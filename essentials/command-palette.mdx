...
- **Direct Kubernetes Links:** Navigate directly to common Kubernetes resource types: Deployments, Pods, Services, Ingresses, ConfigMaps, Secrets, RBAC, Storage, Helm Releases, and more
...
| Command | Description |
|---------|-------------|
| `Dashboard` | View fleet health, map, and cost rollups |
| `Activity & Inbox` | Review work that needs attention |
| `Applications` | Manage app delivery from repo to workload |
| `Cloud Cost` | Jump to fleet cost rollups on the dashboard |
| `Clusters` | Go to cluster list |
| `Charts` | Browse Helm charts |
| `Chart Repositories` | Manage Helm chart repositories |
| `Credentials` | Manage credentials |
| `Organisations` | Team management |
| `Stack Profiles` | Manage reusable stack profiles |
| `Profile` | Your profile settings |
| `Profile Authentication` | Manage passkeys, security keys, authenticator apps, and recovery codes |
| `API Tokens` | Manage API tokens |
| `Support Tickets` | View and track support requests |
...
| `Cluster Access` | Manage scoped Kubernetes API access |
| `Kubernetes Access Settings` | Configure kubectl and Kubernetes API access |
| `Cluster Integrations` | Configure metrics, logs, traces, and cloud cost signals |
| `Cluster Nodes Settings` | Manage node and control-plane settings |
...
| `Generate random password` | Generate a secure password |
| `Create Cluster` | Start creating or importing a cluster |
| `Reconcile Cluster` | Open the reconcile dialog for the selected cluster |
| `Support` | Open the support request dialog |
...
| `Search Pods` | Find pods in the selected cluster |
| `Search Credentials` | Find stored credentials |
| `Search Add-ons` | Find add-ons in the selected cluster |
| `Search Manifests` | Find manifests in the selected cluster |
