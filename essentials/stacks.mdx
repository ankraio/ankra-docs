...
```yaml
domain: ${{ ankra.my_variable }}
```

<Info>
Stack variables have the highest precedence. They override cluster and organisation variables with the same name. See [Variables](/essentials/variables) for the full precedence hierarchy.
</Info>
...
Dependencies order components **within a single stack**. To order whole stacks against each other, use [deploy waves](#deploy-waves).

---

## Deploy Waves

Deploy waves order the **stacks on a cluster** relative to each other, the way dependencies order components inside one stack. Give a stack a **deploy wave** - a whole number, `0` and up - and:

- Stacks in a lower wave finish before any stack in a higher wave starts.
- Teardown unwinds in reverse: higher waves are removed before lower ones.
- Stacks without a wave keep the existing unordered behaviour and deploy in parallel.

This lets platform prerequisites land before the workloads that need them - without hand-wiring cross-stack dependencies. A common layout:
...
<Steps>
  <Step title="Open the stack">
    In the Stack Builder, open the stack you want to order and choose **Configure Deploy Wave**.
  </Step>
  <Step title="Pick a wave number">
    Enter a whole number. Lower waves deploy first; stacks sharing a wave deploy together.
  </Step>
  <Step title="Deploy">
    Ankra deploys wave by wave, waiting for each wave to finish before starting the next.
  </Step>
</Steps>

In an ImportCluster YAML, set `deploy_wave` on a stack:
...
The wave is validated by `ankra cluster apply`, preserved by partial patches (`ankra cluster addons upgrade`, `ankra cluster manifests upgrade`), and shown as a **Wave** column in `ankra cluster stacks list`.
