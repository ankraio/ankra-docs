 ...
 **Workers** Count and flavor (e.g., 2x `b2-15`)
  ...
  <Step title="Create & Track Progress">
    Click **Create** to start provisioning. A live progress view tracks every step network creation, gateway setup, control plane provisioning, worker provisioning, Kubernetes installation (k3s or kubeadm), and Ankra Agent setup. The cluster appears with an **offline** state until provisioning completes, then transitions to **online**.
  </Step>
</Steps>
...
```bash
curlX POST https://platform.ankra.app/api/v1/clusters/ovh \
...
| `etcd_flavor_id` | `b2-15` | kubeadm `external` topology only. Instance flavor for dedicated etcd nodes |
...
OVH clusters can be provisioned with either **k3s** (default) or **kubeadm**.
...
By default kubeadm runs etcd **stacked** on the control plane nodes. For larger clusters you can run etcd on dedicated instances by setting `etcd_topology` to `external`:
...
curl -X POST https://platform.ankra.app/api/v1/clusters/ovh \
  ...
  -d '{
    ...
    "control_plane_flavor_id": "b2-15",
    "node_groups": [
      {"name": "default", "instance_type": "b2-15", "count": 2}
    ],
    "distribution": "kubeadm",
    "etcd_topology": "external",
    "etcd_node_count": 3,
    "etcd_flavor_id": "b2-15"
  }'
```

The equivalent CLI flags are `--distribution kubeadm`, `--etcd-topology external`, `--etcd-node-count 3`, and `--etcd-flavor-id b2-15`.
...
curlX POST https://platform.ankra.app/api/v1/clusters/ovh/<cluster_id>/scale-workers \
H "Authorization: Bearer $ANKRA_API_TOKEN" \
H "Content-Type: application/json" \
d '{"worker_count": 4}'
```
...
You can upgrade the Kubernetes version on all nodes in an OVH cluster. Upgrades are applied to control plane nodes first, then workers. Both k3s and kubeadm clusters are supported.
...
```bash cURL
curlX POST https://platform.ankra.app/api/v1/clusters/ovh/<cluster_id>/upgrade-k8s-version \
H "Authorization: Bearer $ANKRA_API_TOKEN" \
H "Content-Type: application/json" \
...
```bash
ankra cluster ovh upgrade <cluster_id> v1.31.0
```
...
```bash cURL
curlX DELETE https://platform.ankra.app/api/v1/clusters/ovh/<cluster_id> \
H "Authorization: Bearer $ANKRA_API_TOKEN"
```
