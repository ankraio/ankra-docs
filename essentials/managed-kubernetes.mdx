...
Every choice in the create flow is fetched **live from the provider with your credential** — regions, Kubernetes versions (with support windows), node sizes, and prices, including spot capacity and quota-aware availability where the provider exposes it. Nothing is hardcoded, so new instance families and price changes show up automatically.

<CodeGroup>

```bash CLI
ankra cluster managed options --provider eks --credential-id <aws-credential-id>
```
...
```bash
# see what the provider offers first
ankra cluster managed options --provider gke --credential-id <gcp-credential-id>
...
ankra cluster managed create \
  ...
  --node-pool-size e2-standard-2 \
  --node-pool-count 2
```

The `--provider` flag accepts `doks`, `uks`, `gke`, `ovh-mks`, `aks`, and `eks`. Optional flags include `--kubernetes-version`, `--node-pool-autoscaling-min/--node-pool-autoscaling-max`, `--ha` (DOKS), `--cluster-plan` (UKS), `--description`, and the GitOps trio (`--gitops-credential-name`, `--gitops-repository`, `--gitops-branch`).
...
Already running managed Kubernetes? Discover clusters at the provider and adopt them into Ankra without touching them.

<CodeGroup>

```bash CLI
# list clusters at the provider for a credential
ankra cluster managed discover --provider eks --credential-id <aws-credential-id>

# adopt one into Ankra
ankra cluster managed import \
  --provider eks \
  --credential-id <aws-credential-id> \
  --cluster-id <provider-cluster-id> \
  --name imported-prod
```
...
Ankra lists the upgrade targets the provider actually offers for your cluster's current version:
...
In the portal, **Settings** → **General** → **Kubernetes Version** shows available upgrades with their support windows. The provider upgrades the control plane first, then rolls node pools. Downgrades are not supported.
...
Because the provider owns the control plane, some lifecycle operations that exist for [self-managed clusters](/essentials/cluster-settings) are **not applicable** here:
...
The Ankra AI assistant has full parity with the portal and CLI for managed Kubernetes. Ask it to:
