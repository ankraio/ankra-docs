...
4. The upgrade process is straightforward with minimal disruption
...
Control how members sign in and what they can do.

### Sign-in providers

Members sign in with single sign-on - **Google, Microsoft, Apple, or GitHub** - or with an email-and-password login. Each member manages their connected providers and second factors on their own [profile](/essentials/user-profile#two-factor-authentication).

### Require two-factor authentication

Administrators can require **two-factor authentication** for the organisation. When enabled, members without a second factor are prompted to enrol one before they can continue - in the browser and when running `ankra login`.

### Roles & access

Assign members [roles](/essentials/roles-and-access) at the organisation, a single cluster, or a [cluster group](/essentials/roles-and-access#cluster-groups), and define custom roles. A role can bundle Kubernetes access that Ankra provisions across every cluster in scope.

### Audit log

Every access change - invitations, role assignments, cluster-group edits, and revocations - is recorded in the organisation **Audit Log**, so you can see who changed what and when.
