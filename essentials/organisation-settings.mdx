...
For full documentation on viewing, filtering, resolving, and analyzing insights, see the [AI Insights](/essentials/ai-insights) page.
...
When enabled, every member of the organisation must have two-factor authentication (2FA) set up to use the platform. Members who have not yet enrolled are guided to set up an authenticator before they can continue, and enrolled members are prompted for a code at sign-in.

<Steps>
  <Step title="Navigate to Security">
    Go to **Organisation Settings** → **General** and find the **Security** section.
  </Step>
  <Step title="Toggle Require Two-Factor Authentication">
    Enable the **Require two-factor authentication** toggle. Only administrators can change this setting.
  </Step>
</Steps>

<Info>
Existing members keep access while they enrol — they are prompted to set up an authenticator the next time they use the platform, not signed out. New members are prompted as part of joining.
</Info>

### Member two-factor status and reset

The organisation members list shows each member's 2FA status — **enrolled**, **not enrolled**, or **unknown** (when the status cannot be read momentarily).

If a member loses their authenticator, an administrator can reset their 2FA as a break-glass action. This removes all of the member's authenticators so they can re-enrol on their next sign-in.

<Steps>
  <Step title="Open Members">
    Go to your organisation's **Members** list.
  </Step>
  <Step title="Reset a member's 2FA">
    Use the **Reset 2FA** action on the member's row and confirm. Administrators cannot reset their own 2FA here — manage your own authenticators from your [User Profile](/essentials/user-profile).
  </Step>
</Steps>

<Warning>
Resetting a member's 2FA removes every authenticator on their account. If your organisation requires 2FA, the member is prompted to enrol again before they can continue.
</Warning>
