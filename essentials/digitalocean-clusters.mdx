...
<CardGroup cols={2}>
  <Card title="DigitalOcean API Credential" icon="key">
    A DigitalOcean personal access token with read/write permissions. See [DigitalOcean Credentials](/integrations/credentials#digitalocean-api-credentials).
  </Card>
 ...
 **Workers** Count and size (e.g., 2× `s-2vcpu-4gb`)
  ...
  <Step title="Create & Track Progress">
    Click **Create** to start provisioning. A live progress view tracks credential setup, VPC creation, SSH key deployment, bastion provisioning, droplet creation, k3s installation, and Ankra Agent setup. The cluster appears with an **offline** state until provisioning completes, then transitions to **online**.
  </Step>
</Steps>
...
You can upgrade the Kubernetes (k3s) version on all nodes in a DigitalOcean cluster. Upgrades are applied to control plane nodes first, then workers.
...
In addition to self-managed K3s on droplets, Ankra can connect provider-native managed Kubernetes services:
