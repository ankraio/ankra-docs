...
<CardGroup cols={2}>
  <Card title="Morpheus API Credential" icon="key">
    The Morpheus appliance URL and a long-lived API access token. See [Morpheus Credentials](/integrations/credentials#morpheus-api-credentials).
  </Card>
  <Card title="SSH Key Credential" icon="lock">
    An SSH public key for instance access. You can provide your own or let Ankra generate one. See [SSH Key Credentials](/integrations/credentials#ssh-key-credentials).
  </Card>
</CardGroup>
...
Both options are configured on the credential — see [Morpheus Credentials](/integrations/credentials#morpheus-api-credentials).
...
<Note>
Cost estimates are not available for Morpheus clusters — neither in the creation wizard nor in [Cloud Cost](/essentials/cloud-cost).
</Note>
