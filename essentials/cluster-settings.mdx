...
Node group edits work on both running and stopped clusters. On a **stopped** cluster the change is recorded against the saved topology; the new shape is provisioned the next time you start the cluster from the General settings tab. Adding a node group while stopped no longer auto-starts the cluster.
...
Both changes apply the next time you start the cluster from the General settings tab — the cluster stays stopped until you do.
...
At the bottom of the Nodes tab, the **All Nodes** table lists every server Ankra manages for the cluster — control plane, workers, bastions and gateways — with their role, node group, instance type, location, state and private IP.

Click any row to open a detail view that shows the resource's full definition, the latest provider-side information Ankra has captured (cloud provider IDs, IPs, etc.), reconciler data, and the graph edges connecting that node to other resources. Soft-deleted entries from a stopped cluster are listed too, so you can inspect the saved topology even before re-provisioning.
...
- The cluster comes back online when you click **Start Cluster** on the General settings tab

**Editing while stopped:**

You can change controller count (1 ↔ 3) and modify node groups while the cluster is stopped. Changes are stored against the saved topology — no cloud infrastructure is provisioned yet. Click **Start Cluster** on the General settings tab when you're ready to apply them.

**Recreating:**

When you click **Start Cluster**, Ankra:
