...
Add a second factor to your account to protect it even if your password is compromised. 2FA applies to your account across every organisation you belong to. Ankra supports:

- **Authenticator app** — a time-based one-time password (TOTP) from any authenticator app (such as 1Password, Authy, or Google Authenticator).
- **Passkeys** — device biometrics such as Touch ID, Face ID, or Windows Hello.
- **Security keys** — roaming FIDO keys such as a YubiKey.
...
<Steps>
  ...
  <Step title="Start setup">
    Click **Set up** under **Two-factor authentication**. A QR code and a secret are shown.
  </Step>
  <Step title="Scan and confirm">
    Scan the QR code with your authenticator app (or enter the secret manually), then type the six-digit code to confirm.
  </Step>
  <Step title="Save your recovery code">
    A one-time recovery code is displayed. Store it somewhere safe — it lets you regain access if you lose your authenticator.
  </Step>
</Steps>

<Warning>
Your recovery code is shown only once. If you lose it, regenerate a new one from the Authentication page — regenerating invalidates the previous code.
</Warning>

### Adding a passkey or security key

Passkeys and security keys are enrolled at sign-in: when you log in on a supported device, Ankra offers to add the device as a passkey, and organisations that require 2FA prompt you to enrol a factor before continuing. Once enrolled, passkeys and security keys appear alongside your authenticator app on the **Authentication** page, where each is shown with its device name.

### Managing authenticators

- **View** every enrolled factor — authenticator apps, passkeys, and security keys — from **Profile** → **Authentication**.
- **Add or remove** factors at any time. Removing a factor stops it from being requested at sign-in.
- **Regenerate** your recovery code if you suspect it has been exposed.

<Info>
If your organisation requires two-factor authentication, you cannot remove your last remaining authenticator — add a replacement first. If you are locked out, an organisation administrator can reset your 2FA so you can re-enrol. See [Organisation Settings](/essentials/organisation-settings).
</Info>
