 ...
 **Distribution** Kubernetes distribution (`k3s` or `kubeadm` — see [Kubernetes Distribution](#kubernetes-distribution))
...
| `etcd_server_type` | `cx33` | kubeadm `external` topology only. Server type for dedicated etcd nodes |
...
Hetzner clusters can be provisioned with either **k3s** (default) or **kubeadm**.
...
curl -X POST https://platform.ankra.app/api/v1/clusters/hetzner \
  ...
  -d '{
    ...
    "etcd_node_count": 3,
    "etcd_server_type": "cx33"
  }'
```

The equivalent CLI flags are `--distribution kubeadm`, `--etcd-topology external`, `--etcd-node-count 3`, and `--etcd-server-type cx33`.
...
You can upgrade the Kubernetes version on all nodes in a Hetzner cluster. Upgrades are applied to control plane nodes first, then workers. Both k3s and kubeadm clusters are supported.
...
- Downgrades are not supported; rolling back a Kubernetes version would require an etcd snapshot restore.
...
```bash
ankra cluster hetzner upgrade <cluster_id> v1.31.0
```
