...
Run the following command to download and install the latest Ankra CLI binary for your system. This script automatically detects your OS, fetches the correct release, and places the CLI in your PATH (e.g., ~/bin or /usr/local/bin):

```sh
bash <(curl -sL https://artifact.infra.ankra.cloud/repository/ankra-install-public/cli/install.sh)
```
