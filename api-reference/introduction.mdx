...
* Revoke a personal token anytime from **Profile → API Tokens**, and a [service token](/guides/service-tokens) from **Organisation → Service tokens**.
* Prefer separate tokens for different use cases (e.g., CLI vs. CI/CD).
* A service token is pinned to its organisation: the `X-Ankra-Organisation-Id` override that `ankra --org` uses is refused for it.
...
* Use a [service token](/guides/service-tokens) for automation, so the credential is not tied to one
  person's membership and can be reviewed and revoked by any administrator.
