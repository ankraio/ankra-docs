...
<Warning>
A scheduled stop behaves like pressing **Stop cluster** yourself. On Hetzner, OVHcloud, UpCloud, DigitalOcean, Proxmox VE and HPE Morpheus the cluster's state is captured first - an encrypted etcd snapshot the next start restores - unless the schedule's **Preserve cluster state** is unticked (API `preserve_state: false`, CLI `--preserve-state=false`), in which case data stored only in the cluster itself is lost. On the other providers the virtual machines are terminated as before, or powered off and kept on Scaleway and AWS. The **Scale down to 0** mode keeps the control plane and needs no snapshot. The **Pause** mode (API `stop_mode: "pause"`, CLI `--stop-mode pause`; k3s clusters on Hetzner, UpCloud and DigitalOcean) powers every server off and keeps it with its disks instead, so nothing needs capturing and the start powers the servers back on - they keep billing while paused. See [Stop cluster](/platform/cluster-settings#stop-cluster) for what is kept and what a start restores.
</Warning>
