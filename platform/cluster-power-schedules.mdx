...
<Warning>
A scheduled stop terminates the virtual machines at the provider like a manual **Stop cluster** with **Preserve cluster state** unticked: it does not yet capture the cluster's state snapshot, so data stored only in the cluster itself (Secrets, ConfigMaps, custom resources, persistent volume claim bindings) is not preserved across a scheduled stop. Use a manual stop for a stateful cluster until schedules gain the option. See [Stop cluster](/platform/cluster-settings#stop-cluster) for what is kept and what a start restores.
</Warning>
