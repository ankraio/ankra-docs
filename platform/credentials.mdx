...
<CardGroup cols={2}>
  ...
  <Card title="AWS" icon="aws" href="/platform/credentials/aws">
    IAM role or access keys for cost, inventory, EKS, and self-managed k3s clusters on EC2.
  </Card>
...
| AWS | Cost, inventory, EKS, and self-managed k3s clusters on EC2 | `ankra credentials aws` | [AWS](/platform/credentials/aws) |
| Google Cloud (GCP) | Cost, discovery, and GKE | — (UI/API) | [GCP](/platform/credentials/gcp) |
| Azure | Provision AKS | — (UI/API) | [Azure](/platform/credentials/azure) |
...
# List every credential in the organisation
response = requests.get(
    "https://platform.ankra.app/api/v1/org/credentials",
    headers=headers
)

# Create a credential: the path names the provider, the body carries
# what that provider needs (a Hetzner credential is an API token)
response = requests.post(
    "https://platform.ankra.app/api/v1/credentials/hetzner",
    headers=headers,
    json={
...
Every provider has its own `POST /api/v1/credentials/{provider}` with the fields that provider needs; the API reference lists them.
