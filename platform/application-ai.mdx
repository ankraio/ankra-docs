...
<Info>
The panel needs an application linked to a GitHub repository with the [Ankra GitHub App](/integrations/github) installed on that repository's owner. Without one it renders the reason instead of the lanes: the application has no repository yet, the App is not installed for the owner (install it under **AI** → **Settings** → **Connections**), or the application is on a provider the lanes cannot mint a repository token for - AI lanes are available for GitHub App repositories today.
</Info>

## The lanes

| Lane | What it does | Runs when |
|---|---|---|
| **Pull request review** | Reviews the diff and posts a verdict with its findings as one comment that updates in place | A pull request is opened or updated |
| **Code review** | A deeper pass over the branch itself, with no pull request involved | A push to a branch in scope |
| **Actions review** | Reads the failed job logs of a red pipeline and explains what broke | A workflow run fails |
| **Org skills review** | Checks the change against your organisation's [skills](/platform/ai-skills) and conventions | Alongside the pull request review, in the same comment |
| **Demo URL** | Deploys a [preview environment](/guides/pr-preview-environments) per pull request and comments its public URL | A pull request is opened or updated |

Every lane is off for a repository nobody has configured. A fresh source-control connection must never start reviewing, replying, or deploying on its own, so each lane is switched on deliberately - here, or for the whole connection under [AI code review](/platform/ai-code-review).
...
| **Max runs per day** | Every lane except Demo URL | A cap on the lane's runs for this repository, counted since midnight UTC across every pull request, requested reviews included. Leave it blank, or set `0`, for no cap. On the pull request review it is checked before the per-pull-request cap, so a brand-new pull request can be declined on its first push once the repository is over it - see [the two budgets](/platform/ai-code-review#the-two-budgets). |
...
<Note>
Mentioning Ankra in a pull request comment always gets an answer: a mention reply is a direct request from a human, so it is never filtered by branch scope, draft state, or either review budget. The run still counts toward the budgets.
</Note>
...
- [AI code review](/platform/ai-code-review) - the organisation-wide switches, review model, and review instructions these lanes inherit from
