...
Watch the assistant build and troubleshoot a stack.

<YouTube id="4Y5Zcza1QG0" title="How to use Ankra AI Assistant for Cloud Native Kubernetes Stacks" />
...
| **AGENTS.md** | Operational learnings your team and the AI have recorded per add-on and manifest - see [AGENTS.md](/platform/agents-md) |
...
<CardGroup cols={3}>
  ...
  <Card title="Agentic" icon="robot">
    The assistant can act, but every change is gated. Each write step pauses for your confirmation with a preview of what will change - unless you switch the conversation's autonomy to Auto, which lets low-risk writes run with a receipt.
  </Card>
...
In **Agentic** mode, when the assistant wants to make a change it stops and shows a confirmation card describing the action, its blast radius, and - for deploys - a diff of what will change. On the default **Manual** autonomy, nothing happens until you confirm.
...
Agentic mode carries a per-conversation **autonomy** setting, available once AI autonomy is enabled for your organisation:

- **Manual** (the default) - every write stops at a confirmation card, exactly as described above.
- **Auto** - low-risk, reversible writes execute immediately and land in the conversation as a receipt instead of a card. That includes a stack deploy whose validated change set removes nothing - new add-ons, new manifests, configuration changes to what is already deployed - executed with its full bill of materials attached to the receipt.

What never auto-executes, whatever the setting:

- **Cluster lifecycle** - create, scale, upgrade, stop, and delete always show a confirmation card. Cost is a human decision.
- **Removals** - a stack deploy that would remove or replace a deployed add-on or manifest falls back to the confirmation card.
- **Unvalidated changes** - a deploy whose pre-flight validation fails is never auto-executed.
- **Protected resources** - writes into protected system namespaces such as `kube-system` are refused outright in every mode.

Every auto-approved action is recorded in the [audit log](/guides/audit-log) with its autonomy attribution, so the trail shows both what ran and why it was allowed to run without a card.

<Note>
This per-conversation setting is separate from the organisation-wide [AI Autonomy](/platform/ai-autonomy) controls, which govern auto-remediation and agents, and from the **Auto** entry in the model picker, which only routes between models.
</Note>
...
### One tool surface everywhere

The assistant's tool surface covers the core platform lifecycle end to end - clusters, Kubernetes workloads, Stacks, add-ons, applications, and credentials - and the same registry backs the [MCP server](/platform/mcp-server), so external clients get the same capabilities under the same gates. The full list lives in the [MCP Tool Reference](/platform/mcp-tools).

Chat with Ankra over Slack, Microsoft Teams, or SCM comments uses this same tool registry and the same permission gates as in-product chat - there is no separate configuration, and nothing extra to grant or restrict per channel.
