...
Deploying an application or setting one up from a repository takes minutes to converge, which is longer than a single chat turn lasts. Rather than have a tool sit and wait, the assistant registers a **watch** on the work and ends its turn. When the work reaches a terminal state, the watch wakes the conversation with what it observed and the assistant reports back. The waiting happens between sessions, so closing the tab or losing the network does not lose the result.

This is what makes a whole request work as one sentence:

```text
Deploy avura to staging and tell me when it is live
Link github.com/my-org/my-repo and make it live
```

Three things make the report trustworthy rather than a guess:

- **The watch is registered from the confirmed action, not by the model.** Application writes are confirmation-gated, so Ankra derives the watch from the action you approved. The assistant cannot forget to wait, and cannot wait on the wrong thing.
- **Headlines are composed from what was observed.** A failure is diagnosed before it is reported, and a timeout names the step it reached rather than only reporting that time ran out. Setup that finishes while still carrying a publishing blocker is reported as success with the blocker named.
- **A deploy that can never run is closed early.** Deploying to a cluster whose agent has stopped checking in is accepted by the API and then never executes. The assistant checks agent liveness before proposing work (`get_ai_environment` also reports which cluster is the organisation's default target), and a watch on such a deploy closes immediately with the reason instead of waiting out its timeout.

Watches, waiting sessions, and actions awaiting your confirmation all appear in [Activity & Inbox](/platform/activity-inbox).
