...
For cost reconciliation, Ankra also reads the account's billing:

| Resource | Operations | Why it's used |
|----------|------------|---------------|
| Invoices | read | Import each month's invoice, and the running month's invoice preview, so Ankra can set what DigitalOcean billed against its own estimate (`ankra cost reconcile`) |
...
If cost reconciliation shows the DigitalOcean credential as **no billing access**, the token lacks the `billing:read` scope: DigitalOcean refused to list its invoices, so what it billed is shown as unknown, never as zero. Add the scope to a new token and rotate it in; the next import (every six hours) reads the invoices.
