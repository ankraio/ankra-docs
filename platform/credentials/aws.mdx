...
The role's trust policy, session cap and external ID binding are the same as the other two roles. The self-managed level does not include `eks:*`: a credential that should do both EKS and self-managed clusters is two credentials in Ankra, one per stack.
