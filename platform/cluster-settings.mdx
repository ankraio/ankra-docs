...
- All virtual machines are terminated at the provider
...
Use **Start cluster** to re-provision the saved topology. You can restore the entire cluster or start only the control plane first. Ankra creates fresh virtual machines, installs Kubernetes, and reconciles the configured stacks and addons.

The cluster gets fresh infrastructure with new servers and IPs. Previously running workloads are not preserved unless their data lives outside the terminated virtual machines, but stacks and addons configured in Ankra are redeployed automatically.
