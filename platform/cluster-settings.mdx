...
- Cluster access grants and kube tokens issued to members of the current organisation
- Notification routes, mutes and subscriptions for the cluster
- Security report schedules and trusted AI actions
- Cluster group memberships and the GitOps repository connection
...
The move is refused while operations are running on the cluster, while the cluster is a member of a cluster mesh, for playground clusters, and when the destination already has a cluster with the same name. The reason is shown in the dialog.
...
<Info>
Organisation-level credentials, Helm registries and git connections are not copied. Add-ons that reference a credential keep working, but reconnect the GitOps repository and any metrics or log sources in the destination organisation.
</Info>
