...
Most CSI drivers default to `Delete`, so deleting a PVC destroys the volume behind it. That includes deletes Ankra performs on your behalf: removing a manifest from a stack, or renaming one (Ankra treats a rename as delete plus create, see the [ImportCluster reference](/reference/importcluster#spec-stacks-manifests)), deletes the objects in that manifest, and with `Delete` the data goes with them.

A StorageClass's `reclaimPolicy` cannot be changed after creation, so to protect new volumes add a second class with `reclaimPolicy: Retain` and use it for stateful workloads. Volumes that already exist can be switched one at a time, without any downtime:

```bash
ankra cluster patch pv <pv-name> --type merge \
  --patch '{"spec":{"persistentVolumeReclaimPolicy":"Retain"}}'
```

With `Retain`, deleting the PVC leaves the PersistentVolume and its data in place as `Released`, so it can be bound again.
...
<Tip>
**Reclaim Policy:** Use `Retain` for production data to prevent accidental deletion. Existing volumes: `ankra cluster patch pv <pv-name> --type merge --patch '{"spec":{"persistentVolumeReclaimPolicy":"Retain"}}'`.
</Tip>
