...
The same page lists every cluster's own delegated Ankra domain (`<cluster-id>.<org-id>.<ankra-domain>`) with its provisioning state. **Remove** hands a cluster's domain back for teardown - the step required on every cluster before [switching the organisation's Ankra domain](/concepts/applications#the-preview-url) - and **Enable domain** brings a cluster (back) under the organisation's current Ankra domain; external-dns picks a newly enabled domain up on the cluster's next cloud-provider pass.

<Note>
**external-dns on a cluster manages that cluster's Ankra domain and nothing else.** The credential Ankra provisions when you create a cluster with public DNS included is scoped to `<cluster-id>.<org-id>.<ankra-domain>`, so an Ingress on any other zone - one of your own domains, or another cluster's - is filtered out by the DNS provider before a record is written. The sync still logs `All records are already up to date`, because from external-dns's point of view there is nothing in scope to publish, so a hostname being dropped looks the same as one being current.

Ingress hostnames on your own domains therefore need a publisher of their own, and an unpublished name also delays cert-manager: an HTTP-01 challenge is answered over the hostname being certified, so no certificate is issued for a name that does not resolve yet. Declare the zone with your own DNS credential - once, for every cluster in the organisation - and Ankra runs a second, zone-pinned external-dns for it on each cluster: see [serving a zone in your own DNS account from every cluster](/concepts/applications#serving-a-zone-in-your-own-dns-account-from-every-cluster).

When the declared zone covers the organisation's **Preview domain**, that domain becomes the cluster's public domain: `${{ ankra.cluster_domain }}` in stacks and profiles, Ankra AI's hostname advice, the deploy wizard's default hostname and `ankra cluster domain` all report it instead of the generated Ankra domain, and the "hostnames will not resolve" warning on the Preview domain clears.
</Note>

The CLI carries the same operations, plus the declarations for zones in your own DNS account:

```bash
ankra org dns zones                          # list every cluster domain and its state
ankra cluster domain <cluster>               # read one cluster's domain (creates nothing)
ankra cluster domain <cluster> --enable      # create it
ankra cluster domain <cluster> --remove      # hand it back for teardown
ankra org dns credentials create --name my-dns --webhook-provider-url <url>   # your own external-dns webhook credential
ankra org custom-dns-zones add --zone example.dev --credential my-dns         # serve the zone from every cluster, present and future
ankra cluster custom-dns-zones add <cluster> --zone example.dev --credential my-dns   # or from one cluster only
```
