...
<Steps>
  ...
  <Step title="Add a record">
    Enter a label (for example `logs`), pick a record type (CNAME, A, or TXT), and enter the target. Your organisation's zone is appended to the label automatically.
  </Step>
...
<Info>
Any organisation member can view the record list; only administrators can add or delete records. A record's own zone must be active before a record can be created under it - new organisations provision this automatically.
</Info>
