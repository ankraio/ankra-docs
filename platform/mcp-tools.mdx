...
| `wait_for_execution` | `mcp:read` | Yes | Block up to 50 seconds for an execution to settle, then return its details, or `timed_out: true` with the last observed state |
...
| Tool | Scope | Cluster-scoped | Description |
| --- | --- | --- | --- |
| `list_credentials` | `mcp:read` | No | List every credential across all providers, with availability and sync state; a GitHub App credential also reports its `installation_id` |
...
| `list_related_repositories` | `mcp:read` | No | The repositories stated as related for AI code review under one GitHub App installation |
| `relate_repositories` | `mcp:write` | No | State that two repositories are related, so a code review of either may read the other (organisation admin) |
| `unrelate_repositories` | `mcp:write` | No | Remove a stated relationship, in whichever order it was stated (organisation admin) |
...
The organisation-wide CVE surface with CISA KEV and EPSS exploitation intelligence, the bill of materials behind it, the disposition and compliance decisions on top of it, and the cluster, stack, pod and add-on drill-downs; `get_security_reports` / `get_compliance_reports` above stay the per-cluster Trivy summaries. Every read needs `security.read`; the writes re-check `security.triage` or `security.manage` the way the portal does, and each is confirmed before it runs. Tools that name a `cluster_id` take it as an argument, so an MCP client can read any cluster of the organisation without binding one.
...
| `list_security_finding_occurrences` | `mcp:read` | No | Where one finding occurs: every cluster, workload, container and image, one row per occurrence, filtered by scan state (active or resolved) and cluster |
...
| `list_security_workloads` | `mcp:read` | No | Workloads carrying findings, one row per container image with cluster, namespace, add-on attribution and actionable counts, with the findings filters |
| `list_security_namespaces` | `mcp:read` | No | Namespaces of the fleet or of one cluster with scanned workloads, images, running pods, SBOM coverage and actionable findings |
| `list_security_pods` | `mcp:read` | No | The running pods of one namespace on one cluster, each container joined to the findings of its scanned workload (an unscanned container is not a clean one) |
| `list_cluster_security_stacks` | `mcp:read` | No | Every stack of one cluster with its attribution status, scope, actionable and known-exploited findings and SBOM coverage, plus the outside-any-stack row |
| `get_cluster_stack_security` | `mcp:read` | No | One stack's posture on a cluster: status, scope, CVE posture across its workloads, exploited findings, top actionable findings and bill-of-materials picture |
| `get_cluster_pod_security` | `mcp:read` | No | One pod container by container: image, scanner coverage, actionable CVEs by severity, known-exploited and fixable-severe counts, bill of materials |
| `get_cluster_addon_security` | `mcp:read` | No | One add-on's posture on one cluster: attributed workloads and images, observed and actionable findings, dispositions in force, scanner state |
| `list_security_sbom_components` | `mcp:read` | No | The fleet's packages grouped by name, version and ecosystem with licences, licence risk tier, where each runs and the findings that name it |
| `list_security_sbom_images` | `mcp:read` | No | Images that publish a bill of materials, with base OS, component count, where they run and their attributed findings |
| `get_security_sbom_image` | `mcp:read` | No | One image's full bill of materials: identity, base OS, the containers running it and every component with licences |
| `list_security_sbom_containers` | `mcp:read` | No | Every running container, init containers included, joined to its image's bill of materials or marked absent |
| `list_security_sbom_image_findings` | `mcp:read` | No | The CVEs named on one image: package and fixed version, KEV listing, EPSS, disposition and spread |
| `get_application_security_versions` | `mcp:read` | No | Every pushed image version of an application with its bill of materials, findings, licence exposure and where it runs |
| `list_security_dispositions` | `mcp:read` | No | The organisation's disposition policies: selector, disposition, reason, expiry, expire-when-fixed and lifecycle status |
| `preview_security_disposition` | `mcp:read` | No | What a disposition would cover before it is written: the selector, the occurrences it matches across clusters and workloads, and the observations it excludes |
| `create_security_disposition` | `mcp:write` | No | Acknowledge a finding (security.triage) or accept its risk (security.manage), scoped to the add-on the occurrence is attributed to, with reason and review deadline |
| `update_security_disposition` | `mcp:write` | No | Change a policy's reason, review deadline or expire-when-fixed flag (security.manage) |
| `revoke_security_disposition` | `mcp:write` | No | Withdraw a policy with a reason so every finding it covered returns to open (security.manage) |
| `get_security_compliance_overview` | `mcp:read` | No | The compliance rollup across every scanned cluster: benchmark pass/fail counts, config-audit totals, scanner freshness and the clusters with no data |
| `list_security_compliance_frameworks` | `mcp:read` | No | The curated frameworks (GDPR, ISO 27001, SOC 2, NIST CSF), whether each is enabled and its live score |
| `get_security_compliance_framework_report` | `mcp:read` | No | One framework control by control, live or for a recorded month |
| `set_security_compliance_framework` | `mcp:write` | No | Enable or disable a framework for the organisation (security.manage); enabling starts scoring and monthly recording |
| `get_cluster_policy_violations` | `mcp:read` | No | One cluster's pod-security policy violations read live: failing resources per policy, totals by severity, and whether the baseline is in audit or enforce |
| `set_cluster_security_policy_mode` | `mcp:write` | No | Switch a cluster's pod-security policies between audit and enforce; enforce can reject pods at admission |
| `get_cluster_network_policy_overprivilege` | `mcp:read` | No | One cluster's NetworkPolicy over-privilege score on ingress and egress, computed live from its policies and workloads |
| `enable_security_baseline` | `mcp:write` | No | Publish the curated security baseline stack on a cluster: trivy-operator plus the pod-security policies in audit mode, committed to its GitOps repository |
| `get_cluster_security_history` | `mcp:read` | No | One cluster's daily security posture trend from its stored snapshots, oldest first: findings by severity, fixable critical/high, workloads and namespaces scanned, risk score and the actionable / acknowledged / accepted-risk split per day (`days`, 1 to 365, default 90) |
| `get_cluster_security_report_schedule` | `mcp:read` | No | The cluster's scheduled security report: whether one is configured, weekly or monthly, the email recipients, enabled or paused, when it last went out and when it is next due |
| `set_cluster_security_report_schedule` | `mcp:write` | No | Store the cluster's scheduled security report: frequency, up to 20 recipients and whether sending is enabled; replaces the whole schedule (clusters.write) |
| `send_cluster_security_report_now` | `mcp:write` | No | Email the cluster's security report to its scheduled recipients within the next minute; refuses when no schedule is configured (clusters.write) |
...
| `create_aws_cluster` | `mcp:write` | No | Create and provision a self-managed Kubernetes cluster on AWS EC2, in a VPC Ankra creates or one you adopt - runs the create preflight first (closed beta; withdrawn from the catalog until the `aws_provider` flag is on for the organisation) |
...
Day-2 operations for Ankra-provisioned clusters on every self-managed provider (Hetzner, OVHcloud, UpCloud, DigitalOcean, Scaleway, AWS EC2, Proxmox VE, HPE Morpheus). These tools accept `cluster_name` or `cluster_id` directly, so they work from any conversation.
...
| `update_node_group` | `mcp:write` | No | Change a node group's autoscaling bounds, labels and taints in one call, or its instance type on its own (that replaces every node). Scales nodes, not pods |
