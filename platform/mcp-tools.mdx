...
| `create_aws_cluster` | `mcp:write` | No | Create and provision a self-managed Kubernetes cluster on AWS EC2, inside a VPC you own - runs the create preflight first (closed beta; withdrawn from the catalog until the `aws_provider` flag is on for the organisation) |
