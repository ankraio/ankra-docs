...
Read-only while the organisation's backups feature is enabled; vault creation and restores stay in the portal and CLI.
...
| `decide_pending_action` | `mcp:write` | No | Approve or reject a parked action; approving resumes the session that proposed it (designated approvers only) |
| `get_ai_settings` | `mcp:read` | No | The active provider, which providers are configured (never the keys), and the autonomy pause state |
