...
### Declaring deployment order

`create_stack`, `update_stack`, and `validate_stack` take the Stack specification as a structured object, and every add-on and manifest in it accepts a `parents` array of `{name, kind}` dependency edges. `kind` is lowercase - `addon` or `manifest` - and must match the referenced member's type; a capitalised `Addon` is silently ignored rather than rejected, so the schema pins the accepted values.

**A member with no parents deploys in parallel with every other parentless member.** That is the default, and it is why a chart that needs a namespace, Secret, ConfigMap, CRD, operator, or database provided by another member of the same Stack will race it and fail unless you declare the edge. Typical edges:

- Namespace, Secret, and ConfigMap manifests are parents of the add-ons that consume them
- cert-manager is a parent of its ClusterIssuer manifest
- an operator add-on is a parent of the custom resources it reconciles
- a database add-on is a parent of the application that connects to it

`validate_stack` reports which members were left with no dependency edges, so you can check the ordering before anything is applied. On `update_stack`, omitting `parents` on an existing member preserves its current edges and passing `[]` clears them - restate the full parent list whenever you re-send a member that has dependencies. Use `update_stack` to add the missing edges to a Stack that was created without them.

[Dependency management →](/concepts/stacks#dependency-management)
...
| `get_ai_environment` | `mcp:read` | No | The organisation's AI environment: which cluster is the staging (default) target, whether its agent is online, demo and workspace TTL defaults, and the demo base domain |
| `update_ai_environment` | `mcp:write` | No | Apply admin-gated AI environment overrides (staging cluster, TTL defaults, demo domain and ingress settings) |

<Note>
Call `get_ai_environment` before proposing a deploy the user did not pin to a named cluster - it is the only way to learn the organisation's default target. It also reports `staging_cluster_online`: a deploy to a cluster whose agent has stopped checking in is accepted and then never runs, so an offline agent is a reason to stop rather than to start.
</Note>
