...
Click a pod to open its page. The header carries the pod's state as chips - phase, QoS class, the GPU it asks for (`1× nvidia.com/gpu`), restarts with how recently the last one happened, and the manifest sync status when Ankra manages it - over a fact strip with age, namespace, owner, node, pod IP and service account.

The **Overview** tab reads top to bottom:

- **Tiles** - containers ready, restarts, and the CPU, memory and GPU the pod requests against its limits.
- **Containers** - one card per container (init and ephemeral containers included, with a kind chip): state and how long it has been in it, restarts, the previous termination (`Previously OOMKilled (exit 137)`), the image with one-click copy, requests → limits per resource including extended resources such as `nvidia.com/gpu`, ports, the startup / readiness / liveness probes with their inferred status, the command line, and shortcuts to the container's logs, volume mounts and environment variables.
- **Conditions** - one row per condition; the reason and the kubelet's message only appear on a condition that needs attention.
- **Placement** - node, node and pod IPs, QoS class, priority, node selector, affinity kinds and tolerations.
- **Runtime** - service account and token mounting, restart policy, termination grace period, DNS, security context, host access (host network / PID / IPC) and image pull secrets.
- **Volumes** - every volume with its type and source (PVC claim, ConfigMap, Secret, EmptyDir with medium and size limit, …), linking to the **Volumes** tab for the mounts.
- **Labels** and **Annotations** - collapsed, grouped by prefix, searchable, click to copy.

The other tabs are **Logs**, **Terminal**, **Metrics** (with a Prometheus metrics source connected), **Events**, **Volumes** and **Manifest**.
...
<Steps>
  <Step title="Navigate to a Resource List">
...
|--------|-------------|
| Name | Node hostname |
| Status | Ready / NotReady and scheduling state |
| Roles | Control plane, worker, etcd |
| Version | Kubelet version |
| Age | Time since node joined the cluster |
...
<Steps>
  ...
  <Step title="Remove old node group">
