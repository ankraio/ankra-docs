...
The same answer can be sent on the API with `POST /api/v1/org/ai-tickets/{ticket_id}/decision` and a body of `option_key` (one of the offered keys) and/or `answer` (required when no option is chosen). A ticket that is not waiting on a choice answers `409`.
