...
<Note>
The Logs feature provides real-time log streaming and analysis for all your Kubernetes workloads, helping you debug issues and monitor application behaviour.
</Note>
...
- **Customize display** with font size, line wrapping, and colour highlighting
...
When a **Log Source** (Loki, Elasticsearch or OpenSearch) is connected under **Settings → Data Sources**, the cluster **Logs** page gains a **Search** tab for time-ranged queries across your log store. See [Log Sources](/integrations/log-sources) for setup and credential guidance.
...
- **On** Log levels are colour-coded
...
For more details on AI-powered debugging, see [Kubernetes AI Troubleshooting](/platform/kubernetes-ai-troubleshooting).
