...
- **Cluster vCPU Summary** - per-cluster breakdown of worker and total vCPU. Clusters that have sent no node data recently (for example because their agent is disconnected or the cluster was torn down) are marked **Not reporting** instead of showing vCPU counts - they do not accrue usage while in that state.
...
<AccordionGroup>
  ...
  <Accordion title="Why is my cluster marked 'Not reporting'?">
    The Ankra agent on that cluster has not synced node data recently, so Ankra cannot see its capacity. This usually means the agent is disconnected, the cluster was deleted outside Ankra, or node syncing is failing. A non-reporting cluster accrues no usage. Reconnect the agent to resume metering, or delete the cluster from Ankra if it no longer exists.
  </Accordion>
</AccordionGroup>
