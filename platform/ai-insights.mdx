...
AI Insights differ from [AI Incidents](/platform/ai-incidents) in that they are generated proactively by scheduled scans rather than triggered by alert rules.
...
<Steps>
  <Step title="Open the Autonomy settings">
    Go to **AI** → **Settings** → **Autonomy**.
  </Step>
  <Step title="Enable Proactive AI insights">
    Toggle **Proactive AI insights** on. The same section holds the detection thresholds and notification routing.
  </Step>
</Steps>
...
The portal reads and updates insights over these session routes (browser session cookie; write operations carry the CSRF header). There is no bearer-token `/api/v1` twin for them yet, so scripts and agents reach insights through the MCP tool `get_cluster_insights` instead.

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/clusters/{cluster_id}/ai-insights` | GET | List insights with filtering and pagination |
| `/clusters/{cluster_id}/ai-insights/count` | GET | Get insight counts by status |
| `/clusters/{cluster_id}/ai-insights/digest` | GET | Analytics digest (MTTR, trends, breakdowns) |
| `/clusters/{cluster_id}/ai-insights/{insight_id}` | GET | Get insight detail |
| `/clusters/{cluster_id}/ai-insights/{insight_id}` | PATCH | Update insight status |
| `/clusters/{cluster_id}/ai-insights/{insight_id}/related` | GET | List related insights |
| `/clusters/{cluster_id}/ai-insights/{insight_id}/feedback` | POST | Submit feedback |
| `/clusters/{cluster_id}/ai-insights/bulk-update` | POST | Bulk update insight statuses |
...
<CardGroup cols={2}>
  <Card title="AI Incidents" icon="bell" href="/platform/ai-incidents">
    Alert-triggered AI analysis with root cause and recommendations.
  </Card>
  <Card title="AI Assistant" icon="message-bot" href="/platform/ai-assistant">
