...
- **Ask for a review.** A mention is a direct request from a human, so it is never filtered by draft state, branch scope, or either review budget. It runs even on a draft and even past the **Auto reviews / PR** cap, and it is counted against both budgets. It does not yet update the commit's check run - see [The two budgets](#the-two-budgets).
...
<Warning>
**A requested review does not yet clear the check run.** Mentioning `@ankraai review` on a head that was skipped for budget runs the full review and updates the review comment and its `Reviewed commit` footer, but the **Ankra AI review** check run on that commit keeps its **Review skipped** conclusion. A merge gate that requires the check run to pass is therefore not cleared by the mention; lift the budget and push a new head instead. A gate that reads the review comment's footer is cleared.
</Warning>

<Warning>
**Saving an application override of the Pull request review lane resets the cap to 5.** Overriding that lane on the [application's AI settings](/platform/application-ai) pins the whole lane, and the override does not carry **Auto reviews / PR**, so a cap you cleared on the repository rules stops applying to that repository. After saving the override, re-clear the cap on the repository's rule.
</Warning>
