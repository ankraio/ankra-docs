...
<Tabs>
  ...
  <Tab title="GitLab">
    Paste an access token with `read_api` and `read_repository`, optionally naming a self-managed instance URL. Ankra validates the token, then shows the webhook URL and secret to register in GitLab by hand.

    Webhook registration is manual for GitLab, and the secret is shown once. The [GitLab integration](/integrations/gitlab) walks through both, including which token scope you need for Ankra to be able to post its review.
  </Tab>
