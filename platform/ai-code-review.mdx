...
<Warning>
**The merge hold needs Merge when green.** The capability that gates merge-when-green has no control in the portal, the CLI, or the API today, so it is off for every organisation and the line holds nothing until that switch ships; sequence the merges yourself until then. The review's reading of the line does not depend on it.
</Warning>
