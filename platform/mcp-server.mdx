...
MCP is stateless: each `tools/call` is one request with nothing parked between calls. Three consequences follow, and the server's `instructions` field repeats them to the model on `initialize`.
