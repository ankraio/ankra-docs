...
- **License:** The chart's license as an SPDX identifier

---

## Chart Licenses

Every chart version's overview shows a **License** field with the license's SPDX identifier (for example `Apache-2.0` or `MIT`). Ankra reads it from the chart's `artifacthub.io/license` annotation in `Chart.yaml`, and when the annotation is missing, detects the license from the `LICENSE` or `COPYING` file inside the chart package.

Copyleft licenses - the AGPL and GPL family - are visually highlighted, so a chart whose license carries obligations for derived works stands out before you deploy it.
