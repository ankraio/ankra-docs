...
A stack's detail page has a **Security** tab that reads the whole stack as one object: every CVE, every CISA-listed exploited vulnerability and every bill of materials behind the workloads its add-ons and manifests deploy, without opening the add-ons one by one.
...
The tab has three views, and the view and its filters live in the page URL so a filtered list can be handed to someone:

- **Overview** — the actionable occurrences by severity, the distinct findings, the **known-exploited** count, the fixable severe count and the accepted risk, each a link to the findings view already filtered; a **Known exploited (CISA KEV)** list of the stack's exploited findings, earliest CISA deadline first, each opening the finding detail; the **software bill of materials** panel (components, images, and how many of the stack's running containers have a bill of materials); a **By member** table with each add-on and manifest's matched workloads, actionable critical/high, KEV count, containers with and without a bill of materials and component count, linking into the Security Center narrowed to that member's namespace; and a **Fix first** shortlist ranked CISA-listed, then severity, then fix availability.
...
- **Components** — every package in the images the stack's workloads run, searchable, with a **Vulnerable only** switch and the findings that name each component.

The **By member** table is also the way down to Kubernetes. **Show workloads** on a matched member lists the workloads that member deploys - the Deployments, StatefulSets, DaemonSets, CronJobs and Jobs its release identity or declared objects resolved to - each with its pods, scan state, severe actionable findings, known-exploited count and how many of its containers have a bill of materials. A workload opens on its own **Security** tab, its **Bill of materials** link opens the Security Center's containers list scoped to that workload, and **Show pods** lists the pods it runs right now, each opening on the pod's [Security tab](#pod-security). Every link from a security surface to a Kubernetes object lands on that object's Security tab rather than its overview, so the path cluster → stack → member → workload → pod → container never leaves the security view. The workloads read is `GET /org/clusters/imported/{cluster_id}/stacks/{stack_name}/security/workloads`; the cluster's stack list is `GET /org/clusters/imported/{cluster_id}/security/stacks`, both with `/api/v1` twins. On the CLI, `ankra security stacks --cluster <cluster>` prints the stack list, `ankra security stack <name> --cluster <cluster>` the stack with its members and their workloads, and `ankra security pod <namespace> <pod> --cluster <cluster>` one pod container by container; Ankra AI and the MCP server answer the same questions through `list_cluster_security_stacks`, `get_cluster_stack_security` and `get_cluster_pod_security`.

The tab never renders an absent answer as a clean one. A cluster the scanner has not reported on reads **not scanned**; a stack whose members resolve to no object in the resource cache reads **unmatched** (it may not be deployed yet, or its workloads carry no Helm release identity); workloads that were attributed but carry no report yet read **no reports yet**; a member with no matched workload is marked **unmatched** in the table rather than shown with zeros; and when the CISA catalog has not been synced, the known-exploited figure reads unknown rather than zero. The same read is available on the API as `GET /org/clusters/imported/{cluster_id}/stacks/{stack_name}/security`, with `/findings` and `/sbom` lists beside it and `/api/v1` twins for API tokens.

---

## Pod security

A pod's detail page has a **Security** tab (shortcut `s`) that reads the pod container by container: every container and init container it runs, each with the CVEs the scanner attributes to the workload container behind it and the bill of materials of its image. The owner chain is walked the way the namespace breakdown walks it (Pod → ReplicaSet → Deployment, Pod → Job → CronJob), so a Deployment's report on its ReplicaSet reaches the pod.

The summary shows the pod's actionable occurrences by severity, how many of its containers the scanner covers, the known-exploited and fixable-severe counts, and how many containers have a bill of materials with the component and image totals. The **By container** table lists init containers first with the image, readiness and restarts, the scan state, actionable findings by severity, the KEV count and the bill of materials (components and OS, or **Absent**). **Findings** on a container opens the image's own CVE list, and **Components** opens its package inventory with the SBOM export - the same views the Security Center's SBOM tab uses - so the reader drills from the pod to the exact package without leaving the page. A container without a bill of materials still opens its CVE list; the package inventory is what is missing. The container you opened and the section you are on live in the page URL (`?activeTab=security&container=<kind>/<name>&section=vulnerabilities`), so a link from a count elsewhere - a stack member's workload, a namespace's pod list - arrives on that container's findings, and the back button returns to the pod.

Nothing absent reads as clean: a container no report covers is marked **Not scanned**, a container whose image has no stored bill of materials reads **Absent**, a pod with neither reads **no report yet**, an unscanned cluster says so, and an unsynced CISA catalog leaves the known-exploited figure unknown. The same read is on the API as `GET /org/clusters/imported/{cluster_id}/security/pods/{namespace}/{pod_name}`, with an `/api/v1` twin for API tokens.
