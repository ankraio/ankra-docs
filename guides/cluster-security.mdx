...
**Images** lists every image with a bill of materials: OS, component count, the workloads, clusters and namespaces running it, the actionable findings attributed to it, and when the bill of materials was generated. Opening an image shows its identity (reference, digest, registry, CycloneDX version), the workload containers running it, and its full component list with the same search, ecosystem and findings-first controls.
...
The API exposes `GET /api/v1/org/security/sbom` (components; `search`, `package_type`, `vulnerable`, `cluster_id`, `namespace`, `image`, `sort`, `order`), `GET /api/v1/org/security/sbom/images`, and `GET /api/v1/org/security/sbom/image?image=<digest or reference>`; every response carries a `coverage` block with `scanned_clusters` against `clusters_with_sbom`.
