...
Organisation-wide vulnerability disposition rules: active, expiring, fix-available, mitigated, unmatched, expired, and revoked. Edit, renew, or soft-revoke with a fresh server-side match preview.

These are Ankra's own dispositions for CVE findings. They are unrelated to Kubernetes `NetworkPolicy` objects, which are covered under [Network Exposure](#network-exposure).
