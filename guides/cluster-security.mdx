...
- **Findings** - the **Exploitability** column, the **Known exploited (CISA KEV)** filter chip, and the **Exploitability** sort (KEV listing first, then EPSS, then severity). The detail sheet explains the listing and shows EPSS and KEV under **Advisory details**.
- **Overview** - a **Known exploited** scorecard tile, and remediation candidates ranked known-exploited first.
- **Clusters**, **Workloads**, and the add-on security panel - a known-exploited count beside the actionable and fixable counts.
...
- **API and MCP** - `known_exploited`, `kev_date_added`, `kev_due_date`, `kev_ransomware_use`, `epss_score`, and `epss_percentile` on every finding and remediation candidate, a `known_exploited` filter on the findings and workloads lists, and `known_exploited` counts on the overview, cluster, workload, and add-on reads. Pull them into your own reporting with a personal API token (`ankra tokens create`) against `GET /api/v1/org/security/overview`, `/findings`, `/findings/{finding_id}`, `/workloads`, and `/clusters` on `https://platform.ankra.app` - for example `GET /api/v1/org/security/findings?known_exploited=true&sort=exploitability&order=desc`. The AI security context carries the same fields.
