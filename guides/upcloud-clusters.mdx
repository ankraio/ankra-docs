...
### `LoadBalancer` Services do not work on a meshed cluster

<Warning>
A meshed cluster advertises each node's overlay address as its Kubernetes
`InternalIP`, and the UpCloud cloud controller manager builds load balancer
backends from exactly that address. An UpCloud load balancer is not part of the
WireGuard mesh, so it cannot reach `172.30.x.x` and every backend stays down.

This fails **silently**: the Service is assigned an `EXTERNAL-IP`, the CCM
reports `EnsuredLoadBalancer`, and the load balancer then answers `503` or
refuses connections. It applies to every `LoadBalancer` Service on the cluster,
**including the Traefik ingress** that the networking stack installs - so a
meshed cluster has no working ingress out of the box.

Until this is fixed, expose a meshed cluster with a `NodePort` Service (the
node's private address serves normally) plus a load balancer you point at the
nodes' private addresses yourself, or keep ingress on a single-zone
`private_network` cluster.
</Warning>
