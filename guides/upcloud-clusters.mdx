...
- The UpCloud cloud controller manager cannot match a node's overlay address to an address UpCloud reports for the server, so it does not maintain node addresses on a meshed cluster and logs `failed to get node address from cloud provider that matches ip` for each node. Nodes are unaffected - they register, stay `Ready` and keep their `providerID` - but treat `LoadBalancer` Services as primary-zone only, and expect that message in the CCM log.
