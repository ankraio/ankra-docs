...
<Steps>
  ...
  <Step title="Distribution, CNI and Image">
    Keep **k3s** (the default) or pick **kubeadm**, which on AWS is Cilium only. k3s clusters default to Cilium and can pick another CNI and its [advanced features](/guides/digitalocean-clusters#advanced-cni-features); prefer Cilium or Calico, because only they can run the [IMDS guard](#instance-metadata-imds) that keeps pods away from the control-plane instance role. The Ubuntu series defaults to 24.04 and the root volume is an encrypted gp3 volume whose size you can raise.
  </Step>
