...
| Preflight's `cidr` item is an error | `network_ip_range` does not hold a private `/20` and a public `/24` per zone - a `/20` VPC fits one zone only. Widen the range (up to `/16`) or use fewer zones |
