...
Create the credentials first. The AWS credential is the same one EKS uses; the self-managed scope is a third CloudFormation stack, and the [credential page](/platform/credentials/aws#self-managed-clusters) walks through connecting it in the dashboard (or over the API with `GET /api/v1/credentials/aws/onboarding?scope=self_managed` and `POST /api/v1/credentials/aws/role` - see the [credential API](/reference/aws#credential-api)). The CLI lists what is connected:

```bash
# AWS credentials the organisation has connected (keys or role, with the role's scope)
...
# cloud-init status and the tail of its log, fetched over the bastion as a tracked operation
curl -X POST https://platform.ankra.app/api/v1/clusters/aws/<cluster_id>/nodes/<node_id>/cloud-init-log \
  -H "Authorization: Bearer $ANKRA_API_TOKEN"
```

See [Restarting a Node](/guides/hetzner-clusters#restarting-a-node) for the response shape and state requirements - identical across providers. The `ankra cluster aws` group carries no `nodes` verb in this release; use the **Nodes** section, the API, or the AI assistant ("restart worker-2 on my-cluster"). Diagnose and repair (an OS-level check over SSH through the bastion, and a repair of the node's k3s or kubelet unit) run from the node's row menu in the **Nodes** section or through the `ssh_diagnose_node` and `ssh_repair_node` [AI tools](/platform/mcp-tools#provisioning--nodes).

### Bastion health and resize

`GET /api/v1/clusters/aws/{cluster_id}/bastion/health` reports the last verdict of the bastion health loop - reachable or not, which hop a failed probe stopped at, and when - and `POST .../bastion/diagnose` probes it now as a tracked operation; both are shown on the **Nodes → Bastion** tab. Resize the bastion from that tab or with `PUT /api/v1/clusters/aws/{cluster_id}/bastion/instance-type` (`{"instance_type": "t3.medium"}`): the instance is stopped, its type changed and started again, keeping its Elastic IP.
