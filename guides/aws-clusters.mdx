...
<CardGroup cols={2}>
  <Card title="AWS Credential (self-managed scope)" icon="key">
    An AWS credential whose role or user carries the **Self-managed clusters** permissions - a separate CloudFormation stack from EKS provisioning. See [AWS Credentials](/platform/credentials/aws#self-managed-clusters).
  </Card>
...
<Steps>
  ...
  <Step title="Select Credentials">
    Pick an AWS credential with the self-managed scope and an SSH key credential. A credential that only carries the read-only or EKS scope is listed but refused at preflight with the missing permission named. You can create either credential from the wizard.
  </Step>
  ...
  <Step title="Configure Nodes">
    ...
    - **Control Plane** - count (1 or 3 - three when the node subnets span more than one availability zone) and instance type
  ...
  <Step title="Distribution, CNI and Image">
    Keep **k3s** (the default) or pick **kubeadm**, which on AWS is Cilium only. k3s clusters can choose a CNI and its [advanced features](/guides/digitalocean-clusters#advanced-cni-features). The Ubuntu series defaults to 24.04 and the root volume is an encrypted gp3 volume whose size you can raise.
  </Step>
...
Create the credentials first. The AWS credential is the same one EKS uses; the self-managed scope is a third CloudFormation stack, and the [credential page](/platform/credentials/aws#self-managed-clusters) walks through it. `ankra credentials aws onboarding` prints the external ID and the quick-create link for the stack, and `create-role` stores the resulting role ARN.

```bash
# AWS credential: launch the AnkraSelfManagedProvisioning stack, then store the role
ankra credentials aws onboarding
ankra credentials aws create-role --name my-aws --role-arn <role-arn> --region eu-north-1
...
`preflight` takes the same flags as `create` and reports before anything is created: whether the VPC and subnets exist and sit in the region, which availability zones the node subnets cover, the resolved egress mode (and why `bastion_nat` is refused, if it is), the AMI resolved for the Ubuntu series and architecture, the vCPU quota for the chosen instance families, the Elastic IP quota, the security-groups-per-interface quota, and whether `bastion_allowed_ips` is valid. Run it until it is clean, then run `create` with the same flags.
...
ankra cluster aws preflight \
  ...
  --control-plane-instance-type t3.medium \
  --worker-count 3 \
...
ankra cluster aws nodes cloud-init-log <cluster_id> <node_id>
```
...
See [Restarting a Node](/guides/hetzner-clusters#restarting-a-node) for the response shape and state requirements - identical across providers. Diagnose and repair (an OS-level check over SSH through the bastion, and a repair of the node's k3s or kubelet unit) run from the node's row menu in the **Nodes** section or through the `ssh_diagnose_node` and `ssh_repair_node` [AI tools](/platform/mcp-tools#provisioning--nodes).

### Bastion health and resize

`ankra cluster aws bastion status <cluster_id>` reports the last verdict of the bastion health loop - reachable or not, which hop a failed probe stopped at, and when. Resize the bastion from the **Nodes → Bastion** tab or with `ankra cluster aws bastion resize <cluster_id> t3.medium` (`PUT /api/v1/clusters/aws/{cluster_id}/bastion/instance-type`): the instance is stopped, its type changed and started again, keeping its Elastic IP.
...
- **`retain`** (the default) keeps them. EBS volumes and Network Load Balancers stay in your account and keep billing; they are tagged with the cluster ID so they are easy to find in the EC2 and load balancing consoles.
- **`delete`** sweeps them: every volume and load balancer carrying this cluster's `ankra.cloud/managed=true` and cluster-ID tags is deleted along with the rest of the infrastructure. Nothing without those tags is touched.
...
- **Instance metadata.** Every instance requires IMDSv2 with a hop limit of 1. The cloud controller and CSI driver run on the host network and can reach the instance role; a pod on the bridge network cannot, so a workload does not inherit the node's AWS permissions by accident. A workload that needs AWS access brings its own credentials.
...
| Issue | Solution |
|-------|----------|
| Preflight refuses the credential | The credential carries the read-only or EKS scope only. Add a credential with the [self-managed scope](/platform/credentials/aws#self-managed-clusters) |
...
Instances are launched with IMDSv2 required and a hop limit of 1. Tools that use IMDSv1 (`curl http://169.254.169.254/latest/...` without a token) fail on the nodes, and any pod on the pod network fails on either version - that is the intended isolation of the instance role. If a workload needs AWS credentials, give it its own (a secret, or a role it assumes with keys you manage); do not raise the hop limit or switch IMDSv1 back on, and note that `ModifyInstanceMetadataOptions` is one of the calls Ankra's credential is allowed to make on its own instances, so Ankra restores the hardening on reconciliation.

### AWS service quotas

Preflight checks the quotas that most often block a create, but a quota can still be consumed between preflight and create. If provisioning fails with a quota error, check in the [Service Quotas console](https://console.aws.amazon.com/servicequotas/) for the region:
