...
Create the credentials first. The AWS credential is the same one EKS uses; the self-managed scope is a third CloudFormation stack, and the [credential page](/platform/credentials/aws#self-managed-clusters) walks through it in the dashboard. From the terminal, `onboarding` prints the external ID, the trust principal, the quick-create link for the `AnkraSelfManagedProvisioning` stack and the `create-role` command to run once the stack shows `CREATE_COMPLETE`:

```bash
# 1. Print the external ID and the CloudFormation launch link for the self-managed role
ankra credentials aws onboarding --scope self_managed
...
ankra credentials aws create-role --name my-aws --role-arn <role-arn> --external-id <external-id> \
  --region eu-north-1 --scope self_managed
