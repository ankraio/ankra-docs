...
<CardGroup cols={2}>
  <Card title="Hetzner API Credential" icon="key">
    A Hetzner Cloud API token with read/write permissions. See [Hetzner Credentials](/integrations/credentials/hetzner).
  </Card>
  <Card title="SSH Key Credentials" icon="lock">
    One or more SSH public keys for server access. You can provide your own or let Ankra generate one. Multiple keys can be attached to a single cluster. See [SSH Key Credentials](/integrations/credentials/ssh-key).
  </Card>
</CardGroup>
