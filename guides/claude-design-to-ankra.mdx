...
<Warning>
**Rolling out per organisation.** The import ships behind the `claude_design_import` feature flag, off by default. While it is off the portal does not offer the Claude Design source, `ankra application import claude-design` explains that the flag is off, and the API answers 404. [Contact support](/platform/support) to have it enabled for your organisation.
</Warning>
