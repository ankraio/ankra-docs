...
<Tip>
Use [SOPS encryption](/guides/sops) to safely store registry credentials in your GitOps repository.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="`denied: permission_denied: write_package` on the first build">
    This happens at **runtime**, inside GitHub Actions, using GitHub's own `GITHUB_TOKEN` - it is **not** an Ankra GitHub App permission. The build succeeds but the image push to `ghcr.io/<owner>/<repo>` is rejected.

    Ankra surfaces this for you: open the application's **CI/CD** tab. Ankra runs a **GHCR publishing readiness** check during onboarding and after each run, and turns a failed push into a fix card with the exact remediation. The most common fixes, in order:

    1. **Grant the repository write on the package.** If a package with that name already exists (created manually or by another repo), open its settings and add this repository with the **Write** role under **Manage Actions access**.
    2. **Set the workflow token to read/write.** Ankra can do this in one click when its GitHub App has the optional **Administration** permission; otherwise change **Settings → Actions → General → Workflow permissions** to **Read and write permissions**.
    3. **Check the organisation Actions policy.** Some organisations cap the `GITHUB_TOKEN` or restrict Actions from creating packages.

    Once published, use **Make public** in the **Generated Files** tab if you want the image to be anonymously pullable.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="GitOps Reference" icon="git-alt" href="/concepts/gitops">
  ...
  <Card title="Operations" icon="list-check" href="/platform/operations">
