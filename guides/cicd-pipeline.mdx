...
<AccordionGroup>
  ...
  <Accordion title="`denied: permission_denied: write_package` on the first build">
    ...
    2. **Set the workflow token to read/write.** Change **Settings → Actions → General → Workflow permissions** to **Read and write permissions**.
    ...
    If the repository is connected as an [Application (closed beta)](/concepts/applications), Ankra surfaces this for you: the application's **CI/CD** tab runs a **GHCR publishing readiness** check during onboarding and after each run, turns a failed push into a fix card with the exact remediation, and can set the workflow token permission in one click when its GitHub App has the optional **Administration** permission.
  </Accordion>
