...
Ankra does not write the login secrets for a registry you operate - the robots there are yours to administer, and overwriting a push robot with the credential Ankra holds would deny every later build. Publish readiness reports whether the named secrets exist. Opt in with `manage_actions_secrets: true` if you want Ankra to write them.
