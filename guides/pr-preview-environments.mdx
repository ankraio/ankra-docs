...
<Steps>
  ...
  <Step title="Configure a staging cluster">
    An admin sets the organisation's staging cluster under **Organisation Settings → AI → Environment**. Without it, pull requests deploy nothing - this is the switch that turns the feature on.
  </Step>
  <Step title="Optional: bring a demo base domain for HTTPS">
    By default previews are served under the staging cluster's `*.ankra.cc` DNS zone over plain HTTP. For HTTPS (or your own hostnames), configure a **demo base domain** with an ingress class and TLS secret in the same settings screen. The [preview URL rules](/concepts/applications#the-preview-url) explain the resolution order.
  </Step>
</Steps>
...
| HTTP instead of HTTPS | Platform-zone (`*.ankra.cc`) previews are HTTP by design. Configure a demo base domain with a TLS secret for HTTPS. |
