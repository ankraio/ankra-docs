...
- **You have a [Cluster Access grant](/guides/cluster-access) on the cluster.** Without one, `ankra cluster kubeconfig add` and `ankra cluster kube-token` fail with *"You do not have access to this cluster"*. This applies to **organisation admins too** - managing access does not itself include kubectl access, so admins must grant themselves a role first.
...
- [Sandbox clusters](/guides/cluster-sandbox) do not support Cluster Access and cannot be reached this way.
...
- Each request carries a short-lived token. Ankra authenticates it, checks your [access grant](/guides/cluster-access), and forwards the call over NATS to the agent, which executes it against the API server.
...
- **Revocable.** An admin can revoke a member's access at any time from [Cluster Access](/guides/cluster-access) (delete the grant, or revoke all). Existing tokens stop working.
...
These are the **token-authenticated user-API** paths the CLI uses. The portal manages the same access through session-authenticated endpoints under `/org/clusters/{cluster_id}/access/...` - see the [Cluster Access API table](/guides/cluster-access#api).

The proxy requires the access gateway to be enabled; when it isn't, these endpoints return `404`. See [Cluster Access](/guides/cluster-access) for grants and the [CLI reference](/integrations/ankra-cli) for the full command set.
