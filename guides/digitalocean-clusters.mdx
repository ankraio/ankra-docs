 ...
 **k3s** Lightweight Kubernetes with a user-selectable CNI.
  ...
  <Step title="GitOps, Networking & DNS">
    ...
    - **Include Networking Stack** - deploys Traefik, cert-manager, and a Let's Encrypt ClusterIssuer configured for DigitalOcean load balancers.
...
DigitalOcean clusters can be provisioned with either **kubeadm** (the default everywhere - the wizard, the API and the CLI) or **k3s** (`--distribution k3s`).
...
<CliVersion since="0.11.0" command="cluster digitalocean stop --force" />
