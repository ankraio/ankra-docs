...
| **Cloud Firewalls** | Two tag-scoped firewalls - the only ingress boundary, since every droplet has a public IP |
...
flowchart TB
    ...
    subgraph VPC [DigitalOcean VPC + tag-scoped cloud firewalls]
        Bastion[Bastion droplet<br/>SSH open to the internet]
        CP[Control plane droplets<br/>inbound closed by firewall]
        Workers[Worker droplets<br/>inbound closed by firewall]
    ...
    CP -->|egress via own public IP| Internet
    Workers -->|egress via own public IP| Internet
    Internet --> LB
    LB --> Workers
```

DigitalOcean droplets always receive public IPs, so isolation works differently here than on providers with truly private nodes: two tag-scoped **cloud firewalls** are the ingress boundary. The node firewall only accepts traffic from other cluster droplets and the VPC range; the bastion firewall additionally opens SSH (port 22) to the internet. Outbound traffic leaves through each droplet's own public IP - there is no NAT hop.

The bastion is the SSH jump host Ankra uses for provisioning and management; inter-node and jump traffic stay on the VPC.
