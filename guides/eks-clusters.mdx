...
An **AWS credential** stored in Ankra (access keys or an IAM role) with permissions for **EKS**, **EC2**, and **IAM role management**. The IAM-role wizard's **Provisioning (EKS clusters)** access level creates a correctly scoped role for you. See [AWS Credentials](/platform/credentials/aws).
