---
title: Clone Stacks Between Clusters & Organisations
description: Learn how to clone one or multiple stacks from one cluster to another - including across organisations - for consistent deployments across environments
---

import { CliVersion } from "/snippets/cli-version.jsx";

<Note>
Cloning stacks allows you to replicate your infrastructure configurations across multiple clusters - even across different organisations. You can clone a single stack or select multiple stacks at once. With **Deploy after cloning** left on, a cloned stack deploys on the target straight away; turn it off to keep the clone as a draft for review first. A stack whose encrypted values had to be removed is always held as a draft, marked **Needs input**, until you re-enter them.
</Note>
...
<Frame caption="Cloning a Stack Between Clusters">
  <video
    ...
    src="https://fsn1.your-objectstorage.com/ankra-vod/docs/clone-stack.mp4"
  ></video>
</Frame>
...
| **Keep a standby ready** | Run the same configuration on a second cluster, ready to take over |
...
<Warning>
**Cloning copies configuration, not data, and it reads the live source.** A clone reproduces the stack's definition on the target; persistent volumes and database contents stay where they are. It also needs the source cluster to be readable, so it is not a disaster-recovery mechanism - for that, protect the stack and restore a [restore point](/guides/backups).
</Warning>

<Warning>
**Security Note:** Encrypted values (secrets) are not transferred during cloning for security reasons. You'll need to reconfigure them in the target cluster. This includes passwords, API keys, and any values encrypted with SOPS. A stripped Secret is still a valid Kubernetes object - each cleared key holds the `ENCRYPTED_VALUE_REMOVED` placeholder (base64-encoded under `data`) so the draft deploys and the workload fails clearly on the placeholder rather than the apply failing on the manifest.
</Warning>
...
<Steps>
  ...
  <Step title="Clone the Stack">
    Click **Clone Stack**. A deployed clone takes you to the running stack on the target; a clone that needs input opens its draft in the builder, with a banner naming the values to re-enter. If you cloned to a different organisation, Ankra automatically switches your active organisation.
  </Step>
...
<Steps>
  ...
  <Step title="Navigate to Target">
    The summary says how many stacks are deploying and how many were held for your input, with a link into each held draft's builder tab. Click **Go to Target Cluster** to watch them come up.
  </Step>
</Steps>

<Tip>
Multi-stack cloning is useful for replicating an entire cluster's configuration. Select all stacks and clone to a new cluster; the ones without secrets deploy on their own, and the **Needs input** chip on the stack list shows which are waiting for you.
</Tip>
...
Warnings:
  ...
  - Cluster variable 'db_host' was copied from the source cluster onto the cloned stack; review its value for the target cluster.
  - Manifest 'prometheus' pins scheduling to nodes labelled purpose=monitoring - make sure the target cluster has matching nodes.
  - Deploy check for addon 'grafana': registry 'bitnami' is not connected in this organisation.
...
| Stack variables | Yes | Transferred as-is; cluster variables the content references are copied onto the stack (same organisation) |
| Manifests | Yes | YAML content (encrypted values cleared) |
| Addons | Yes | Chart references, versions, settings, job timeouts and AGENTS.md notes |
| Applications | Yes | Same organisation only - applications are organisation-scoped, so a cross-organisation clone drops them with a warning |
| Addon configurations | Optional | Use `--include-config` flag |
| Encrypted secrets | No | Must be reconfigured |
| Deployment state | By default | **Deploy after cloning** deploys the clone; a stack whose secrets were removed is held as a draft |
| Resource positions | Yes | Canvas layout and deploy wave carry over |
| Dependencies | Yes | Parent relationships preserved by name and kind |

### Pre-checks in the warnings

The clone runs the checks a deploy would, and reports what it finds as warnings rather than refusing - a clone never fails on them:

- **Variables**: a `${{ ankra.* }}` token the target cannot resolve is named, and a same-organisation clone copies the source cluster's value onto the stack so the manifest does not deploy the literal token.
- **Node pinning**: manifests and applications whose `nodeSelector` targets labels the target may not have.
- **SOPS readiness**: encrypted paths that survived the strip, on a target with no deployed SOPS key.
- **Chart references**: registries not connected in the target organisation, and charts or versions the catalog does not know.
...
<Tip>
Organisation and cluster variables are scoped to their organisation, and no variable value crosses that boundary. A cross-organisation clone names every `${{ ankra.* }}` reference it could not resolve in its warnings - define those variables in the target organisation or on the target cluster before deploying.
</Tip>
...
1. **Manifests:** Encrypted fields are replaced with the `ENCRYPTED_VALUE_REMOVED` placeholder
2. **Addon configurations:** Encrypted values are replaced with the same placeholder
3. **Held as a draft:** the stack is not deployed; the clone result, the **Needs input** chip on the stack card and a banner in the builder name each member and field to re-enter, and the banner links straight to it
...
<AccordionGroup>
  <Accordion title="Re-encrypting Secrets After Clone">
    ...
    4. List the keys under **Encrypted Keys (SOPS)** so they are sealed with the target organisation's key
    5. Save and deploy
    
    <Tip>
    The SOPS key pair belongs to the organisation, not to a cluster. Clusters in the same organisation share it, so a value sealed for one can be decrypted by another. A clone into a different organisation cannot decrypt the source's values - this is by design for security.
    </Tip>
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Replicate All Stacks to a New Cluster">
    ...
    ankra cluster stacks list   # note the stack names

    # Clone each stack to the new cluster
    for stack in monitoring ingress app-data; do
