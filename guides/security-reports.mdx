...
The report reflects the same data as the cluster's Security view: actionable findings by severity, and how the posture has moved since the previous report. Findings on the CISA Known Exploited Vulnerabilities catalog get their own banner and a **KEV** marker in the top-CVE table, with the FIRST EPSS exploitation probability where the platform holds one; package upgrades that clear a known-exploited CVE are listed first under **Fix these first**. Accepted risk is excluded from actionable counts - the report is built for the people who need to know what still requires attention, not for restating everything Trivy has ever observed.
...
**From the CLI and MCP.** `ankra security report-schedule --cluster <cluster>` shows the stored schedule (or "not configured"), `ankra security report-schedule set --cluster <cluster> --frequency weekly|monthly --recipient ops@example.com --recipient sec@example.com [--disabled]` creates or replaces it after asking (`--yes` skips the prompt; `--disabled` stores it paused), and `ankra security report-schedule send --cluster <cluster>` queues one report now. From chat or the MCP server the same three are `get_cluster_security_report_schedule`, `set_cluster_security_report_schedule` and `send_cluster_security_report_now`; the two writes are confirmed before they run and need `clusters.write`. With an API token the routes are `GET`/`PUT /api/v1/org/clusters/imported/{cluster_id}/security/report-schedule` and `POST .../report-schedule/send-now`.
