...
### Example: Envoy Gateway and the Gateway API

Envoy Gateway ships its chart as an OCI artifact on Docker Hub, so it installs through an organisation registry rather than the built-in catalogue:

1. Add an OCI registry with the URL `oci://registry-1.docker.io/envoyproxy`. It is public, so no credential is needed.
2. Add the `gateway-helm` chart to a stack as an add-on. Ankra recognises it as Envoy Gateway. Pin the chart version, as for any add-on.
3. Deploy the stack. The chart installs the Gateway API and Envoy Gateway CRDs with the release. Turn on **Server-Side Apply** in the [add-on settings](/concepts/addon-settings), which is the recommended option for charts that carry large CRDs.
4. Define your `GatewayClass`, `Gateway` and `HTTPRoute` resources as [manifests](/concepts/manifests) in the same stack, with the Envoy Gateway add-on as their parent so they are applied after the CRDs exist.

The same steps apply to any Gateway API implementation that publishes a Helm chart, such as NGINX Gateway Fabric.
