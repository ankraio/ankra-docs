...
<Note>
Each organisation can have one playground at a time, provisioned on Ankra's shared playground infrastructure and isolated per organisation. A free Trial expires once everyone in the organisation has been away for a while — **3 hours on the free plan, a week on a paid one** — with a heads-up notification and email first, and signing back in resets the clock. A paid size does not idle-expire: it stays until you destroy it.
</Note>
...
- One playground per organisation; recreate a Trial after it expires.
