...
Ordering a paid size requires a billing plan and a payment card on file - both are set up on the billing page, and the picker says so before the order if either is missing. The free Trial needs neither. The price you order at is the price of record: later catalog changes apply to new orders only, never to an environment you are already running.
