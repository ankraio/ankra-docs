...
- **Empty rows are dropped for you.** After substitution Ankra removes Adaptive Card `FactSet` facts whose value came out empty (and a `FactSet` left with no facts), empty `TextBlock` entries, and Slack `section` fields whose `*Label*` has no value under it. The built-in presets therefore render correctly for every kind with no editing. Containers, columns and action sets are never removed.
