...
The JSON-valued variables (`{{data}}`, `{{route_hint}}`, `{{trigger_details}}`, `{{resource_snapshot}}`) follow where you put them: written unquoted they splice in as a nested object, and written inside a quoted string they are escaped for you so the payload stays valid JSON.

```json
{
  "details": {{trigger_details}},
  "text": "Details: {{trigger_details}}"
}
```
...
Before you rely on a set of rules, run `ankra alerts routes preview` to see which destinations a notification would actually reach. [Notification routing](/guides/notification-routing) covers the full matching, ordering, and de-duplication behaviour.
