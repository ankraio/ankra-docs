 ...
 **kubeadm** Vanilla upstream Kubernetes bootstrapped with `kubeadm` and containerd (preselected). kubeadm clusters always use Cilium CNI and optionally support an external etcd topology with dedicated etcd instances.
 **k3s** Lightweight Kubernetes with a user-selectable CNI.

    See [Kubernetes Distribution](#kubernetes-distribution) for details.
  </Step>
  <Step title="Name, Networking & DNS">
    Name the cluster and set its environment. Two checkboxes on this step are on by default:
    - **Include Networking Stack** - deploys Traefik and cert-manager as Ankra-managed stacks. On k3s they run through the built-in service load balancer; unchecking it leaves k3s with its bundled Traefik and no certificate manager (on kubeadm, with no ingress controller or certificate manager at all).
...
Morpheus clusters can be provisioned with either **kubeadm** (the default everywhere - the wizard, the API and the CLI) or **k3s** (`--distribution k3s`).
...
- Upgrade the service plan (upgrade only - see [Service Plan Changes](#service-plan-changes))
...
```bash
curl https://platform.ankra.app/api/v1/clusters/morpheus/<cluster_id>/node-groups \
  -H "Authorization: Bearer $ANKRA_API_TOKEN"
```
...
```json
{
  "node_groups": [
    {
      "name": "default",
      "instance_type": "<service-plan-name>",
      "count": 2,
      "min": 0,
      "max": 100,
      "labels": {},
...
```bash
curl -X POST https://platform.ankra.app/api/v1/clusters/morpheus/<cluster_id>/node-groups \
  -H "Authorization: Bearer $ANKRA_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "workers-large",
...
```bash
curl -X PUT https://platform.ankra.app/api/v1/clusters/morpheus/<cluster_id>/node-groups/default/scale \
  -H "Authorization: Bearer $ANKRA_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"count": 4}'
```
...
```bash
curl -X PUT https://platform.ankra.app/api/v1/clusters/morpheus/<cluster_id>/node-groups/default/instance-type \
  -H "Authorization: Bearer $ANKRA_API_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
# Update labels
curl -X PUT https://platform.ankra.app/api/v1/clusters/morpheus/<cluster_id>/node-groups/default/labels \
  -H "Authorization: Bearer $ANKRA_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"labels": {"env": "production", "tier": "backend"}}'

# Update taints
curl -X PUT https://platform.ankra.app/api/v1/clusters/morpheus/<cluster_id>/node-groups/default/taints \
  -H "Authorization: Bearer $ANKRA_API_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE https://platform.ankra.app/api/v1/clusters/morpheus/<cluster_id>/node-groups/workers-large \
  -H "Authorization: Bearer $ANKRA_API_TOKEN"
```
...
You can stop a Morpheus cluster to release its compute (the cluster instances provisioned through your appliance) while keeping its configuration, stacks, and credentials in Ankra. Starting the cluster re-provisions the instances and reconciles the cluster back to a running state. This is useful for freeing capacity when a cluster is not needed.
...
```bash
curl -X POST https://platform.ankra.app/api/v1/clusters/morpheus/<cluster_id>/stop \
  -H "Authorization: Bearer $ANKRA_API_TOKEN"

curl -X POST "https://platform.ankra.app/api/v1/clusters/morpheus/<cluster_id>/start?scope=all" \
  -H "Authorization: Bearer $ANKRA_API_TOKEN"
```
