...
7. **What executes today.** `checkout` and `run` stages execute end to end; `build` refuses at dispatch until the dispatcher resolves its push target, and `scan`, `verify` and the platform-settled kinds do not run yet - see [What is in place today](/guides/ankra-pipelines#what-is-in-place-today). A converted pipeline's `checkout` stage runs for real; its first `build` stage, if the workflow published an image, will not until that wiring lands.
