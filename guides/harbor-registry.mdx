...
The robots above are Ankra's: one push robot per application, and the organisation's `ankra-harbor-ci` and `ankra-harbor-pull` logins. For anything that logs in to your project without Ankra in the loop - a CI system you run yourself, a laptop pushing an image by hand, a cluster Ankra does not manage pulling from the project - create a robot of your own.

A robot is a named login with **push** (push and pull) or **pull** rights on your organisation's project and nothing else. Its secret is shown exactly once, when it is created or rotated. The login is also stored as the managed registry credential `ankra-harbor-robot-<name>`, so clusters and applications can reference it like any other registry credential; it cannot be edited or deleted through the generic credential lanes, only revoked here.

```bash
# Mint one. The secret is printed once, with the docker login that uses it.
ankra registry robots create jenkins --description "Jenkins on the office server"

# Pull-only, for a cluster outside Ankra
ankra registry robots create edge-cluster --scope pull

# Script it: the secret rides -o json as `secret`
ankra registry robots create jenkins -o json | jq -r .secret

# What exists (secrets are never listed)
ankra registry robots list
ankra registry robots get jenkins

# Lost or leaked: mint a new secret, the old one stops working
ankra registry robots rotate jenkins

# Done with it: deletes the robot from the registry and drops its credential
ankra registry robots delete jenkins --yes
```

| Field | Value |
|---|---|
| **Login** | `robot$org-<organisation-id>+user-<name>` |
| **Rights** | `push` = push and pull on your project; `pull` = pull only |
| **Expiry** | Never; rotate or revoke it yourself |
| **Credential** | `ankra-harbor-robot-<name>`, managed, revoked with the robot |

On the API the same lives at `/api/v1/org/registry-robots` (`POST` create, `GET` list, `GET`/`DELETE` `/{name}`, `POST /{name}/rotate`); creating and rotating need `credentials.write`, revoking `credentials.delete`. Names are 2 to 32 lower-case letters, digits and hyphens.
