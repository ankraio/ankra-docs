...
- **A cloud-init VM template** (e.g., Ubuntu 24.04) with the QEMU guest agent installed, registered on the node you deploy to. Ankra clones this template for every cluster VM and reads each VM's IP address through the guest agent. You can pin a specific template with the `template` option; otherwise the first template on the node is used. See [Creating a cloud-init template](#creating-a-cloud-init-template).
- **A network bridge.** You can provide an existing active Linux bridge. If none exists, Ankra converts the single active management Ethernet or bond interface (static, DHCP, or `manual`-method) to `vmbr0` and also creates a private NAT bridge `vmbr1` (10.20.0.0/16) — see [Networking](#networking).
...
When you save a Proxmox VE credential, Ankra initializes two bridges when they do not already exist:

| Bridge | Purpose | Address |
|--------|---------|---------|
| `vmbr0` | Management — carries the host's public or provider-assigned IP | Inherited from the physical interface |
| `vmbr1` | Private compute — isolated from the management interface | 10.20.0.1/16 (NAT to `vmbr0`) |

**`vmbr0` (management bridge).** If no active bridge exists, Ankra converts the single active Ethernet or bond management interface to `vmbr0`. The interface's address, gateway, and addressing method (static, DHCP, or `manual`) move to the bridge; the physical interface becomes its uplink. Inactive bridges must be fixed or removed before connecting. Ankra refuses the conversion when the management interface is ambiguous, has custom options, or another network change is pending.

**`vmbr1` (private bridge).** Ankra also creates `vmbr1` as an internal bridge with no physical uplink. It has a fixed RFC-1918 address (10.20.0.1/16) and two `post-up` rules: one that enables IPv4 forwarding on the host, and one that masquerades outbound traffic from the 10.20.0.0/16 subnet through `vmbr0` so VMs can reach the internet. A dnsmasq DHCP server runs on `vmbr1` (range 10.20.1.0–10.20.254.254) so VMs get addresses automatically.

**Recommended setup.** Place cluster VMs on `vmbr1`. This keeps Kubernetes traffic off the management bridge and provides internet access through NAT without exposing VM addresses on the public network. Select `vmbr1` as the bridge in the cluster creation wizard.

---

## Creating a cloud-init template

Ankra clones a cloud-init VM template for every cluster VM. You must create this template on each Proxmox node before provisioning a cluster.

The steps below use Ubuntu 24.04. Adjust the image URL and VM ID for your environment.
...
# 6. Enable the QEMU guest agent (required for Ankra to read VM IPs)
...
flowchart TB
    ...
    subgraph Host [Proxmox host]
        ...
        vmbr1[vmbr1 — private bridge<br/>10.20.0.0/16 · NAT → vmbr0]
        subgraph Private [Cluster VMs on vmbr1]
            Bastion[Bastion VM]
...
All VMs are QEMU virtual machines on the Proxmox node you selected, attached to the selected bridge. Ankra initializes `vmbr0` (management) and `vmbr1` (private, 10.20.0.0/16 with NAT) during credential setup when they do not already exist. Place cluster VMs on `vmbr1` — they receive DHCP addresses from a dnsmasq server on the host and reach the internet via NAT through `vmbr0`.
...
| No VM templates found on the node | Create a cloud-init template on the selected Proxmox node first — see [Creating a cloud-init template](#creating-a-cloud-init-template) |
