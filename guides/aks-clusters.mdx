...
AKS is the only managed provider with native cluster stop/start, and Ankra exposes it: stopping deallocates the control plane and agent pools (Azure stops billing for compute) while keeping all configuration, and starting brings the same cluster back.
