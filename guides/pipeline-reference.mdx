...
On GitHub, a run plans its *logic* from this file as committed at the run's own head sha; on GitLab and Bitbucket Cloud it plans from the definition stored through `ankra pipeline definition put`. A run's *authority* - its [protected sections](/guides/ankra-pipelines#logic-is-open-authority-is-closed) - always comes from the default branch's approved definition, whichever source the logic came from.
...
| `cache` | list | `defaults.cache` | `key` (expressions allowed, typically `hashFiles`) and `paths` under the workspace; one 5Gi volume per path, keyed by (repository, cluster, key, path) so the same key resolves to the same volume across runs. `restore_keys` is an ordered list of prefixes a miss on `key` falls back to, restoring the most recently used matching volume instead of starting cold - GitHub Actions' own cache semantics. A volume nothing resolves to for `ci_cache_retention_days` is reaped. |
