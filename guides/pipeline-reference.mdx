...
| `cache` | list | `defaults.cache` | `key` (expressions allowed, typically `hashFiles`) and `paths` under the workspace; one 5Gi volume per path, keyed by (repository, cluster, key, path) so the same key resolves to the same volume across runs. `restore_keys` is an ordered list of prefixes a miss on `key` falls back to, restoring the most recently used matching volume instead of starting cold - GitHub Actions' own cache semantics. A volume nothing resolves to for `ci_cache_retention_days` is reaped. `size` is held to the organisation policy's `max_cache_size`. The whole block is protected - a cache decides whose earlier output lands in this run's workspace. |
...
| `timeout` | duration | `defaults.timeout`, else 30 minutes | Wall-clock budget; at most 6 hours, longer values are clamped and recorded. Protected, so a head cannot remove its own or the trusted definition's |
| `resources.cpu`, `resources.memory`, `resources.gpu` | quantities | 500m, 1Gi, none | Requests equal limits; at most 8 cores, 32 GiB and 4 GPUs. All three are protected, and `gpu` also needs `runs_on.node_selector`. |
