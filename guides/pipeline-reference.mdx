...
A stage of a platform-settled kind is left exactly as the dispatcher found it; a run containing one does not conclude until that kind's evaluator ships, and `ankra pipeline cancel` is how to release its concurrency group in the meantime. A validator change to refuse a platform-settled kind up front, with a named diagnostic, rather than let a run hang on one until it is cancelled, is planned but not shipped.
...
| `scan` | `scanners` (`semgrep`, `checkov`, `trivy`) and `fail_on`, a map from scanner to a severity. `fail_on` is accepted and carried today, but nothing reads it back: it does not decide what blocks a `publish`. What actually blocks is the `gate` stage's own policy (below) - the organisation's [`ci_image_gate`](/guides/ankra-pipelines#organisation-ci-settings), tightened, never loosened, by a `gate` stage's `with:`. `with.semgrep_config` names the space-separated Semgrep registry packs to run (for example `"p/default p/docker p/golang"`); undeclared, Semgrep runs its own `--config=auto`. |
