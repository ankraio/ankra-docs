---
title: Build a Cloudflare Stack
...
<Steps>
  ...
  <Step title="Add a Secret Manifest">
    ...
    <Tip>
    **Encrypt your token with SOPS**: If you have [SOPS encryption](/guides/sops) enabled, add `token` to the **Encrypted Keys** section in the manifest editor. Your token will be encrypted before being pushed to Git and automatically decrypted during deployment.
    </Tip>
  </Step>
...
<Steps>
  ...
  <Step title="Enable SOPS on Your Cluster">
    Navigate to your cluster → **Settings** → **Encryption** and toggle on **Enable SOPS Decryption**. On the ArgoCD engine this deploys the decryption key to ArgoCD; on the native engine the agent decrypts on its own.
  </Step>
...
1. When Ankra writes the Stack to your Git repository, it seals the `token` field with your organisation's AGE public key
2. The sealed manifest is what your Git repository stores (if GitOps is enabled)
3. When the Stack deploys, the value is decrypted inside your cluster - by the agent on the native engine, or by ArgoCD's helm-secrets plugin on the ArgoCD engine
...
<Note>
Learn more about SOPS encryption in the [SOPS Encryption guide](/guides/sops).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Stack Builder" icon="cubes" href="/concepts/stacks">
