...
every key at every depth of the document. So an entry `auth.password` names the
key `password`, and **every** key called `password` in that file is sealed -
...
or seal the whole credential as its own Secret manifest.
</Warning>
