...
<Steps>
  ...
  <Step title="Notifications">
    ...
    In addition to the alert's own destinations, org-wide **routing rules** (Alerting → Destinations → Routing rules) can match alert firings by severity and cluster - for example, "every critical firing also goes to PagerDuty". A destination that the alert already notifies directly is skipped by routing rules, so nothing is delivered twice.
