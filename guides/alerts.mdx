...
<Steps>
  ...
  <Step title="Notifications">
    Select which destinations (webhooks) should receive notifications when this alert fires. You can configure destinations in the [Webhooks](/guides/webhooks) section.

    In addition to the alert's own destinations, org-wide **routing rules** (Alerting → Destinations → Routing rules) can match alert firings by kind, severity, cluster, and source - for example, "every critical firing also goes to PagerDuty".

    A destination that the alert already notifies directly is skipped by routing rules. That is a guarantee, not a coincidence of timing: **a notification is delivered at most once per destination**, so you can safely keep a destination attached to an alert *and* pointed at by a routing rule. See [Notification routing](/guides/notification-routing) for how rules are ordered, what Exclude and Stop on match actually do, and how to preview which destinations a notification would reach.

    If you have set a [home channel](/guides/webhooks#home-channel), a critical or warning firing that no destination or routing rule handles falls back to the home so it is never missed.
  </Step>
</Steps>
...
| **GitOps Sync** | Monitor the cluster's GitOps sync verdict | `failing`, `degraded`, `healthy` |
| **Metric Threshold** | Compare a platform metric against a numeric threshold | `cpu_utilisation_pct`, `memory_utilisation_pct`, `disk_utilisation_pct`, `pod_restarts_15m`, `oom_kills_15m`, `nodes_not_ready`, `pods_pending` |
| **Metrics Stopped Flowing** | Fire when the cluster has shipped no platform metrics for longer than the threshold (minutes) | `platform_metrics` |
| **Security Findings** | Compare the Trivy vulnerability posture against a numeric threshold | `critical`, `high`, `severe`, `fixable_critical`, `fixable_high`, `new_severe_cves`, `new_kev_cves`, `kev`, `risk_score` |
...
- [AI Incidents](/platform/ai-incidents) - AI-powered root cause analysis for triggered alerts
- [Webhooks](/guides/webhooks) - Configure notification endpoints for alerts
