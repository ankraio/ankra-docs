...
Some OVH regions are *3-AZ*: one region spanning three availability zones with independent power, cooling, and networking. `EU-WEST-PAR` is the first, holding `eu-west-par-a`, `eu-west-par-b`, and `eu-west-par-c`.
...
The zone comes from what OVH reports for the live instance rather than what was requested, so it is correct even on clusters created before zone placement existed. Those clusters pick the labels up on their next sync.
...
- **A `WaitForFirstConsumer` StorageClass** so the volume is provisioned in the zone the pod was actually scheduled to, rather than binding first and stranding the pod. Ankra ships `csi-cinder-sc-topology` as the default class on OVH clusters for exactly this reason.
