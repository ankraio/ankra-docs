...
- The **output directory** (`--out`, default `ankra-migration`): the conversion is written to `<out>/stack` and the dumps to `<out>/data`. A directory that is not empty is refused unless you pass `--force`.
...
- **Warnings**, such as a database over 5 GiB (which cannot be uploaded in one piece yet), or the `postgres` maintenance database being skipped. If `postgres` really is your application database, say so with `--option databases.<workload>=postgres`.
...
```bash
ankra migrate up ./app --cluster production --stop-source --yes --force
```

<Note>
A second run needs `--force`. The first run leaves `stack/` and `data/` in the output directory, and the command refuses to write into a directory that is not empty without it. `--plan` skips the check entirely, so a plan never needs the flag.
</Note>
...
- **Anything over 5 GiB in one piece.** A single artifact above that is refused; multipart upload is in progress.
...
| Exit `2`: `... is not empty; pass --force to overwrite` | An earlier run left `stack/` and `data/` behind | Pass `--force`, or point `--out` somewhere new |
...
| An artifact above 5 GiB is refused | Single upload limit; multipart is not supported yet | Dump that database separately, or split it |
