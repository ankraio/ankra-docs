...
A **GCP service account key** stored in Ankra, with the **Kubernetes Engine Admin** (`roles/container.admin`) and **Service Account User** (`roles/iam.serviceAccountUser`) roles, and the **Kubernetes Engine API** enabled on the project. See [Google Cloud (GCP) Credentials](/integrations/credentials#google-cloud-gcp-credentials).
...
- [Google Cloud (GCP) Credentials](/integrations/credentials#google-cloud-gcp-credentials)
