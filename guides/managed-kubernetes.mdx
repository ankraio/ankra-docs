...
<CardGroup cols={3}>
  ...
  <Card title="Scaleway (Kapsule)" icon="cloud" href="/reference/managed-kubernetes" />
</CardGroup>
...
| `kapsule` | [Scaleway Kapsule](/reference/managed-kubernetes) | Scaleway API key |
...
|---|---|---|
| Providers | DOKS, UKS, GKE, OVHcloud MKS, AKS, EKS, Scaleway Kapsule | Hetzner, OVH, UpCloud, DigitalOcean droplets, Scaleway, [Proxmox](/guides/proxmox-clusters), [Morpheus](/guides/morpheus-clusters) |
...
<Note>
**Surface coverage.** The **portal, API, and AI assistant** cover all seven providers. The **`ankra` CLI** creates, deletes, scales node pools, and upgrades DOKS, UKS, GKE, OVHcloud MKS, AKS, and EKS (`--provider doks|uks|gke|ovh_mks|aks|eks`), but does **not** expose Kapsule, provider-specific control-plane options (HA, network plugin, SKU tier, release channel, and so on), node-pool autoscaling bounds, or cluster **discovery and import**. For those, use the portal or the API.
</Note>
