...
- **DOKS** - [DigitalOcean API token](/integrations/credentials/digitalocean) with read/write
- **UKS** - [UpCloud API credential](/integrations/credentials/upcloud)
- **GKE** - [GCP service account key](/integrations/credentials/gcp) with the **Kubernetes Engine Admin** (`roles/container.admin`) and **Service Account User** (`roles/iam.serviceAccountUser`) roles, and the **Kubernetes Engine API** enabled
- **OVH MKS** - [OVHcloud application keys](/integrations/credentials/ovh) with a Public Cloud project (`project_id`)
- **AKS** - [Azure service principal](/integrations/credentials/azure) with **Contributor** on the target subscription
- **EKS** - [AWS credential](/integrations/credentials/aws) with permissions for EKS, EC2, and IAM role management
