...
A few notification kinds are **opt-in**: a rule only matches them if it *names* them. "All kinds" does not cover an opt-in kind, and neither does "All kinds except ...", because that spelling says what a rule skips rather than what it covers. The home channel fallback never picks one up either, whatever its severity.

These kinds report routine, successful work rather than something that needs attention, and they can fire often. Leaving them out of the catch-all rules an organisation already has is what keeps a channel worth reading.

| Kind | What it reports |
|---|---|
| `gitops_repository_write` | Ankra committed to your GitOps repository: the reconciler converging drift, or a save in the portal, the API, the CLI, chat, or a cloud provider publish. The notification names the repository, the branch, the commit, who or what wrote, and the files it re-encrypted, wrote and removed. |

To subscribe, create a rule that names the kind:

<CodeGroup>

```bash CLI
ankra alerts routes create --destination-id <destination-id> --kinds gitops_repository_write
ankra alerts routes preview --kind gitops_repository_write --severity info --cluster-id <cluster-id>
```

```bash cURL
curl -X POST https://platform.ankra.app/api/v1/org/notifications/routes \
  -H "Authorization: Bearer <your-token>" \
  -H "Content-Type: application/json" \
  -d '{"kinds": ["gitops_repository_write"], "destination_id": "<destination-id>"}'
```

</CodeGroup>

Add a `cluster_id` filter to follow one cluster's repository rather than every cluster in the organisation.

<Note>
The **Kind** picker on the Routing rules page offers a shorter list than the API accepts. A rule for a kind the picker does not offer is created through the CLI, the API, or Ankra chat, and then reads and edits normally on that page.
</Note>
...
- It never fires for an [opt-in kind](#opt-in-kinds), which reaches a destination only through a rule that names it.
- It never fires for an alert firing that already delivers to the alert's own destinations.

## Notifications addressed to a person

Most notifications belong to the organisation: a cluster went offline, a backup
failed. Some belong to one member - the run that failed is attributed to them,
and the card says "you". Those follow their own rules.

**A person-addressed notification never reaches an organisation webhook.** A
webhook destination is a shared channel, and nothing about it narrows by
audience, so routing one there would post a second-person message into a room
the whole organisation reads - and, at critical severity, page whoever an
"everything critical" rule points at for a message that was never theirs. The
home channel fallback is refused for the same reason. Routing rules govern the
organisation's notifications; they are an administrator's surface.

Where it does go is the member's own channels, under **Settings** →
**Notifications** → **How you are notified**:

| Channel | Default | Notes |
| --- | --- | --- |
| Email | Critical only | Sent to the contact address on the account. Warnings are opted up to, not out of - email is the one channel that leaves the platform and cannot be recalled. |
| Mobile push | Critical and warnings | Delivered to the devices registered from the Ankra app. |
| Browser | Critical and warnings | Shown by the browser while Ankra is open. Needs the browser's own permission as well as the switch. |

A member who has never opened that page is on the defaults above; nothing has to
be configured for a failure to reach them. Turning a channel off, or narrowing it
to a higher severity, applies to the notifications addressed to that member only -
it never silences the organisation's routing rules, and a routing rule never
overrides it. Per-kind mutes apply to these channels exactly as they apply to the
inbox.

The organisation still sees its own copy: a failing branch raises a standing card
for the organisation as well as the personal one, so the two audiences read
