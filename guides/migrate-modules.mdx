...
```bash
ankra migrate modules install https://example.com/downloads/ankra-module-procfile \
  --sha256 <hex-digest>
...
`ankra migrate modules install <url-or-file>` fetches one module executable, from an `https` URL or a local file, into `~/.ankra/modules`. There is no index and no registration: the URL is the distribution, and the module's own `describe` answer is the handshake. Nothing is kept until that answer checks out, because the download is staged in a temporary file, run there, and only then moved into place under the name the module calls itself - which is the name you will later type after `--module`.

| Flag | What it does |
|------|--------------|
| `--sha256 <hex>` | Pin the download. A digest that does not match is refused before anything runs |
| `--name <name>` | Assert the name you expect. A module that calls itself something else is refused rather than installed under a surprise name |
| `--force` | Replace a module of the same name that is already installed. Without it, an existing one is left alone |
| `-y, --yes` | Skip the confirmation prompt |

Plain `http://` is refused, because the network would otherwise choose which executable you run, and a download larger than 256 MiB is refused as well. The confirmation states in as many words that the module's `describe` verb is about to run and that the module later runs with your permissions. The command prints the sha256 of what it installed, so you can record it or compare it with a checksum the author published.

<Warning>
A module is an executable you are choosing to run. It reads your files, and an `export` module reaches whatever the source's data lives behind, such as your Docker socket. Install only modules you trust, and pass `--sha256` whenever the author publishes a digest.
</Warning>

`ankra migrate modules uninstall <name>` removes what `install` put in `~/.ankra/modules`, after a confirmation that `--yes` skips. It never deletes anything else: a module that lives on `PATH` was put there by something other than the CLI, so the command refuses it and names the location, leaving you to remove it the way it was installed.

Placing the executable yourself still works, and discovery does not care how it got there. `install` exists so that fetching, verifying and naming a module is one auditable step rather than four.
...
```bash
ankra migrate modules install ./examples/modules/ankra-module-procfile --yes
...
## Plugins are a different mechanism

A module teaches `ankra migrate` a source format. If what you want is a new `ankra` command instead, that is a **plugin**, and the CLI dispatches to them the way `kubectl` does.

An executable named `ankra-<command>` in `~/.ankra/plugins` or anywhere on `PATH` runs as `ankra <command>`, with the remaining arguments, this terminal's standard input, output and error, and `ANKRA_CLI_VERSION` in its environment. Its exit code becomes the CLI's, so a plugin is scriptable exactly like a built-in command.

```bash
ankra plugins
```

- **Built-in commands always win.** A plugin can extend the CLI but never change it, and `ankra plugins` warns about one that a built-in shadows and will therefore never run.
- **Multi-word names resolve longest first.** With both installed, `ankra foo bar` runs `ankra-foo-bar` rather than `ankra-foo` with `bar` as an argument.
- `~/.ankra/plugins` is searched before `PATH`, the same order `ankra migrate modules` uses for `~/.ankra/modules`.

The two mechanisms never collide, because they are told apart by the file name: `ankra-module-<name>` is a migrate module, listed by `ankra migrate modules`, and anything else beginning with `ankra-` is a plugin, listed by `ankra plugins`. There is no installer for plugins; put the executable in `~/.ankra/plugins` yourself. A plugin runs with your permissions, so the same rule applies as for modules: only install ones you trust.
