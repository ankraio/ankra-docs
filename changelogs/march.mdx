...
description: Hetzner Cloud provisioning, AI Agentic Mode, Log Explorer, Pod Terminal, Alerting with AI Incident Reports, Metrics Revamp, Appearance Control (Light/Dark/System), SOPS with ArgoCD, Variables, Sandbox Clusters, CLI & API Updates
---
...
```bash
ankra cluster encrypt manifest my-secret-key DB_PASSWORDf cluster.yaml
ankra cluster encrypt addon-name grafana-key adminPasswordf cluster.yaml
ankra cluster decrypt manifest my-secretf cluster.yaml
```
...
## Appearance Control: Light, Dark & System Modes

The Ankra portal now supports full appearance customization with three theme modes: Light, Dark, and System.

**Dedicated Appearance Page**: Navigate to **Profile → Appearance** to choose your preferred theme. Select Light for an always-light interface, Dark for an always-dark interface, or System to automatically follow your operating system's preference.

**Sidebar Theme Toggle**: A quick-access theme toggle in the sidebar lets you cycle between Dark, Light, and System modes without leaving your current page. The icon updates to reflect the active mode — moon for dark, sun for light, monitor for system. When the sidebar is collapsed, the toggle shows as an icon with a tooltip.

**Persistent Preference**: Your theme choice is saved to your account and synced across all devices and sessions.
