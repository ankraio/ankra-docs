---
title: Introduction
description: "Ankra is one Kubernetes platform with four first-class surfaces: a web UI with Stack Builder and AI Assistant, a scriptable CLI (ankra chat, ankra cluster apply), a remote MCP server with 180+ tools for Claude/Cursor/agents, and CI/CD integrations (GitHub Actions, GitLab, pipeline agents, Terraform). One tool registry powers them all."
---
...
All four surfaces share one tool registry: the same 180+ tools power the in-product AI Assistant, the MCP server, chat over Slack and Microsoft Teams, and SCM comments - so what you can ask Ankra to do is the same everywhere.
...
<CardGroup cols={2}>
  ...
  >
    A remote MCP server exposing 180+ tools with `mcp:read`/`mcp:write` scopes and OAuth. Built for Claude, Cursor, and any other MCP client.
  </Card>
  <Card
    title="CI/CD & IaC"
    icon="code-branch"
...
Then teach your assistant Ankra's conventions - `ankra skills install` detects
the AI assistants configured on your machine (Claude Code, the Claude app, Cursor,
Codex, GitHub Copilot and more) and installs Ankra's curated
[Agent Skills](/integrations/agent-skills) into all of them.
...
<CardGroup cols={2}>
  ...
  >
    Provision Kubernetes clusters (kubeadm with Cilium, or K3s) on Hetzner, OVHCloud, or UpCloud with automated networking, bastion access, and NAT gateways.
  </Card>
  <Card
    title="Cloud Cost"
    icon="coins"
