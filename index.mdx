...
Import any cluster EKS, GKE, AKS, on-prem, or local and manage it from one place. Or provision managed clusters on Hetzner, OVHCloud, UpCloud, or DigitalOcean directly from the platform.
...
<CardGroup cols={2}>
  ...
  >
    Provision K3s clusters on Hetzner, OVHCloud, UpCloud, or DigitalOcean with automated networking, bastion access, and NAT gateways.
  </Card>
  <Card
    title="Applications"
    icon="rocket"
...
<Steps>
  ...
  <Step title="Add a cluster">
    Import an existing cluster by installing the Ankra agent, or provision a new one on Hetzner, OVHCloud, UpCloud, or DigitalOcean.
    ```bash
