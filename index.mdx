...
Import any cluster EKS, GKE, AKS, on-prem, or local and manage it from one place. Or provision managed clusters on Hetzner, OVHCloud, UpCloud, or DigitalOcean directly from the platform — or on your own infrastructure with Proxmox VE and Morpheus.
...
<CardGroup cols={2}>
  ...
  >
    Provision K3s clusters on Hetzner, OVHCloud, UpCloud, DigitalOcean, Proxmox VE, or Morpheus with automated bastion access and node group management.
  </Card>
  <Card
    title="Applications"
    icon="rocket"
    href="/essentials/applications"
  >
    Connect an app repo and Ankra generates the Dockerfile, Helm chart, and CI/CD - then ships it as a stack.
  </Card>
  <Card
    title="Cloud Cost"
    icon="coins"
    href="/essentials/cloud-cost"
  >
  ...
  <Card
    ...
    icon="copy"
    href="/essentials/stack-profiles"
  >
...
<Steps>
  ...
  <Step title="Add a cluster">
    Import an existing cluster by installing the Ankra agent, or provision a new one on Hetzner, OVHCloud, UpCloud, DigitalOcean, Proxmox VE, or Morpheus.
    ```bash
