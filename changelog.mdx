...
A canvas exported from Claude Design - the artboards, `canvas.json` and images, a zip of them, or a saved canvas page - becomes a deployable application in one step. Ankra converts the artboards into a static site, creates a GitHub repository under your GitHub credential, commits the site with a Dockerfile, and registers the application; the setup pull request, build and deploy lane takes over from there. Artboards that depend on the Claude Design runtime are kept as authored and reported as warnings.
