...
<Update label="2026-09-12" tags={["AI", "Cost"]} rss={{ title: "Stack READMEs, code review and background runs follow your model settings, on GLM 5.2 by default" }}>

## Stack READMEs, code review and background runs follow your model settings, on GLM 5.2 by default

Stack README generation, AI code review with no model picked, and background agent runs - tickets, missions, remediation, automations and alert tasks - used a fixed Claude model. They now follow your organisation's model settings and default to the **Think** tier: GLM 5.2 on the Ankra provider, or your own Think model when you run on your own key. **Stack README** and **AI code review** each get a setting under **Models per function**, and changing any function's model is now admin-only. An unselected code review uses the same model whatever the size of the diff.

Chat served by the Ankra default models on the platform key now counts against the [free monthly AI allowance](/platform/ai-usage), like the rest of Ankra-funded AI usage.
