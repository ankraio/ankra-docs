...
# Team Roles, Cluster Groups, Deploy Waves, Autoscaling, and Vulnerability Scanning

A wave of platform control-plane features landed this month: fine-grained access control, ordered deployments, elastic node groups, and built-in security scanning.

## Roles, scoped assignments, and cluster groups

Access is now governed by [roles](/essentials/roles-and-access) you assign at a chosen scope. Assign a member a role across the whole organisation, a single cluster, or a **cluster group** - a named, static or label-selected set of clusters - and a role can bundle Kubernetes access that Ankra provisions on every cluster in scope. Built-in roles (`owner`, `admin`, `operator`, `member`, `viewer`, `read-only`) cover the common cases, and you can define custom roles. Every change is captured in a new organisation **audit log**.

[Roles & Access →](/essentials/roles-and-access)

## Ordered rollouts with deploy waves

Stacks now accept an optional [deploy wave](/essentials/stacks#deploy-waves). Stacks in a lower wave finish before stacks in a higher wave begin, and teardown unwinds in reverse - so platform prerequisites (ingress, cert-manager, secrets operators) land before the workloads that depend on them, without hand-wiring cross-stack dependencies.

[Deploy Waves →](/essentials/stacks#deploy-waves)

## Node group autoscaling

Worker [node groups](/essentials/cluster-settings#node-group-autoscaling) on Hetzner, OVH, and UpCloud clusters can now scale themselves. Set a minimum and maximum and the Cluster Autoscaler adds nodes when pods can't be scheduled and removes them when they're idle - installed automatically the first time you enable it.

[Node Group Autoscaling →](/essentials/cluster-settings#node-group-autoscaling)

## Cluster vulnerability scanning with Trivy

Install the Trivy Operator add-on and Ankra reads its vulnerability reports into a dedicated [Security console](/essentials/cluster-security): severity breakdowns, a blast-radius-ranked priority queue, remediation campaigns, and a phased burn-down plan. The AI assistant can summarise and prioritise CVEs for you with the same data.

[Cluster Security →](/essentials/cluster-security)

## Cluster integrations hub

Cluster data sources - metrics, security, cost, and logs - now live in one place under [Settings → Integrations](/essentials/cluster-settings#integrations). Connect Prometheus (self-hosted, Grafana Cloud, or VictoriaMetrics), Trivy security, Cloud Cost, an [external log viewer](/integrations/log-sources), or a log source, each with a live connection status.

[Cluster Integrations →](/essentials/cluster-settings#integrations)

</Update>
<Update label="July 2026">
# kubeadm Clusters, Selectable CNI, and External etcd

Self-managed clusters on Hetzner, OVH, UpCloud, DigitalOcean, Proxmox VE, and Morpheus can now be provisioned with **kubeadm** (upstream Kubernetes) as an alternative to k3s, and you can choose the **CNI** at creation.

- **Distribution choice.** Pick `k3s` (lightweight, default) or `kubeadm` (vanilla upstream with containerd) in the create wizard, CLI, or API.
- **Selectable CNI.** k3s clusters choose Flannel (default), Calico, or Cilium, with advanced feature toggles (Cilium `kube_proxy_replacement`, Hubble, WireGuard encryption; Calico eBPF dataplane). kubeadm clusters always use Cilium. The CNI is fixed once the cluster is created.
- **External etcd topology.** kubeadm clusters can run etcd stacked on the control planes or on dedicated etcd nodes (3 or 5), upgraded one at a time with a pre-upgrade snapshot.

[Hetzner Clusters →](/essentials/hetzner-clusters#kubernetes-distribution) · [Kubernetes upgrades support both distributions.](/essentials/cluster-settings)

</Update>
<Update label="July 2026">
# Managed Kubernetes Everywhere: GKE, AKS, EKS, and OVHcloud MKS

Ankra now provisions and manages **provider-native managed Kubernetes** on the major clouds — [Google Kubernetes Engine](/essentials/managed-kubernetes), [Azure Kubernetes Service](/essentials/managed-kubernetes), [Amazon EKS](/essentials/managed-kubernetes), and [OVHcloud Managed Kubernetes](/essentials/managed-kubernetes) — joining DigitalOcean Kubernetes (DOKS) and UpCloud Managed Kubernetes (UKS). The provider runs the control plane; Ankra manages everything on top.

- **Full lifecycle.** Create with node pools, labels, and autoscaling; scale, add, and delete pools; upgrade Kubernetes to the versions the provider actually offers; delete — from the portal, the CLI, the API, or Ankra AI chat.
- **Live options and pricing.** Regions, Kubernetes versions with support windows, node sizes, prices (including spot where available), and quotas are fetched live from the provider with your credential — never from a static list — so you always choose from current, accurate data.
- **Preflight checks.** Every create runs provider-side checks (credential, quota, naming) first and reports problems before anything is provisioned.
- **Discover & import.** Point a credential at your cloud account, see every managed cluster that already exists (with already-imported markers), and adopt them into Ankra without touching the running workloads.
- **Provider-specific depth.** GKE release channels and Autopilot, AKS network plugins / private clusters / SKU tiers, EKS subnets and control-plane logging with automatic IAM role management, OVH MKS plans and update policies, DOKS HA control planes, UKS control-plane plans and GPU pools.

```bash
ankra cluster managed options  --provider gke --credential-id ...
ankra cluster managed create   --provider aks --name prod --credential-id ... --location westeurope ...
ankra cluster managed discover --provider eks --credential-id ...
ankra cluster managed import   --provider eks --credential-id ... --cluster-id ...
```

Azure joins the credential catalog with service-principal support, and GCP credentials can now provision GKE (grant `roles/container.admin`).

[Managed Kubernetes →](/essentials/managed-kubernetes)

</Update>
<Update label="July 2026">
# Bring Your Own Infrastructure: Proxmox VE and Morpheus Clusters

Ankra now provisions self-managed Kubernetes clusters on [Proxmox VE](/essentials/proxmox-clusters) and through [Morpheus](/essentials/morpheus-clusters) — with the same lifecycle you already use on Hetzner, OVHcloud, UpCloud, and DigitalOcean: create with k3s or kubeadm (including multi-node control planes), manage worker node groups with labels and taints, scale, upgrade Kubernetes versions, stop/start, manage SSH keys, and deprovision from the portal or CLI.

Both providers are designed for private environments:

- **Hybrid connectivity.** If the Proxmox or Morpheus API and the VM network are not directly reachable from Ankra, attach an SSH jumphost (host, port, username, private key) to the credential — Ankra tunnels both API calls and node SSH through it.
- **Self-signed certificates.** A TLS insecure toggle on the credential supports appliances without a publicly trusted certificate.
- **Your network, your rules.** Ankra provisions VMs onto an existing Proxmox bridge or Morpheus network with DHCP addressing — no networks or bridges are created for you.

```bash
ankra credentials proxmox create --name my-proxmox
ankra cluster proxmox create --name prod --credential-id ... --proxmox-node pve01 --storage local-lvm --bridge vmbr0 ...
ankra credentials morpheus create --name my-morpheus
ankra cluster morpheus create --name prod --credential-id ... --group-id ... --cloud-id ... --network-id ...
```

With full Proxmox VE provisioning in place, the internal **Ankra Cloud** demo cluster (a single-VM Proxmox demo) has been removed. To try the platform without infrastructure, use a [sandbox cluster](/essentials/cluster-sandbox) instead.

[Proxmox VE clusters →](/essentials/proxmox-clusters) · [Morpheus clusters →](/essentials/morpheus-clusters)

## Roadmap Preview: VMware vCenter

Support for provisioning clusters on VMware vCenter is coming soon.

</Update>
<Update label="July 2026">
# DigitalOcean Clusters and Managed Kubernetes

Ankra now provisions self-managed K3s clusters on [DigitalOcean](https://www.digitalocean.com/) alongside Hetzner, OVHcloud, and UpCloud — with the same lifecycle you already use: create, scale node groups, upgrade Kubernetes, stop/start, and deprovision from the portal or CLI.

You can also connect **DigitalOcean Kubernetes (DOKS)** and **UpCloud Managed Kubernetes (UKS)** as managed clusters: Ankra creates the cluster via the provider API, installs the agent using a kubeconfig, and exposes scale and upgrade through the platform.

```bash
ankra credentials digitalocean create --name my-token
ankra cluster digitalocean create --name prod --credential-id ... --region nyc3 ...
ankra cluster managed create --provider doks --name prod-doks --credential-id ... --location nyc3
```

[DigitalOcean clusters →](/essentials/digitalocean-clusters)

</Update>
<Update label="June 2026">
# Pipeline Agents: CI-Native Infrastructure Review, Deploy Watching, and Health Checks

Build infrastructure agents from plain CI jobs and the Ankra CLI - no framework, no orchestration layer. `ankra chat` is one-shot and scriptable, with server-side access to cluster context (logs, events, manifests, and stack history).

- **PR review agent** - validate cluster-definition changes against the live platform and comment on the PR with what will actually change, including capacity headroom.
- **Deploy watcher** - `ankra cluster apply --wait` on merge, verify rollout health, and post a root cause to Slack when a deploy fails instead of a red X.
- **Scheduled health watcher** - run on a cron, stay silent while healthy, and post to Slack only when something degrades.

[Build pipeline agents →](/guides/pipeline-agents)

</Update>
<Update label="June 2026">
# Ask Ankra from Your Own Tools: MCP Server, Hermes, and Bitbucket

- **Model Context Protocol (MCP).** Connect Ankra to any MCP-compatible AI client (Claude Desktop, Cursor, and more) so your assistant can read cluster context and drive Ankra workflows directly. [MCP integration →](/integrations/mcp)
- **Hermes autonomous agent.** Hermes brings `ankra chat`'s reasoning to autonomous infrastructure workflows. [Hermes →](/integrations/hermes)
- **Bitbucket Cloud.** Connect Bitbucket Cloud repositories for GitOps alongside GitHub and Bitbucket Data Center. [Bitbucket Cloud →](/integrations/bitbucket-cloud)

</Update>
<Update label="June 2026">
# Two-Factor Authentication, Built into Ankra

Two-factor authentication is now managed by Ankra and protects every sign-in - in the browser and from the CLI. Add a phishing-resistant **passkey** (Touch ID, Face ID, Windows Hello, or a device passkey), a hardware **security key**, or an **authenticator app** (Google Authenticator, 1Password, Authy, or any TOTP app) from **Profile → Authentication**.

- **One set of recovery codes.** Enrolling your first factor gives you one-time recovery codes that back up every method - no separate codes per factor. Regenerate them anytime; the Authentication page shows how many remain.
- **Step-up at every sign-in.** Once a factor is enrolled you are asked for it each time you log in. `ankra login` opens your browser to complete the second step and then finishes automatically.
- **Organisation enforcement.** Organisations that require two-factor authentication prompt members to enrol before they can continue, and admins can reset a locked-out member's factors.
...
The [Ankra Agent](/essentials/cluster-agent) has been rebuilt as a single, statically compiled Go binary and is now the production agent on the new `2.0.x` version track.
...
See the [Ankra Agent](/essentials/cluster-agent) docs for the full configuration reference.

</Update>
<Update label="June 2026">
# Fleet Dashboard, Cloud Cost, kubectl Access, Stack Profiles, Applications, and Support

A wave of platform features landed this month, spanning the dashboard, cost visibility, cluster access, reusable stacks, app delivery, and in-product support.

## A fleet dashboard with a world map

The organisation [Dashboard](/essentials/dashboard) now opens on a world map of every cluster, alongside health and cost rollups. Clusters are placed by cloud region, or by the agent's public egress IP when [public IP reporting](/essentials/cluster-agent) is enabled.

## Cloud cost estimates

[Cloud Cost](/essentials/cloud-cost) estimates per-cluster and fleet-wide spend from your live node and volume inventory plus cloud pricing - broken down by compute, storage, network, and per-namespace allocation. Connect an [AWS or GCP credential](/integrations/credentials) to get started.

## Scoped kubectl access through Ankra

Grant teammates SSO-backed, scoped Kubernetes access with [Cluster Access](/essentials/cluster-access), then reach the cluster with your own `kubectl` through Ankra's [API proxy](/essentials/kubeconfig) - short-lived credentials, no inbound network access, works with private clusters.

## Reusable Stack Profiles

Capture a working stack as a versioned, parameterised template with [Stack Profiles](/essentials/stack-profiles), then instantiate it on any cluster - with diffing, update tracking, and IaC export.

## Applications: from repo to running workload

[Applications](/essentials/applications) connect an app's Git repository, generate the Dockerfile, Helm chart, and CI/CD via a pull request, then build and deploy the result as a stack.

## In-product support

Raise and track [support tickets](/essentials/support) without leaving Ankra - with AI triage, attachments, and threaded replies - and keep an eye on everything that needs you in your new [Activity & Inbox](/essentials/activity-inbox).

</Update>
<Update label="June 2026">
# Manage Cloud Clusters Without Picking a Provider

Upgrading, scaling, and managing node groups on a cloud cluster no longer means remembering whether it runs on Hetzner, OVH, or UpCloud. New provider-agnostic verbs detect the provider automatically from the cluster:

```bash
ankra cluster k3s-versions                       # list upgrade targets (stable highlighted)
ankra cluster upgrade <cluster_id> v1.36.1+k3s1  # upgrade the Kubernetes version
ankra cluster scale <cluster_id> 3               # scale the default worker pool
ankra cluster node-group list <cluster_id>       # manage node groups
```

`ankra cluster deprovision` now takes a cluster ID or name and releases cloud resources for cloud clusters. The older provider-specific `ankra cluster {hetzner,ovh,upcloud} upgrade | scale | node-group | deprovision` commands still work but are deprecated and print a warning pointing at the new verbs.

[Ankra CLI →](/integrations/ankra-cli)

# Grant Cluster Access and Connect kubectl from the CLI

You can now manage who reaches a cluster's Kubernetes API and wire up `kubectl` without leaving the terminal. `ankra cluster access` grants, lists, and revokes per-user access through the Ankra kube gateway, mapping an organisation member to a Kubernetes role (`view`, `edit`, `admin`, or `cluster-admin`), cluster-wide or scoped to a single namespace.
...
`ankra cluster kubeconfig add` writes an `ankra-*` context into your kubeconfig that fetches a short-lived, SSO-backed token on demand (run `ankra login` once), leaving your other clusters and contexts untouched. Managing access requires organisation admin rights.

[Ankra CLI →](/integrations/ankra-cli)

# Query Cluster Metrics from the CLI

`ankra cluster metrics query` and `query-range` run PromQL against the Prometheus metrics source configured for a cluster, proxied through the Ankra agent - handy for ad-hoc checks and CI, with table, JSON, or YAML output.
...
ankra cluster operations list <operation_id> --watch --interval 3s

# Track the per-step status for an operation
ankra cluster operations steps <operation_id>
```

`--watch` redraws the screen on each poll (default every 5 seconds, tunable with `--interval`) and stops automatically once the execution - or every execution in the list - finishes, so the command exits cleanly instead of spinning forever. `ankra cluster operations steps <operation_id>` shows the execution summary plus each step's status, created, and updated times. Statuses are colorized (green success, red failure, yellow running) to make in-progress work easy to spot at a glance.
...
- **Encrypt:** `ankra cluster encrypt manifest db-secret --key password`, `ankra cluster encrypt addon --name grafana --key adminPassword --cluster prod`
...
`ankra skills` installs the curated Ankra Agent Skills into your Cursor or Claude Code skills directory, so your AI agent follows Ankra's recommended practices for the CLI, ImportCluster YAML, stacks/addons, GitOps, CI/CD, SOPS secrets, Helm registries, observability, alerts, Terraform, and cloud clusters. The skills are embedded in the CLI binary, so installation works offline and is versioned with the release.
...
Use `--project <dir>` for a per-project install in the selected editor's config directory (`./.cursor/skills` or `./.claude/skills`) and `--force` to overwrite existing skills. This is separate from `ankra openclaw skill`, which generates a SKILL.md describing one of your own clusters; `ankra skills` installs the static, curated skill set.
...
- `ankra cluster get pods` / `ankra cluster get resources` / `ankra cluster logs` - Access Kubernetes resources and stream pod logs
...
This month we've revamped our documentation to put the AI Assistant front and center, added full guides for building production stacks, and expanded integrations.

## Stack Builder

The visual Stack Builder is now a first-class citizen with full documentation.
...
- `ankra cluster stacks list` List all stacks in the active cluster
- `ankra cluster stacks list <name>` View stack details with manifests and addons
- `ankra cluster clone <source> <dest>` Clone stacks between clusters
- `ankra cluster apply -f stack.yaml` Apply stack configurations
...
**Unified Context Layers**: The AI combines all context for full understanding:
| Layer | What AI Sees |
|-------|-------------|
...
**Live Documentation**: full documentation is now available across the platform.
</Update>
...
**Local-to-Cloud Parity**: Use the `helm install ankra-agent` command from the import cluster on your laptop or on your Kubernetes clusters.
