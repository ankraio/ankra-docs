...
# Hand-Maintained Files Survive Every Sync, Live Usage, Agent and GitOps Posture From Chat and the MCP Server, Ankra Pipelines: Your CI Runs in Your Cluster, Raise Support Tickets From Chat and the MCP Server, One Status Comment per Pull Request, Convert a Workflow Into an Ankra Pipeline From Chat, Ask Ankra AI to Create Your Alerts, A Software Bill of Materials for the Fleet, Security by Namespace and Pod, A What-If Draft That Predicts the Bill Before You Resize, Your Money in Your Currency, A Cost Center That Answers Whose Money and What It Runs, Cost for Capacity Nobody Publishes a Price For, The Overview Says What Every Cluster Is, Playground Storage Says What It Is, Connect to Ankra From the Teams Welcome Card, and Rollout Verifications That Stop Paging You
...
## Ankra Pipelines: your CI runs in your cluster

Ankra can now run a repository's CI inside a cluster you already operate through Ankra, from one committed file, `.ankra/pipeline.yaml`, instead of a GitHub Actions workflow on runners you have to keep alive. What is live today: the definition with its validator and dry run (`ankra pipeline validate`, and `cicd_validate_pipeline_yaml` in chat); conversion of an existing GitHub Actions, GitLab CI, Bitbucket Pipelines or CircleCI definition; the organisation's CI settings and pipeline policy; runs you can dispatch, list, follow live, cancel and re-run from the API and the new `ankra pipeline` commands; triggers from push, pull request and tag deliveries on GitHub, GitLab and Bitbucket; execution of `run` stages as hardened Jobs in the `ankra-ci` namespace of your CI cluster, once the organisation's CI cluster is set and the agent's `ci` worker pool is enabled (`ci_worker_count`); one `Ankra pipeline` check per commit on GitHub with a Cancel button, a commit status on GitLab and Bitbucket, and the pipeline section of the pull request's single status comment; and step logs and artifacts kept in your own backup vault. Connecting a repository to its pipeline through the setup pull request is the next piece, together with the runners for the `checkout`, `build`, `scan` and `verify` stages - the guide says exactly where the line is.
