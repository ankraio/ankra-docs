...
Still ahead: trust isolation for a fork's cache claims, so a fork's pull request cannot write into the cache a trusted run later restores, and a narrower `egress-https` than "port 443 to anywhere".
...
Still ahead: the per-run identity that `permissions` mints, the `ankra/*` step library behind `uses:`, the executors for the kinds listed above (skipped rather than hanging in the meantime), trust isolation for a fork's cache claims, and the agent release that moves live step output onto its own dedicated stream - the guide says exactly where the line is today. The five extra protected sections and the applied `--spec-file` override that were listed here have since shipped, below.
