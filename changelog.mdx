...
A profile with two dozen inputs never fitted the dialog **Use Profile** opened: most of the form sat below the fold and the button hid under a scroller. It now opens a page of its own. The destination and the inputs take the main column, and a summary beside them stays in view while you scroll, showing the profile, version and target, a checklist that ticks off as the form fills in, and **Create draft**. Picking a version from History, updating a deployment, and starting an application or a metrics stack from a profile all land on the same page with their cluster and version already chosen.
