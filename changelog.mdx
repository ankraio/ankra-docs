...
Still ahead: a narrower `egress-https` than "port 443 to anywhere", with the per-organisation `ci_egress_allowed_cidrs` allow-list that makes the tightening survivable, and the fix for an image-policy prefix written with a trailing separator, which matches nothing today. Both are open on the platform and on the agent, so neither is deployed.
...
Still ahead: a narrower `egress-https` than "port 443 to anywhere". Trust isolation for a fork's cache claims has since shipped, above.
