...
Organisation admins can now pick which Ankra-managed domain their zones live under. Deployments offer a list of Avura-hosted roots - `ankra.cc` stays the default, and `smartoptics.dev` is the first alternative - and an **Ankra domain** picker sits next to the preview domain under **AI** → **Settings** → **Workspaces**. Zones you already have keep the domain they were provisioned under; after a switch, Ankra tears down the organisation zone and re-creates it under the new domain automatically, and every cluster zone and preview URL minted from then on follows.
