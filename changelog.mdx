...
Saving a protected stack as a profile now keeps its protection: the builder shows a **Protection** section with whether to protect, which backup vault to write to, and how often. The vault travels by name and is resolved against the organisation the profile is launched into, so one profile protects instances across a fleet. A launch into an organisation with no ready vault says so and keeps the draft rather than bringing a stateful stack up unprotected, and a public profile carries no vault name - the organisation installing it supplies its own.
