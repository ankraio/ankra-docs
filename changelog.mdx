...
The bot's welcome card now carries a **Connect to Ankra** button. Press it and the portal opens with the claim code already filled in - press **Connect** and the tenant is bound. For admins starting from the portal instead, the Teams card leads with **Add to Microsoft Teams**, and the manual walkthrough collapses behind **Connect manually instead**. Pasting the code by hand still works exactly as it did.
