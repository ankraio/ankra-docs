...
Skills target Cursor by default; pass `--editor claude-code` to install into Claude Code's `.claude/skills` instead. Use `--project <dir>` for a per-project install and `--force` to overwrite existing skills. This is separate from `ankra openclaw skill`, which generates a SKILL.md describing one of your own clusters; `ankra skills` installs the static, curated skill set.
