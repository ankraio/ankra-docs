...
[Proxmox VE credentials →](/platform/credentials/proxmox#tailscale-headscale-optional)

## Proxmox clusters get their own DNS zone

Proxmox VE was the only self-managed provider whose clusters never received a delegated DNS subzone. The networking stack shipped Traefik and cert-manager but left out `external-dns`, because the subzone that scopes its credentials was never requested — so hostnames under the cluster's own zone had nowhere to resolve, and PR preview deploys stayed reachable only from inside the cluster unless the organisation had configured its own preview domain.

New Proxmox clusters now request that subzone like every other provider, and `external-dns` is wired into the networking stack against it. Pass `include_dns: false` at create time to opt out. Clusters created before this change keep their current setup.

[Proxmox VE reference →](/reference/proxmox#cluster-configuration-options)
