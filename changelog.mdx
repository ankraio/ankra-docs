...
Two ways a mesh member could sit in `configuring` for good are closed. A cluster created with networking off has no Cilium release the platform manages, so the mesh values had nowhere to land; readiness now reports it under a new `cilium_stack` check, the join refuses it, and make-ready refuses it by name instead of reporting success. And a member whose trust-root job never ran, or whose confirmation never reached the platform, is re-dispatched by the mesh reconciler once fifteen minutes have passed without a confirmation, instead of waiting for a peer list change that never comes.
