...
The **Where the same fleet would bill** row on a cluster's Compare providers page now offers every provider Ankra can price, not only the five it builds on. AWS, Google Cloud and Azure are priced from their own catalogues as the platform's inventory loops read them; because those books are per region, each column says which region it was priced in - the cluster's own where the target prices it, else the nearest European one. Proxmox VE and HPE Morpheus have no size to land on, so the fleet is priced per vCPU, per GB of memory and per GB of disk from the organisation's rate card, and the column names whether that card is one you stated or the rates Ankra ships. Unpriced is still never free: a hyperscaler whose inventory has not run, or a private provider nobody has priced, is left out rather than shown at zero.
