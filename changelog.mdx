...
Merge when green shipped with no control. Each GitHub installation's row now carries a **Merge when green** switch beside its AI mode, for organisation admins, and the same switch is `merge_when_green` on the binding profile, writable with a personal access token through `PUT /api/v1/org/ai-gateway/scm-bindings/{provider}/{binding_external_id}`. A request is honoured only in Agent mode, and with the switch on a `Depends-on:` line holds a pull request's merge behind the one it names.
