...
Both halves shipped with the agent release of 5 September 2026. Your cluster's agent renders a step's NetworkPolicy and boundary-checks its image itself, so a cluster still on an older agent keeps the previous `egress-https` tier and the plain textual prefix check until the fleet roll reaches it.
...
The narrower `egress-https`, the per-organisation `ci_egress_allowed_cidrs` allow-list that makes the tightening survivable, and the fix for an image-policy prefix written with a trailing separator - all listed here once - have since shipped on the platform, above, and their agent halves shipped with the agent release of 5 September 2026.
