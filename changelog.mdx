...
A cluster's cost page gains **Compare providers**: the same fleet - node groups, the control plane Ankra provisioned, volumes and load balancers - priced on every provider Ankra can build on, side by side, with today metered where it runs. Matching is like-for-like by default, so a fleet on dedicated cores is not quoted as a saving on shared ones; the cheapest-fit figure stays as a footnote. Behind it the what-if estimate now reads a size's memory the way a node booted on it reports it, prices the control plane on both sides, re-prices metered volume storage on the target, and keeps a floor for a target it can only partly price.
