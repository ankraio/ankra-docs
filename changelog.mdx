...
## Previews on your own domain can get certificates

Fixed: a preview domain with no wildcard certificate left demos on plain `http`, and nothing said so.

Ankra requests a certificate for demos published on a staging cluster's own Ankra subzone. Once an organisation sets its own **Preview domain**, the zone belongs to that organisation, so Ankra stopped short of ordering anything against it - the assumption being that a wildcard certificate would be named in the **TLS secret** field. An organisation holding only per-host certificates had no way to say otherwise: its demo ingresses carried no TLS at all, and the only sign was the preview URL coming back as `http://`.

**Certificate issuer** is the missing setting. Name a cert-manager ClusterIssuer and Ankra annotates each demo ingress with it, so cert-manager issues a per-preview certificate. An HTTP-01 issuer is enough - the challenge is answered through the same ingress your preview hostnames already resolve to. Leave it empty and nothing changes, so no certificate is ever ordered against your domain without you asking.

The settings screen now states the plain-`http` fallback while both fields are empty instead of leaving it to be discovered, and `ankra org ai-environment get` says the same from the terminal.
