...
<Update label="2026-09-17" tags={["AI", "API"]} rss={{ title: "The MCP server speaks protocol revision 2026-07-28 and keeps serving earlier clients" }}>
## The MCP server speaks protocol revision 2026-07-28 and keeps serving earlier clients

Ankra's MCP endpoint now implements the current Model Context Protocol revision: per-request protocol metadata instead of an `initialize` handshake, `server/discover`, `resultType` and the server identity on every result, the `Mcp-Method` and `Mcp-Name` request headers validated against the body, and a cache hint on `tools/list`. Clients on 2025-11-25 and earlier are served at the same URL exactly as before. The MCP client behind the AI assistant's registered tool servers made the same move: it opens with `server/discover`, falls back to the handshake for servers that still need it, and mirrors `x-mcp-header` parameters into headers.

[Protocol compatibility →](/platform/mcp-server#protocol-compatibility)
</Update>
