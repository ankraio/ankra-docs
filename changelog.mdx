...
# A Bill of Materials for Every Running Container, Hand-Maintained Files Survive Every Sync, Live Usage, Agent and GitOps Posture From Chat and the MCP Server, Ankra Pipelines: Connect a Repository and Watch It Run, Ankra Pipelines: Build, Scan, Gate and Publish by Digest, Raise Support Tickets From Chat and the MCP Server, One Status Comment per Pull Request, Convert a Workflow Into an Ankra Pipeline From Chat, Ask Ankra AI to Create Your Alerts, A Software Bill of Materials for the Fleet, Security by Namespace and Pod, A What-If Draft That Predicts the Bill Before You Resize, Your Money in Your Currency, A Cost Center That Answers Whose Money and What It Runs, Cost for Capacity Nobody Publishes a Price For, The Overview Says What Every Cluster Is, Playground Storage Says What It Is, Connect to Ankra From the Teams Welcome Card, and Rollout Verifications That Stop Paging You

## A bill of materials for every running container

The SBOM tab answered "what did the scanner produce" and nothing else, so a container whose image no scan had covered was not a row, not a count, not anything - on one production cluster that hid the newest tag of every application. A new **Containers** view starts from what is actually running: every container on the scoped clusters, init containers included, joined to the bill of materials of its image or marked **No bill of materials**, listed first, with a headline that counts how many running containers are covered and how many are not. Every view now scopes by **namespace** and **workload** as well as cluster, and column headers sort like every other list in the portal. An image's bill of materials can be **downloaded** as a CycloneDX 1.5 document or a CSV, rebuilt complete from the stored components. An application's Security tab gains **Versions & SBOM**: one row per pushed tag with its bill of materials, its current findings and where it runs, downloads on each row. The CLI carries all of it (`ankra security sbom containers`, `ankra security sbom export`, `--workload-kind` / `--workload-name`, and the `-o json` the sbom family had promised and refused), and chat and the MCP server gain `list_security_sbom_containers` and the workload selector on the existing bill-of-materials tools.

[SBOM →](/guides/cluster-security#sbom)
