...
A failing run raised one organisation-wide card at warning severity, whatever it broke and whoever caused it. Now whoever pushed also gets a card addressed to them, and a failure on the default branch is critical. A pull request stays a warning whatever it targets. The card stands for the branch, so three failures in a row are one card, and a later green run closes it for everyone who was told.
...
Settings, then Notifications, now carries a switch and a severity floor for each of email, mobile push and browser, covering the notifications addressed to you. Email is new here and defaults to critical only. Push and browser cover critical and warnings. A member who never opens the page is already on those defaults, so a broken branch reaches them with nothing configured.
...
An Android phone could register for push and pass a test notification while never receiving a real one, because the query deciding who a routed notification reaches still selected iOS devices only. Every enabled device is now selected, and the transport is chosen per device when it is sent.

[Mobile push →](/guides/mobile-push)
</Update>
