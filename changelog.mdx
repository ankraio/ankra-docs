...
On Hetzner, OVHcloud, UpCloud and DigitalOcean a stop used to terminate the control plane with etcd on its disk, so Secrets, ConfigMaps, custom resources and the bindings between persistent volume claims and their cloud disks were gone on the next start, and StatefulSets came back on fresh empty volumes. Stop now captures an encrypted snapshot of the cluster's state first, terminates the machines only once it is safely in your backup vault, and the next start restores it: workloads come back with their configuration and reattach their existing disks. The dialog, the API (`preserve_state`, `restore_state`) and the CLI (`--preserve-state`, `--restore-state`) let you opt out.

Adding control planes no longer needs a stop either: growing from one to three controllers happens live on a running cluster, which also unlocks the live instance-type change for clusters that had a single controller. See [Stop cluster](/platform/cluster-settings#stop-cluster).
</Update>
