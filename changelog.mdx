...
**Clone Cluster** can now build the copy on a cloud provider instead of minting an import cluster. Pick **Build on a cloud provider** in the dialog, choose Hetzner, OVHcloud, UpCloud, DigitalOcean, Scaleway, Proxmox VE or HPE Morpheus, and the provider's create page opens with a *Cloning from* notice; Create Cluster there provisions the cluster and copies every stack from the source onto it as drafts. Stacks the provider installs itself - the cloud controller, networking, external-dns and Cilium - are left to the provider and reported as skipped, so a Hetzner cluster cloned to UpCloud comes up with UpCloud's controllers rather than Hetzner's.

The API takes the same shape: `POST /org/clusters/imported/{cluster_id}/clone` with `target_provider` and a `target` object carrying the provider's create body.
...
A canvas exported from Claude Design - the artboards, `canvas.json` and images, a zip of them, or a saved canvas page - becomes a deployable application in one step. Ankra converts the artboards into a static site, creates a GitHub repository under your GitHub credential, commits the site with a Dockerfile, and registers the application; the setup pull request, build and deploy lane takes over from there. Artboards that depend on the Claude Design runtime are kept as authored and reported as warnings. The lane ships behind the `claude_design_import` organisation feature flag - ask Ankra to enable it for your organisation.

`ankra application import claude-design <export>` on the command line, **Claude Design** as a source under Add application in the portal, or `POST /api/v1/org/applications/imports/claude-design`. Repositories that hold only pages (an `index.html` and no source) are now recognised as static sites when registered the ordinary way too, and get the same Dockerfile instead of an unbuildable one.
