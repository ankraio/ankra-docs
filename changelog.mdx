...
<Update label="2026-09-14" tags={["Pipelines", "Fixes"]} rss={{ title: "A generated pipeline is checked against your repository before the setup pull request carries it" }}>

## A generated pipeline is checked against your repository before the setup pull request carries it

A generated `.ankra/pipeline.yaml` used to be validated only as a pipeline: the stage kinds exist, the `needs` graph has no cycle, every secret a stage names is declared. Nothing asked whether the repository it was written for actually contains the files it points at, so a definition naming a Dockerfile nobody committed could be proposed, merged, and refused on the first push.

Setup now checks the definition against your repository before proposing it. Every `build` stage's Dockerfile is resolved exactly as the builder resolves it and looked for in the branch plus the files the same run is committing; a `run` stage with no image and a `publish` stage with no image repository are reported too. The findings appear on the setup run, naming the stage, so a monorepo says which component's push would fail. The setup pull request stays a proposal you can edit, so nothing is blocked by a finding.

Two related build fixes ship with it. A `build` stage that declares a `context` and no `dockerfile` now builds that context's own Dockerfile, which is what `docker build backend` reads; a declared `dockerfile:` is still a path from the repository root. And a build the runner refuses before it starts, because the Dockerfile is not in the repository at that commit or the target platform needs an emulator the node has not registered, now settles as `recipe_missing` or `build_emulation_missing` carrying the runner's own sentence, instead of `the step exited with status 78`.

## Generated test stages run your project's own commands

The test stage a generated pipeline renders now follows the lock file your component commits, rather than assuming pip and npm. A `uv.lock` project runs `uv sync --locked && uv run pytest` in a uv image; `poetry.lock` and `Pipfile.lock` get their own install and run; `pnpm-lock.yaml` and `yarn.lock` run pnpm and Yarn with a frozen lockfile instead of npm, which cannot reproduce their `node_modules` layout. Detection is still evidence-only: a repository that declares no suite gets no test stage, because a green stage that ran no tests reads as a passing suite.

Go test stages also get their own `GOPATH`, `GOCACHE`, `GOMODCACHE` and `GOTMPDIR` inside the workspace, in a directory named for the component, and a monorepo's workspace is sized by its component count instead of the single-application default. Several Go components testing into one workspace with none of that filled it, and the linker failed with `no space left on device` long before any test could fail on its merits.

A scan stage also stops running Semgrep's `p/github-actions` pack on a repository Ankra has established commits no workflow.
