...
rss: true
---
<Update label="2026-09-07" tags={["Security"]}>

## Licence risk on every bill of materials, and a warning when a private repository owes its source

Every package in the Security Center's SBOM now carries a **licence risk tier**: network copyleft (AGPL, SSPL, EUPL, OSL - serving it over a network obliges publishing the source of the service that links it), source-available (BUSL, Elastic, Redis Source Available, Commons Clause - a managed service needs a commercial licence), copyleft (GPL - safe on a private backend, distribution when shipped in a browser bundle, mobile app or CLI), weak copyleft (LGPL, MPL, EPL, CDDL), permissive, and unclassified. SPDX expressions are read as written - `MIT OR GPL-2.0` is permissive, `MIT AND GPL-2.0` is copyleft - and a package naming several licences is under all of them. The tier is stored beside the licences when the bill of materials lands, so "every AGPL package we run" is an indexed filter on the Components view, a chip beside the ecosystem chips, a **Licence obligations** figure on the summary that links to that list, a per-tier breakdown on every image, and a `license_risk` column in the CSV export. `ankra security sbom --license-risk network_copyleft` and the `list_security_sbom_components` AI tool take the same filter.

An application's **Versions & SBOM** pane reads those tiers against the repository it builds from. When the repository is **private** and the newest scanned version links a network-copyleft package, the pane opens with a **Licence obligation** notice - hosting that version as-is obliges you to publish its source, or to move the package behind a separate service - naming the packages, each a link into the image's bill of materials filtered to the tier. A source-available package raises the notice regardless of visibility; a public repository is told the obligation is already met; a repository the platform has not synced is treated as private, never as public. The verdict is on the API too, as `repository.visibility` and `license_exposure` on the application's versions endpoint.

[Licence risk →](/guides/cluster-security#licence-risk)

</Update>
<Update label="2026-09-06" tags={["AI"]}>
