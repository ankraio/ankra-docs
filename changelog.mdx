...
The cross-repository pass answered from default branches only, so it could not see the client fix that was still open in the other repository, and it fired only on removals, so a field going nullable or a version constant moving consulted nothing. The review now reads the companion pull requests - a `Depends-on:` line in the description, or any pull request in a related repository that links this one - and shows each on its own branch, with state, head and diff. Where it found a file on a related repository's default branch it reads the same file on the companion's branch and states whether the name survives there. Changed contracts are searched alongside removals: a serialised field whose declaration changed, an exported declaration whose line changed, a constant's old value. `@ankraai` replies see the same companions.
