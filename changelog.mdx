...
Organisation admins can now create their own **CNAME, A, and TXT records** under their organisation's delegated `ankra.cc` subdomain - handy for aliasing an add-on's auto-generated hostname to something memorable. Add and remove records from **Organisation Settings → DNS records**; new records provision asynchronously and show their state (pending, active, or failed) in the list.
