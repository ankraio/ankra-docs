...
A stack's new **Runtime** tab answers what its add-ons actually put on the cluster: the addresses it publishes, the pods it owns, the ports it exposes and what Kubernetes has said about them. **Endpoints** lists every host and path the stack's Ingresses serve as openable links, with TLS state and the backing `service:port`. **Pods** lists the pods the stack owns, unhealthy first, with the status `kubectl` prints rather than the phase that calls a crash-looping pod `Running`. **Services** carries the ports you need to wire another service up or forward one. **Events** is filtered to the stack's own objects, warnings first - a busy namespace's feed buries the one `FailedScheduling` that matters. Scope comes from the objects Ankra already attributes to the stack, followed down owner references, so a pod a chart's Deployment created counts as the stack's and a pod rolled seconds ago is placed through its ReplicaSet rather than going missing. Nothing absent reads as empty: a listing the cluster could not answer says so instead of reporting no pods, and a partial one marks its counts as a floor.
