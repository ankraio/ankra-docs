...
<Update label="2026-09-11" tags={["Clusters", "API"]} rss={{ title: "Self-managed k3s clusters on AWS EC2, inside a VPC you own" }}>

## Self-managed k3s clusters on AWS EC2, inside a VPC you own

Ankra now builds self-managed k3s or kubeadm clusters on EC2: a bastion in your public subnet, nodes in your private subnets with no public IP, the AWS cloud controller manager and EBS CSI driver on per-cluster instance profiles, a Network Load Balancer for Traefik, stop and start, node groups, upgrades, and teardown with a retention policy. Your existing AWS credential gains a **Self-managed clusters** scope. Closed beta, behind the `aws_provider` organisation flag (ankra-rtpno).
