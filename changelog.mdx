...
# Moving a Cluster to Another Organisation, Registry Image Retention, Moving a Docker Deployment Into a Cluster in One Command, Debug Pods and Recorded Terminal Sessions, Multi-Zone UpCloud Clusters on a WireGuard Mesh, A Nodes Section for Every Cluster, a Playground for Every Organisation, a New Default AI Model, Microsoft Teams at Slack Parity, Monorepo Demos, Add-on Publishing, Cluster Audit Trails, Power Schedules, Kubernetes Debugging From the CLI, the Largest CLI Release Yet, CISA KEV Exploit Intelligence in the Security Center, a Searchable Organisation Switcher, a DevSecOps Agent on Every AI Team, Per-Agent Models and Daily Token Limits, and a Verified Email Before Invitations

## Move a cluster to another organisation

A cluster can now change organisations without being re-imported. From the cluster's **⋯** menu, the cluster overview or **Settings → Danger Zone → Move cluster**, an organisation admin picks a destination organisation they also administer, types the cluster name, and the cluster - with its agent, executions, resources, findings, insights, DNS records and variables - now belongs there, still connected. Access grants, kube tokens, notification routes and mutes, report schedules and cluster group memberships from the previous organisation are detached and listed in the result; billing, audit and chat history stay where they were. Both organisations record the move in their audit trails. The move is refused while operations are running, while the cluster is in a cluster mesh, for playground clusters, and when the destination already has a cluster with that name. The same move is `ankra cluster move <cluster> --organisation <id|slug|name>` in the CLI and the `move_cluster_to_organisation` tool in Ankra AI, both behind the same two-organisation admin check.
