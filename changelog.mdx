...
The Applications list, the fleet row and the application journey said **Merge the setup pull request to go live** for applications that Ankra Pipelines already builds and deploys, and kept saying it for setup pull requests that had been closed before the close was being recorded. The setup pull request is a proposal to commit Ankra's generated files to your repository; once your application builds on an Ankra pipeline, or on a workflow your repository wrote itself, nothing waits on the merge and no surface asks for it. A push to the repository now also re-checks any setup pull request still recorded open against GitHub, so a merge or close the platform missed is recorded from GitHub's own answer. An application whose pipeline Ankra stores and runs is now recorded as building on Ankra Pipelines even while the committed pipeline file is still only in that pull request; **Reconcile** moves an existing application over.
