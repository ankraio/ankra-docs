...
Ankra's MCP endpoint now speaks the current Model Context Protocol revision: per-request protocol metadata instead of an `initialize` handshake, `server/discover`, `resultType` on every result, and the `Mcp-Method` and `Mcp-Name` headers checked against the body. Clients on 2025-11-25 and earlier are served at the same URL exactly as before, and the AI assistant's MCP client follows the same rules with its registered tool servers.
