...
<Update label="2026-09-16" tags={["Pipelines"]} rss={{ title: "A failed pipeline tells the person who pushed it, and breaking the default branch is critical" }}>
## A failed pipeline tells the person who pushed it, and breaking the default branch is critical

A failing run used to raise one organisation-wide card at warning severity, whatever it broke and whoever caused it. Now the run is attributed: whoever pushed gets a card addressed to them, alongside the organisation's own. A failure on the repository's default branch is **critical**; every other branch stays a warning. A failing pull request is never critical whatever it targets, because the change is not on that branch yet.

The card stands for the branch rather than the run, so a branch that fails three pushes in a row is one card, not three, and a later green run closes it and says which run fixed it. Everyone who was told it was broken is told it recovered, not only whoever pushed the fix.

Pull request runs now record the account that opened or pushed to them instead of the provider's name, so "Requested by" names a person.

[Notifications addressed to a person →](/guides/notification-routing#notifications-addressed-to-a-person)
</Update>
<Update label="2026-09-16" tags={["Pipelines"]} rss={{ title: "Choose whether notifications addressed to you arrive by email, phone or browser" }}>
## Choose whether notifications addressed to you arrive by email, phone or browser

**Settings** → **Notifications** → **How you are notified** now carries a switch and a severity floor for each of email, mobile push and browser. These govern the notifications addressed to you; an organisation's routing rules are unchanged and stay an administrator's surface.

Email is new for notifications, and defaults to critical only - warnings are opted up to rather than out of. Mobile push and browser default to critical and warnings. A member who never opens the page is on those defaults, so a broken branch reaches them without anything being configured.

Push for a notification addressed to you now follows your own preference rather than needing a routing rule an administrator wrote, and stops when you turn it off. Browser notifications are shown while Ankra is open and need the browser's own permission as well as the switch.

[How you are notified →](/guides/notification-routing#notifications-addressed-to-a-person)
</Update>
<Update label="2026-09-16" tags={["Fixes"]} rss={{ title: "Android devices receive routed notifications" }}>
## Android devices receive routed notifications

An Android phone could register for push and pass a test notification while never receiving a real one: the query that decides who a routed notification reaches still selected iOS devices only, which was correct for the week before Android delivery shipped beside it and wrong from that day on. Every enabled device is now selected, and the transport is chosen per device when it is sent.
</Update>
