...
On Hetzner, OVHcloud, UpCloud and DigitalOcean a stop used to lose everything stored in the cluster itself, and StatefulSets came back on empty disks. Stop now captures an encrypted snapshot of the cluster's state before the machines go, and the next start restores it, so workloads return with their Secrets and reattach their disks. Adding control planes no longer needs a stop either. See [Stop cluster](/platform/cluster-settings#stop-cluster) for the details and the opt-outs.
</Update>
