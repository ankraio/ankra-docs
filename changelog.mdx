...
Changes are saved against the stopped topology and applied the next time you click **Recreate Cluster**. Topology edits now also serialize against each other and against start/stop, so two operators clicking at the same time can't end up with a half-applied change.
