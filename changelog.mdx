...
## Order a playground at the size you need - priced 10% under UpCloud

The playground is no longer one fixed try-it size. **Create cluster** now offers **Playground** as a first-class option: pick a size - from the free Trial up to 16 vCPU / 32 GB - and Ankra provisions it on its own infrastructure, no cloud account or credentials needed. Each paid size is priced **10% below the UpCloud Cloud Native plan of the same vCPU/RAM shape**, and the picker shows UpCloud's list price next to Ankra's, so the saving is something you can check rather than something we claim.
