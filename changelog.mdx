...
<Update label="2026-09-08" tags={["CLI", "Applications"]} rss={{ title: "The CLI reads your checkout, so adding an application and running a pipeline need no flags" }}>

## The CLI reads your checkout, so adding an application and running a pipeline need no flags

`ankra pipeline run` and `ankra pipeline list` now resolve the application from the repository your `origin` remote points at, and the commit from your `HEAD`. Inside a checkout they need no flags at all, and they say which application and commit they used. Outside one, or when a repository carries more than one application, they ask for the flag and name the candidates.

`ankra application add . --wait` follows the analysis instead of ending on "Ankra is now analyzing the repository": it reports progress, prints the setup pull request's link when the analysis completes, and fails with the platform's reason when it does not.

Two refusals also stopped misleading people. Adding an application no longer requires the organisation's GitHub credential to be healthy for *every* repository every application binds to it: one application pointing at a repository the installation cannot reach used to hide the credential from `application add` entirely, which reported it as "no available GitHub credential found; install the Ankra GitHub App" even though the App was installed. Reachability is now judged for the repository being added, and a repository the installation genuinely cannot see is named, with the page that grants it. `ankra credentials get` says whether a credential is usable and, when it is not, the reason.
