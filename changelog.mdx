...
<Update label="2026-09-22" tags={["GitOps", "CLI", "API"]} rss={{ title: "A stack profile version can be deprecated, and nothing deploys it again" }}>
## A stack profile version can be deprecated, and nothing deploys it again

A published version could only ever be superseded. When one turned out to be incompatible, to carry a critical bug or to ship a CVE, every launch form, `apply`, rollout and in-place update still offered it. The owner can now **deprecate** a version with a reason, a note and the advisories it relates to - from the History tab or `ankra stack-profiles deprecate <profile> <version> --reason cve --note "Fixed in v4." --reference CVE-2026-12345` - and every lane then refuses it with that reason. Deployments already on the version are flagged in the Deployments tab, in `ankra stack-profiles deployments` and on the profile's `update_status`, so a fleet sitting on a withdrawn version is visible from the profile. The current version cannot be deprecated: make another version current first. `undeprecate` restores it.
