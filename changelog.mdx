...
Ankra now supports two-factor authentication, so you can protect your account with a second factor: an authenticator app, a passkey, or a security key.

## Set up 2FA on your account

Enrol an authenticator from your [User Profile](/essentials/user-profile) under **Authentication**: scan the QR code with your authenticator app, confirm the six-digit code, and store the one-time recovery code somewhere safe. You can add or remove authenticators and regenerate your recovery code at any time.

## Passkeys and security keys

In addition to authenticator apps, you can secure your account with a **passkey** (device biometrics such as Touch ID, Face ID, or Windows Hello) or a roaming **security key** (such as a YubiKey). These are offered when you sign in on a supported device, and once enrolled they appear next to your authenticator app on the **Authentication** page.

## Require 2FA across an organisation

Administrators can require two-factor authentication for everyone in an organisation from [Organisation Settings](/essentials/organisation-settings). Members who have not yet enrolled are guided to set up an authenticator before they can continue, and enrolled members are prompted for a code at sign-in - with the option to trust a device for 30 days.

## See and reset members' 2FA

The organisation members list shows each member's two-factor status at a glance. If a teammate loses their authenticator, an administrator can reset their 2FA as a break-glass action so they can re-enrol.

See [User Profile](/essentials/user-profile) and [Organisation Settings](/essentials/organisation-settings) for details.
