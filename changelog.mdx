...
`make-ready --pod-cidr` no longer moves a running cluster onto another pod range. kube-controller-manager exits at start while any node still holds a range outside `--cluster-cidr`, so that move took a cluster's controllers down instead of renumbering it. The request is now refused by name, and the update lane refuses to write a `--cluster-cidr` that excludes a node's range, so a cluster cannot be left mid-move. `--pod-cidr 10.244.0.0/16` still works, to put a cluster an earlier move split back onto the range its nodes run. Two clusters that both run the kubeadm default pod range cannot mesh with each other until the move is redesigned.
