...
## A force mode that reclaims UpCloud volumes and load balancers

Stopping or terminating an UpCloud cluster released its servers, network, and gateway, but never the storage volumes its workloads had provisioned through the CSI driver, and a load balancer could survive teardown too. The disks sat detached in the account and kept billing: on one of our own accounts, 76 orphaned volumes were 83% of the monthly spend before anyone noticed.

Stop and deprovision now take a force mode. `ankra cluster upcloud stop --force` and `ankra cluster deprovision --force` (or `?force=true` on the API) delete the cluster's CSI volumes and load balancers along with the rest of its infrastructure. Ankra records which volume UUIDs belong to the cluster before tearing anything down, so the sweep deletes exactly those disks and nothing else in the account, and a cluster that was stopped earlier without force can still be cleaned up by a forced terminate afterwards.

Without the flag nothing changes: a plain stop keeps your data so the cluster can come back, and a plain deprovision behaves as before. A forced stop destroys the data on those volumes, and the CLI says so before it asks you to confirm.
