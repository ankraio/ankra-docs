...
<Update label="2026-09-14" tags={["Clusters", "GitOps", "CLI"]} rss={{ title: "Protect a stack with a backup block in the cluster file" }}>

## Protect a stack with a backup block in the cluster file

A stack now carries its own backup policy: `backup: {enabled, vault, schedule, retention, selection}` in the cluster file, applied with `ankra cluster apply` like everything else. Ankra materialises it - a Velero `Schedule` for the volumes you selected, a CloudNativePG `ScheduledBackup` and object store per Postgres cluster, a schedule entry inside each Percona cluster - and those objects are derived from the block every time the stack renders rather than stored as stack resources, so no apply, clone or profile capture can leave a stack whose schedule and policy disagree. Databases are captured by default; volumes only when you name them. A file that omits the block leaves protection exactly as it was, and an apply that narrows protection says what stops being covered. `ankra cluster addons settings get|set` reads and writes the same policy per add-on. Closed beta, enabled per organisation.
