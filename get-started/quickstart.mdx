...
description: "Set up Ankra end to end: try the live playground, import or provision a Kubernetes cluster, build and deploy a stack, wire up GitOps and CI/CD, then operate it with AI - from the web UI, the ankra CLI, an MCP-connected agent, or your pipelines."
---
...
<Steps>
  <Step title="Start the playground">
    On your first visit, choose **Start building, no cluster needed**. You can also start one later from **Clusters** → **Try Playground**. You land straight on the cluster overview and watch it provision live; the starter stack lands as the first deploy in your activity feed.
  </Step>
  <Step title="Open your app, then ship something">
...
<Note>
Applications is in closed beta and is enabled per organisation on request - the full journey, demos, and PR previews are documented in [Applications](/concepts/applications).
</Note>

<Steps>
  <Step title="Connect a repository">
    **Applications** → **Create Application**, pick the repository and branch - or from a local checkout, simply:
    ...
    `--wait` follows the analysis and prints the setup pull request's link when it finishes, so you do not have to watch the portal for it. Without it the command returns as soon as the application exists and the analysis carries on in the background.
  ...
  <Step title="Review and merge the setup pull request">
  ...
  <Step title="Deploy to a cluster">
    ...
    ```bash
    ankra application deploy <app> --cluster <cluster> --set image_tag=sha-<short>
    ```
  ...
  <Step title="Get your URL">
    ...
    <CliVersion since="0.11.0" command="cluster domain" />

    ```bash
    ankra cluster domain <cluster>
    ```
...
<Tabs>
  <Tab title="CLI">
    ...
    Without Homebrew, the install script fetches the same binary:
  ...
  <Tab title="MCP (Claude, Cursor)">
    ...
    Then install Ankra's curated [Agent Skills](/integrations/agent-skills) - `ankra skills install` detects the assistants configured on your machine (Claude Code, the Claude app, Cursor, Codex, GitHub Copilot and more) and installs into all of them.
