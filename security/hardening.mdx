...
Ankra generates a **4096-bit RSA key pair per cluster**. The key is held authoritatively in Ankra's secret store (HashiCorp Vault or OpenBao) and is never stored in the cluster definition - exporting, cloning, or committing a definition to Git cannot leak key material.
...
Newly provisioned clusters start with the `ankra-security-baseline` stack profile: Trivy Operator for scanning, and Pod Security Standards enforced through the API server's own Pod Security Admission with a guard policy, so no policy engine has to sit on the admission path. Kyverno is available as the opt-in `ankra-policy-engine` profile. See [Security baseline](/guides/cluster-security#security-baseline).
