...
Export is gated by the same audit read permission as the audit viewer, held by **owners** and **admins** by default and grantable to [custom roles](/guides/roles-and-access). A caller without it receives a `403` naming the missing permission.
...
<Warning>
Truncation is silent if you only look at the file. A monthly export of a busy organisation can exceed the cap without any visible sign in the CSV itself. Always check the header or the JSON member, and narrow the date window until the export comes back untruncated.
</Warning>
