...
- **Values decrypted from Git** - values sealed with [SOPS](/guides/sops) are decrypted by the agent inside your cluster at deploy time, and the decrypted result is never sent back. Git holds ciphertext only. The control plane separately holds the values you entered in the Stack Builder - see [Data Handling](/security/data-handling#secret-lifecycle).
