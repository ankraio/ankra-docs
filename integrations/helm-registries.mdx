...
curl -H "Authorization: Bearer $TOKEN" \
  https://platform.ankra.app/api/v1/org/helm/registries

# Add an HTTP registry: the body is a resource specification, and the
# key under spec (helm_http_registry or helm_oci_registry) says which kind
curl -X POST -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"spec": {"helm_http_registry": {"name": "my-charts", "url": "https://charts.example.com"}}}' \
  https://platform.ankra.app/api/v1/org/helm/registries

# Add an OCI registry
curl -X POST -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"spec": {"helm_oci_registry": {"name": "ghcr-charts", "url": "oci://ghcr.io/org/charts"}}}' \
  https://platform.ankra.app/api/v1/org/helm/registries
```

A private registry adds `"credential_name"` next to `url`, naming a registry credential created beforehand.
