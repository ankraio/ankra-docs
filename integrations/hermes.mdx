...
<Steps>
  ...
  <Step title="Configure Core Settings">
    Click the component and set these values. Every value the chart reads lives
    under the top-level `openclaw` key:
  ...
  <Step title="Configure API Key">
    The chart renders `openclaw.secrets` into a Kubernetes Secret and injects
    each entry as an environment variable:
    ...
    Encrypt the value with SOPS before it reaches Git, or point the entry at an
    existing Secret (see [Production Setup](#production-api-key-management) below).
  </Step>
  ...
  <Step title="Enable Persistent Storage">
    Hermes stores workspace data and conversation memory. Persistence is on by
    default at 20Gi; shrink it if your cluster is small:
...
The gateway ingress publishes the webhook endpoints Hermes listens on,
`/api/hooks` and `/api/messages`, so a chat platform can reach it. The chart
renders those paths for you; you only supply the host, class and TLS.
...
openclaw:
  ingress:
    ...
    hosts:
      - host: hermes.your-cluster.ankra.cc
    tls:
      - secretName: hermes-tls
        hosts:
          - hermes.your-cluster.ankra.cc
```

<Note>
On an Ankra-provisioned cluster the networking stack installs Traefik as the
default ingress class, the `letsencrypt-prod` ClusterIssuer, and external-dns
bound to the cluster's delegated `ankra.cc` subdomain. Use a host under that
subdomain and the DNS record and certificate are created for you. See
[UpCloud Clusters](/guides/upcloud-clusters) for where the subdomain comes from.
</Note>

Alternatively, use port-forwarding for local access:

```bash
kubectl port-forward svc/hermes-openclaw 8080:80 -n hermes
```
...
<Steps>
  ...
  <Step title="Keep the value out of Git">
    The chart takes the key as a literal in `openclaw.secrets`, so encrypt that
    value with [SOPS](/guides/sops) and let Ankra decrypt it at deploy time:
    ...
    Add the path to the stack's `encrypted_paths` so the plaintext never reaches
    the repository.
  </Step>
</Steps>
...
Without Homebrew, use the install script instead:
...
| Helm chart not found | Search for `openclaw` in the chart picker. The **OpenClaw** registry (`oci://registry.gitlab.com/xrow-public/helm-openclaw/charts/openclaw`) is global, so it needs no setup. |
