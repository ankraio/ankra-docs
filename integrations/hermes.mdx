...
<Tip>
Hermes can also consume Ankra's hosted [MCP server](/integrations/mcp) for a
structured, scope-gated tool surface alongside (or instead of) the CLI skill
below.
</Tip>
...
- `ankra cluster draft -f <file>` -- Stage stack changes as reviewable drafts
- `ankra cluster apply -f <file>` -- Apply cluster and stack configuration
...
Hermes: I'll create a new stack called "monitoring" on staging-cluster.
        Running `ankra cluster draft -f cluster.yaml`... Draft created.
...
Hermes: Building...
        Updating `cluster.yaml` with the approved app-data stack.
        Adding postgresql-ha, redis, and the SOPS secret manifest...
        Running `ankra cluster apply -f cluster.yaml --wait`. Watch progress in Operations.
```
