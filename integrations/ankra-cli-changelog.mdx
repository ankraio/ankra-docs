...
A big release: stage changes as drafts, validate specs server-side, run `kubectl` through Ankra, query metrics, manage stack profiles and support tickets from the terminal, and keep the CLI current with a built-in self-update.
...
OVH gains region discovery and full node-group management, including Kubernetes labels and taints: `ankra cluster ovh node-group add ... --labels --taints`, plus `node-group labels` and `node-group taints` subcommands to update them later.
