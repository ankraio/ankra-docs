...
## Propose and review stack profile changes

The drafts family grew the cross-organisation review lane. `ankra stack-profiles drafts create --profile` on a profile another organisation owns opens a suggestion draft; `drafts submit-suggestion` sends it to the owner for review, and `ankra stack-profiles suggestions list | get | approve | reject | withdraw` works the queue - approving publishes the proposal as the profile's next version. `drafts rebase` moves a stale draft of your own profile onto its latest published version, keeping or discarding your edits.

```bash
ankra stack-profiles drafts create --profile <profile-id>
ankra stack-profiles drafts submit-suggestion <draft-id> --title "Raise the default replica count"
ankra stack-profiles suggestions approve <profile-id> <suggestion-id>
```

[Stack Profile Suggestions →](/guides/stack-profile-suggestions)
...
`ankra stack-profiles list | export-iac | import` manages reusable, organisation-level stack profiles as `ClusterInfrastructureAsCode` YAML (export a profile version, import one from a file). The family has since grown much further - drafts, suggestions, sharing, demos - see the [current reference](/reference/cli/stack-profiles).
