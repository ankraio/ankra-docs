...
Ankra AI can review Bitbucket Cloud pull requests automatically and answer when someone mentions `@ankraai` in a pull request comment, with the change as context. Both are switched on per workspace under **Organisation settings → Integrations → Bitbucket** (**AI review** and **Mention replies**), with repository rules for exceptions, and behave as described for GitHub in [Ankra AI on pull requests](/integrations/github#ankra-ai-on-pull-requests). PR previews are GitHub-only.
