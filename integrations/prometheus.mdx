...
- **Expect no Git commit.** On a cluster connected to a Git repository the call ends with the platform's routine push of the cluster state, which commits nothing when the branch already matches - the usual case. If that push is refused, the call answers `422 Failed to push changes to Git repository` *after* the credential has been saved: verify with the query below instead of retrying. To skip the push altogether, send your existing `prometheus_metrics` block verbatim alongside the credential; a save that changes only the metrics members never pushes.
