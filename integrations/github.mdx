...
<Warning>
**Off by default, and the control is not in the portal yet.** Merge when green is a per-installation switch, `merge_when_green`, stored on the same binding profile as **AI review** and **Mention replies** and off for every installation until an organisation admin turns it on. The portal and the CLI render no control for it yet, and the binding-profile route that carries it is the browser-session route, so it cannot be set from an API token either; until a control ships, every request below is refused with the capability-off reason. The binding's **AI mode** must also be **Agent**, or a request is refused saying so.
</Warning>
