...
The Ankra GitHub App can also take part in the pull requests of every repository it is installed on. The capabilities are switched on per installation under **AI** → **Settings** → **Connections** → **Source control & AI review** → **GitHub**, and each can be overridden for an individual repository with a repository rule.

[AI code review](/platform/ai-code-review) is the canonical guide to those switches: what each one does, how repository rules layer over the installation-wide defaults, which model a review runs on, what the review posts, and when one is skipped. This page covers what is specific to GitHub.

| Switch | GitHub specifics |
| --- | --- |
| **AI review** | For a repository connected to [Ankra Pipelines](/guides/ankra-pipelines), the review resolves the newest pipeline run of the reviewed commit before it falls back to a GitHub check run, and cites the run's own facts - failing step, findings, gate verdict, build digest - see [What a pull request gets](/guides/ankra-pipelines#what-a-pull-request-gets). |
| **Mention replies** | Answers when someone mentions `@ankraai` in a pull request comment, with the change as context. See [Ask Ankra in a comment](#ask-ankra-in-a-comment) below. |
| **PR previews** | GitHub only. Deploys a throwaway preview of an application's pull request build - see [Preview demos](/concepts/applications#preview-demos). |
...
<Note>
**Off by default, per installation.** Turn it on with the **Merge when green** switch on the installation's row under **Settings** → **Connections** → **Source control & AI review**, next to the **AI mode** control it depends on: the binding's AI mode must be **Agent**, or every request is refused saying so. Organisation admins only, GitHub only.

The same switch is `merge_when_green` on the binding profile, for scripts and CI: `PUT /api/v1/org/ai-gateway/scm-bindings/github/<installation id>` with a personal access token, sending `ai_review` and `mention_replies` as they are plus `merge_when_green: true`; a later request that omits the field leaves it as stored. `GET /api/v1/org/ai-gateway/scm-bindings` lists every installation with its value.
</Note>
...
<Steps>
  ...
  <Step title="Choose the installation-wide switches">
    On the **All repositories** row, turn on **AI review**, **Mention replies** and **PR previews** as needed. They apply to every repository the installation covers. **Merge when green** sits in the row's header next to **AI mode**, because it needs **Agent** mode to act; it has no per-repository rule.
  </Step>
...
For the review model, the per-pull-request review cap, draft handling, and your organisation's review instructions, see [AI code review](/platform/ai-code-review).

**Related repositories** sits at the bottom of each installation's panel and is GitHub-only, because the cross-repository search runs on GitHub code search. Use it to tell a review that two repositories belong together when nothing in your deploy already says so - a client and the API it calls, or a shared library and the service that consumes it. Both are picked from drop-downs listing the repositories that installation can reach, since the review reads them with that installation's token. See [Relating two repositories directly](/platform/ai-code-review#relating-two-repositories-directly).
...
jobs:
  deploy:
    runs-on: ubuntu-latest
    steps:
      - name: Reconcile the cluster from its repository
        run: |
          curl -X POST \
            -H "Authorization: Bearer ${{ secrets.ANKRA_API_TOKEN }}" \
            https://platform.ankra.app/api/v1/org/clusters/imported/$CLUSTER_ID/reconcile
```
