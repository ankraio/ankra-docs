...
<Note>
**Administration** is optional. When granted, Ankra's **CI/CD** tab can read whether the repository's default `GITHUB_TOKEN` is read-only and offer a one-click fix to set it to read/write - a common cause of `write_package: denied` on the first build. Without it, Ankra still detects the problem and links you to the exact GitHub settings to change manually. Approve it under **Settings → Applications → Installed GitHub Apps → Ankra → Configure**.
</Note>
