...
Manage who can reach a cluster's Kubernetes API through Ankra's API proxy (organisation admins only). A grant maps an organisation member to a Kubernetes role, cluster-wide or limited to one namespace. See [Cluster Access](/guides/cluster-access) for roles, scopes, and how grants are reconciled into RBAC.
...
Manage the Ankra entries in your local kubeconfig so you can run `kubectl` against a cluster through Ankra's API proxy. See [Accessing Clusters with kubectl](/guides/kubeconfig) for the full flow.
...
level. See [Stack Profiles](/concepts/stack-profiles) for the concept.
