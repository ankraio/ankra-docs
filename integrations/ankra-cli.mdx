...
Once installed, the CLI can update itself in place — no need to re-run the install script:
...
Pin an exact release with `--version` (the leading `v` is optional) to upgrade **or** downgrade — a pinned version is installed whether it is newer, older, or the same as the running binary, so the same command rolls back a bad release. The download is verified against its published SHA-256 checksum before the binary is swapped; if a release has no checksum the upgrade aborts unless you pass `--allow-unverified`.

Opt into pre-release builds with the beta channel:
...
If the binary lives in a directory you cannot write (such as `/usr/local/bin`), re-run with `sudo ankra upgrade`.
...
# Validate locally without calling the API (structure, referenced-file YAML, dependency tree)
...
| Flag | Description |
|------|-------------|
| `--key` | Key (path) to encrypt — required |
...
`--labels` and `--taints` on `add` create the group with its scheduling rules in place. The `labels` and `taints` subcommands set them on every node in an existing group (an empty value clears them; a taint effect defaults to `NoSchedule`).
