...
| Read the log a crash-looping container left behind | `ankra cluster logs my-pod -n default --previous` |
| Ask one object why it is not ready | `ankra cluster describe pod my-pod -n default` |
| See only the events for one object | `ankra cluster events --for pod/my-pod -n default` |
| Find what just ate the memory | `ankra cluster top pods -A --sort-by memory` |
| Chat with the AI about live cluster state | `ankra chat "why is my pod crashing?"` |
| Confirm a write that agent-mode chat proposed | `ankra chat actions confirm <action_id>` |
| Inspect what your AI agents are doing | `ankra agents runs` |
| Answer the choice a blocked AI board ticket is waiting on | `ankra tickets decide T-8 --option a` |
| See which of your vulnerabilities CISA lists as exploited in the wild | `ankra security findings --known-exploited` |
| Read the fleet's security posture and CISA KEV exposure | `ankra security overview` |
| Clone a stack between clusters | `ankra cluster stacks clone` |
| Move a Docker deployment into a cluster | `ankra migrate up ./app --cluster production` |
| Register a backup vault | `ankra backup vaults create` |
| Encrypt a Secret's keys with SOPS | `ankra cluster encrypt manifest db-secret --key password` |
| Provision a Hetzner cluster | `ankra cluster hetzner create` |
| Mint a kubeconfig context | `ankra cluster kubeconfig add my-cluster --use` |
