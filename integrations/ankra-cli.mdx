...
- **Cloud Cluster Lifecycle** Provider-agnostic `upgrade`, `scale`, `node-group`, and `k3s-versions` (Hetzner/OVH/UpCloud/DigitalOcean auto-detected)
...
3. If your account requires MFA, open the browser challenge and wait for you to complete the second step
4. You can then use all ankra CLI commands
...
These verbs detect the cloud provider (Hetzner, OVH, UpCloud, or DigitalOcean) automatically from the cluster, so you no longer need to remember which provider a cluster runs on. They are the **recommended replacement** for the provider-specific `cluster {hetzner,ovh,upcloud,digitalocean} ...` commands documented below, which are deprecated and now print a runtime warning.
...
#### cluster upgrade
Upgrade the Kubernetes (k3s) version on all nodes of a cloud cluster. Only Hetzner, OVH, UpCloud, and DigitalOcean clusters can be upgraded.
...
#### cluster ssh-keys
View, replace, or re-sync the SSH key credentials attached to a Hetzner, OVH, UpCloud, or DigitalOcean cluster. The provider is detected from the cluster.
