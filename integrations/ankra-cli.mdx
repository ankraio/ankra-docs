...
| Read the platform's advisory for a CVE - the parsed NVD/OSV record, CISA's guidance, affected versions and your exposure | `ankra security advisory <cve>` | v0.14.0 (pre-release) |
| Accept the risk of a finding with a reason and a review deadline, after a server-side preview | `ankra security dispositions create --occurrence <id> --disposition accepted_risk --reason "…"` | v0.15.0 (pre-release) |
| Download the compliance evidence report an auditor asks for | `ankra security compliance export --format csv` | v0.15.0 (pre-release) |
| Switch a cluster's pod-security policies from audit to enforce | `ankra security policy-mode enforce --cluster production` | v0.15.0 (pre-release) |
...
| Extend the CLI with plugins | `ankra plugins` | v0.14.0 (pre-release) |
