...
- **Cloud Cluster Lifecycle** Provider-agnostic `upgrade`, `scale`, `node-group`, and `k3s-versions` (Hetzner/OVH/UpCloud auto-detected)
...
- **Agent Skills** Install the curated Ankra Agent Skills into Cursor or Claude Code — `--editor claude-code` targets `.claude/skills` (works offline)
...
#### cluster deprovision
Deprovision (stop) a managed cluster. Accepts a cluster name **or** ID, and falls back to the selected cluster. For cloud clusters (Hetzner, OVH, UpCloud) it routes to the provider-specific teardown endpoint so cloud resources are released.
...
These verbs detect the cloud provider (Hetzner, OVH, or UpCloud) automatically from the cluster, so you no longer need to remember which provider a cluster runs on. They are the **recommended replacement** for the provider-specific `cluster {hetzner,ovh,upcloud} ...` commands documented below, which are deprecated and now print a runtime warning.
...
#### cluster upgrade
Upgrade the Kubernetes (k3s) version on all nodes of a cloud cluster. Only Hetzner, OVH, and UpCloud clusters can be upgraded.
...
Cursor or Claude Code skills directory so your AI agent follows Ankra's recommended practices
for the CLI, ImportCluster YAML, stacks/addons, GitOps, CI/CD, SOPS secrets, Helm registries,
observability, alerts, Terraform, and cloud clusters. The skills are embedded in the CLI, so
installation works offline.

Skills install into Cursor by default. Pass `--editor claude-code` to target Claude Code's
`.claude/skills` directory instead (the flag also accepts `cursor`).
...
ankra skills install --editor claude-code --project .  # install into ./.claude/skills (project)
