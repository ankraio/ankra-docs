...
DigitalOcean API credentials store a DigitalOcean personal access token, used to provision and manage self-managed droplet clusters and [DigitalOcean Kubernetes (DOKS)](/guides/doks-clusters). The token is validated against the DigitalOcean API on creation.
...
For self-managed droplet clusters you also need an [SSH key credential](#ssh-key-credentials) - create one with `ankra credentials digitalocean ssh-key create --name my-key --generate`.
...
UpCloud API credentials store the API access for an UpCloud subaccount, used to provision and manage self-managed UpCloud clusters and [UpCloud Managed Kubernetes (UKS)](/guides/uks-clusters). The credential is validated against the UpCloud API on creation.
...
For self-managed UpCloud clusters you also need an [SSH key credential](#ssh-key-credentials) - create one with `ankra credentials upcloud ssh-key create --name my-key --generate`.
...
For Proxmox clusters you also need an [SSH key credential](#ssh-key-credentials) - create one with `ankra credentials proxmox ssh-key create --name my-key --generate`.
...
For Morpheus clusters you also need an [SSH key credential](#ssh-key-credentials) - create one with `ankra credentials morpheus ssh-key create --name my-key --generate`.
