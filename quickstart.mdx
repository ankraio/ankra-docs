...
-set apiKey=YOUR_API_KEY
  ...
  <Tab title="Provision a Managed Cluster">
    Ankra provisions production-ready K3s clusters on Hetzner, OVHCloud, UpCloud, or DigitalOcean with automated networking, bastion hosts, and NAT gateways.

    <Steps>
      <Step title="Add cloud credentials">
        Go to **Credentials** and add your cloud provider API key (Hetzner, OVH, UpCloud, or DigitalOcean). Don't have a Hetzner account? [Sign up for Hetzner Cloud](https://hetzner.cloud/?ref=jmvDfwl8QEmF) to get started.
      </Step>
 ...
 **Kubernetes version** pick a supported K3s version
    ...
    <Tip>
    You can also provision clusters from the CLI: `ankra cluster hetzner create`, `ankra cluster ovh create`, `ankra cluster digitalocean create`.
    </Tip>
  </Tab>
</Tabs>
