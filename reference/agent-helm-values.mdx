...
| `ci_worker_count` | `0` | Concurrent worker slots for [Ankra Pipelines](/guides/ankra-pipelines) steps. Zero - the default - means the agent runs no pipeline-step scheduler and does not advertise the capability. **Set through Ankra, not here** (below) |
| `ci_storage_class` | `""` | StorageClass for pipeline cache volumes and for a workspace claim that names none; empty is the cluster default. **Set through Ankra, not here** (below) |

<Warning>
`ci_worker_count` and `ci_storage_class` are the two values Ankra owns on the agent's behalf. Set them per cluster through the platform - `PUT /api/v1/org/clusters/{cluster_id}/agent/ci-settings`, or the CLI verb [Ankra Pipelines](/guides/ankra-pipelines#connect-a-repository-push-see-the-run) shows - and Ankra re-renders the agent's own release with them. A value you pass to `helm upgrade` yourself is overwritten the next time Ankra renders that release.
</Warning>
