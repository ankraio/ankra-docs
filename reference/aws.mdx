...
| `bastion_allowed_ips` | `["0.0.0.0/0"]` | CIDRs allowed to reach the bastion on SSH. The default opens it to everyone (key-only, password login off, sshd rate-limited on the host); name your own CIDRs to restrict it. Ankra's own egress addresses are always admitted so provisioning keeps working. Preflight reports the exposure as `bastion_ssh_exposure` |
