...
| Security group (bastion) | in the VPC | SSH from `bastion_allowed_ips` only |
...
| Control plane instances | the private subnets, or `node_subnet_ids` | No public IP; encrypted gp3 root volume; IMDSv2 required, hop limit 2 (the EBS CSI controller runs on the pod network, guarded by the [IMDS guard](#imds-guard)); provider ID `aws:///<zone>/<instance-id>`; node name = EC2 private DNS name |
