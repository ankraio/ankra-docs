...
| `cni` | `cilium` | CNI plugin. kubeadm clusters always use `cilium`; k3s clusters may pick `calico` or `flannel`. Only Cilium and Calico can enforce the [IMDS guard](#imds-guard) - on flannel the create result reports `imds_guard: unavailable` |
