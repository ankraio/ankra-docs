...
<Note>
The CCM creates Hetzner Cloud Load Balancers when you create Kubernetes `Service` resources of type `LoadBalancer`. These Load Balancers are managed by Hetzner Cloud, not by Ankra. Delete LoadBalancer services before [terminating the cluster](/guides/hetzner-clusters#terminate-cluster) to avoid orphaned resources and unexpected charges.
</Note>
