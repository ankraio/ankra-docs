...
| `/org/clusters/managed/{provider}/{cluster_id}/node-pools/{name}` | PATCH | Update a node pool (count and/or autoscaling bounds), or hand its count to an external autoscaler with `externally_managed` |
...
Every pool in the `node-pools` listing carries `externally_managed` (boolean) and `observed_count` (integer or null). A `PATCH` whose body is only `{"externally_managed": true}` hands the pool's node count to something outside Ankra, such as a cluster autoscaler you run; sending it beside any other field is refused. While the flag is set, scale, a `count` update, delete and size replacement answer `409` with `Node pool is externally managed; clear externally_managed before changing it through Ankra`, `count` stays the last value Ankra wrote, and `observed_count` is what the provider reports now (null when it could not be read). `{"externally_managed": false}` hands the pool back and adopts the provider's current count. See [externally managed node pools](/guides/managed-kubernetes#externally-managed-node-pools).
