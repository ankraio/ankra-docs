...
| `network_mode` | *derived* | `private_network` or `wireguard_mesh`; `wireguard_mesh` for a pool, and on a single zone it makes the cluster mesh-capable so zones can be added later. `LoadBalancer` Services, including the Traefik ingress, do not work on a `wireguard_mesh` cluster - see [the guide](/guides/upcloud-clusters#loadbalancer-services-do-not-work-on-a-meshed-cluster) |
