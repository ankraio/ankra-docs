...
`ankra cluster stacks protect <stack>` writes this same block without editing the file; `ankra cluster stacks unprotect <stack>` sets `enabled: false`. See [Protect a stack](/guides/backups#protect-a-stack).
...
| `encrypted_paths` | no | Key names Ankra seals with SOPS when it writes the manifest to Git. An entry names a key, not a location: every key of that name is sealed at every depth - see [What an Entry Actually Matches](/guides/sops#what-an-entry-actually-matches). On an update, omit it to keep the stored declaration; an explicit `[]` sent over the API clears it, while the CLI drops an empty list - see [An Empty or Absent Declaration](/guides/sops#an-empty-or-absent-declaration) |
