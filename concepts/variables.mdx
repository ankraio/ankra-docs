...
| Variable Name | Example Value | Use Case |
|---------------|---------------|----------|
| `cluster_domain` | `prod.example.com` | Override the [built-in cluster domain](#built-in-variables) for this cluster |
...
<Warning>
Secret variables are masked in the UI but are still stored as-is on the server. For encrypted storage, use [SOPS encryption](/guides/sops) instead.
</Warning>
...
| **Best For** | UI-only protection | Values sealed in your Git repository |
...
Two names are reserved and resolve from platform state without any variable being created:

| Variable | Resolves to |
|----------|-------------|
| `${{ ankra.cluster_domain }}` | The cluster's **public domain**: the organisation's [Preview domain](/platform/ai-workspaces#preview-domain) whenever a [custom DNS zone](/concepts/applications#serving-a-zone-in-your-own-dns-account-from-every-cluster) declared for the organisation or the cluster covers it (the cluster's own external-dns then publishes every hostname under it), otherwise the cluster's generated Ankra domain (`<cluster-id>.<org-id>.<ankra-domain>`). |
| `${{ ankra.stack_name }}` | The name the stack carries on its cluster, so a profile deployed twice renders distinct namespaces and hostnames. |

An organisation whose own domain cannot be registered as its Ankra domain (because it lives in the organisation's own DNS account) gets that domain as the cluster domain this way: set it as the Preview domain and declare it as a custom DNS zone, and `hermes.${{ ankra.cluster_domain }}` renders `hermes.example.dev` on every cluster. Every cluster that serves the zone resolves the same cluster domain, so give each deployment a hostname label that is unique across the organisation's clusters. `ankra cluster domain <cluster>` reports both the generated domain and the public domain, and Ankra AI's hostname advice uses the same value.

A stack, cluster, or organisation variable of the same name overrides the built-in. A cluster with no public domain at all leaves the reference literal, which an Ingress host rejects loudly rather than resolving to nothing.
...
## Managing variables via API and CLI

Organisation and cluster variables have full CRUD over the API and the [Ankra CLI](/integrations/ankra-cli), so you can manage them from scripts and pipelines, not just the UI. (Stack variables are managed as part of the stack via the CLI's stack commands.)
...
```bash
# Organisation scope - available to every cluster
ankra org variables set DB_HOST db.example.com --description "Primary DB"
ankra org variables list

# Cluster scope - shadows org variables on that cluster
ankra cluster variables set DB_HOST db.prod.example.com --cluster prod
```

`set` is an upsert, and values can be read from stdin with `-` so secrets can be piped in from a vault or password manager. See the [CLI reference](/integrations/ankra-cli) for the full command set.

### REST API

Both scopes expose the same list/create/update/delete shape under `/api/v1/org/variables` (organisation) and `/api/v1/org/clusters/imported/{cluster_id}/variables` (cluster), bearer-token authenticated. See the [API Reference](/api-reference/introduction) for endpoints and schemas.
