...
Beyond planning, Ankra's AI can manage the full Stack lifecycle: list, inspect, create, update, redeploy, clone, and delete Stacks. In chat every change is confirmation-gated; the same tools are available to MCP clients with the `mcp:write` scope - see the [MCP Tool Reference](/platform/mcp-tools#stacks).
...
Define stack-specific variables in the **Variables** tab:

1. Click the **Variables** tab in the Stack Builder
2. Click **Add Variable** to create a new entry
3. Enter a name and value
4. Click **Save Variables**

Use variables in your manifests and add-on values with the syntax:
...
<Info>
Stack variables have the highest precedence. They override cluster and organisation variables with the same name. See [Variables](/concepts/variables) for the full precedence hierarchy.
</Info>
...
Always deploy namespace manifests before the add-ons and resources that live in them, and prefer several focused stacks (one concern each) over one giant stack.
...
### Cloning a Stack (Within Cluster)

1. Click the stack's menu (⋯)
2. Select **Clone**
3. Edit the cloned stack as needed

### Cloning a Stack to Another Cluster

You can clone a stack from one cluster to another to replicate configurations across environments:

1. Click the stack's menu (⋯)
2. Select **Clone to Cluster**
3. Choose the target cluster
4. Optionally rename the stack
5. Click **Clone Stack**

The cloned stack is created as a draft in the target cluster, allowing you to review and configure secrets before deployment.

<Note>
Encrypted values (secrets) are not transferred during cross-cluster cloning for security reasons. You'll need to reconfigure them in the target cluster.
</Note>

[Learn more about cloning stacks →](/guides/clone-stack)
...
The stack page describes what a stack **declares** - its add-ons and manifests, the deployments that applied them, the changes behind them. The **Runtime** tab is the other half: what those add-ons actually put on the cluster, right now.

A stack is not a namespace. Several stacks routinely share one and one stack routinely spans several, so "the pods in the add-on's namespace" would claim a neighbouring stack's pods and miss the stack's own next door. Runtime scopes by ownership instead: Ankra already records every object it manages against the stack it belongs to, and follows owner references down, so the ReplicaSets, Jobs and Pods a chart's Deployment created count as the stack's. A pod rolled seconds ago - too new to be in that record - is placed through its ReplicaSet or Job, so it is never missing from its own stack.

Four panels sit under a summary of pod health, namespaces, pods, restarts, services, endpoints and recent warnings:

- **Endpoints** - every host and path the stack's Ingresses publish, as links you can open, each showing whether TLS terminates on it and the `service:port` behind it. This is the fastest answer to "what URL does this stack serve".
- **Pods** - the pods the stack owns, unhealthy first, with the status `kubectl` would print (`CrashLoopBackOff`, `Init:1/2`, `Terminating`) rather than the phase that calls a crash-looping pod `Running`, plus ready containers, restarts and age. Each row opens the pod.
- **Services** - the stack's Services and their ports (`80→8080/TCP`, with the node port where there is one), which is what you need to point another service at this one or to forward a port while debugging.
- **Events** - the Kubernetes events raised against this stack's objects and no others, warnings first, each naming the object it is about. A busy namespace's own event feed buries the one `FailedScheduling` that matters.

<Note>
Nothing absent reads as empty. A listing the cluster could not answer says so rather than reporting "no pods"; a listing that lost a namespace, or was cut short by the read limit, marks its counts as a floor (`12+`) and names what is missing. If Ankra cannot read which objects belong to the stack at all, Runtime shows nothing rather than guess - every pod in those namespaces would be as likely to belong to another stack.
</Note>

The tab reads the cluster only while it is open, and does not poll a cluster that is offline.
...
[Learn more about GitOps →](/concepts/gitops)
...
Check out the [API Reference](/api-reference/introduction) for endpoints to create, update, and apply stacks programmatically.