...
## Backup Override (Closed Beta)

<Warning>
**Closed beta.** The `backup` block on an add-on's settings is part of [Backups](/guides/backups), which is in closed beta and enabled per organisation on request. [Contact support](/platform/support) to have it turned on for your organisation.
</Warning>

An add-on's `backup` settings override the [backup policy of the stack it belongs to](/guides/backups#protect-a-stack), for the one member of a protected stack that has to be captured differently - a cache nobody wants copied to object storage every night, or a database that needs a longer tail than the rest.

**An add-on with no `backup` block follows its stack, and that is the right answer for almost every add-on.** The override is not a second place to turn protection on: a stack that is not protected captures nothing, whatever its add-ons say.

| Field | Meaning |
|-------|---------|
| `enabled` | Whether this add-on's data assets are captured under the stack's policy |
| `consistency` | How the capture is taken: `transactional`, `application`, `crash` or `logical`. **Leave it empty** unless you have a reason - empty keeps the engine's own answer, and an override claiming more than the engine delivers is a lie about the restore point |
| `schedule` | `hourly`, `daily`, `weekly` or a cron expression. Empty inherits the stack's |
| `retention.hourly` / `.daily` / `.weekly` / `.monthly` / `.yearly` | How many restore points of each period to keep for this add-on. Absent inherits the stack's |
| `retention.minimum_count` | Keep at least this many, however the tiers fall out |
| `retention.minimum_age` | A duration (`24h`, `720h`). Nothing younger is expired |

```bash
ankra cluster addons settings get cnpg-cluster
ankra cluster addons settings set redis-cache --backup-enabled=false
ankra cluster addons settings set cnpg-cluster \
  --backup-enabled=true --backup-consistency=transactional \
  --backup-retention-monthly=12
```

<Info>
**The Stateful profile turns it on.** Choosing **Stateful** for an add-on enables its backup override, because an add-on somebody chose stateful settings for is an add-on with data. As with every other setting, changing one by hand switches the profile to **Custom**.
</Info>
...
<CardGroup cols={2}>
  <Card title="Backups and restore points" icon="vault" href="/guides/backups">
    Protect a stack, take restore points, and restore one.
  </Card>
