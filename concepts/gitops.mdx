...
All three conditions have to hold together - that `path_regex`, an `age` key, and no field beyond those two and `encrypted_regex` - so the only rules at risk are those whose `path_regex` is character-for-character one of the two Ankra generates for the cluster's own directory:
