...
Build state, published images, deploys, demos, and container scanning are all reported per component, so an application whose api built and whose frontend did not says exactly that.
...
A demo runs one component. A single-app application has exactly one, so there is nothing to choose; a monorepo asks which component to demo and defaults to its primary one.

<Tabs>
  <Tab title="Portal">
    Open the application's **Demos** tab, pick a branch or enter a pull request number, and deploy. The tab shows active demos, the preview URL, and the remaining TTL. On a monorepo the launch dialog adds a **Component** picker, and the container port follows the component you pick.
  </Tab>
  <Tab title="CLI">
    ```bash
