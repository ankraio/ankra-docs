...
<Note>
The throwaway database is ephemeral by design: it starts empty on every deploy and is wiped with the namespace. Run your app's migrations on boot, or treat the demo as stateless. The database runs a pgvector-capable Postgres, so migrations using `CREATE EXTENSION vector` work. `.url` and `.password` references are always delivered through a Kubernetes Secret, never as plain env text.
</Note>

### Full-stack demos

An application rarely runs alone. When it needs a message broker, a cache, an operator, or anything else its Stack provides, a demo of the application by itself is not a demo of the thing you are trying to show.

Point an application's demo settings at a **stack profile** and every demo launch instantiates that profile as a real, demo-scoped Ankra Stack that lives and dies with the demo. Configure it under *Full-stack demo* in **Demo settings**: enable it, pick the profile, pin a version or track the current one, and bind its parameters (secret parameters are stored as Ankra secret slots exactly like demo environment secrets).

Everything about the demo namespace still applies - the Stack is instantiated **into the demo's own namespace**, so the resource quota bounds it and the TTL reaper removes it. Cluster-scoped resources in the profile are skipped rather than leaked into the cluster.

| | Single-service demo | Full-stack demo |
|---|---|---|
| Quota | Standard per-demo quota | 20 pods, 4 CPU / 8 Gi requests, 8 CPU / 16 Gi limits |
| Provisioning window | Standard | Longer, to allow the Stack to converge |
| Teardown | Namespace removed | Stack uninstalled first, then the namespace |

Use `${DEMO_NAMESPACE}` in a parameter value where the profile needs to address the demo's own namespace; it resolves per launch. The Demos tab badges full-stack demos with the profile name, and the demo details show the Stack's status and a "Stack instantiated" step alongside the usual progress.

Demos of applications with no stack profile designated are completely unchanged.

<Note>
Teardown removes the Stack before the namespace, and is bounded: if a Stack uninstall wedges, cleanup proceeds anyway after a grace period rather than leaving the demo namespace stranded.
</Note>
...
1. When a demo fails with a **startup crash** (CrashLoopBackOff or a container configuration error), Ankra dispatches a one-shot **pre-setup agent** for it automatically. The run appears on the **AI agents** page like any other mission.
