...
<Steps>
  ...
  <Step title="Delete the DNS records">
    `ankra org dns delete <record>`, or delete them from the DNS records table.
  </Step>
  ...
  <Step title="Re-enable the cluster domains">
    `ankra cluster domain <cluster> --enable` re-creates each cluster's zone under the new root - the staging cluster's zone comes back on its own - and the cluster's external-dns picks up the new zone on its next cloud-provider stack pass.
  </Step>
</Steps>
