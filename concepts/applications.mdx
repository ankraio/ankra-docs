...
**Ankra confines itself to one subzone of the domain you register.** Registering `example.com` creates `<org_short_id>.example.com` and nothing else. Records you already publish at the apex or under any other name in `example.com` are never read, written or deleted - not by the switch, and not by anything Ankra runs afterwards. The delegation is strict below that too: each cluster gets its own subzone under the organisation zone, and the `external-dns` Ankra installs on a cluster holds a token pinned to that cluster's subzone alone, so it cannot reach a name outside it.
