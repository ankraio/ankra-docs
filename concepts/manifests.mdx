...
Every manifest can carry an **AGENTS.md** - a markdown document of operational learnings (upgrade gotchas, safe rollout order, fields that must stay pinned) that teammates and Ankra's AI build up over time. Edit it from the **AGENTS.md** tab on the manifest in the Stack Builder; editing it never triggers a redeploy. See [Teach Your Agents with AGENTS.md](/platform/agents-md).
