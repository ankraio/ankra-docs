...
Installing or updating an add-on on the native engine runs
`helm upgrade --install`, and the step does not succeed until the release is
...
<Warning>
An update whose resources cannot become ready without a further action from you
will not stick under the default settings. The common case is a DaemonSet with
`updateStrategy: OnDelete`: its Pods keep the old spec until something deletes
them, so the release never reaches ready, Helm times out and rolls back, and the
add-on is left on its previous values.

Apply such a change either by deleting the affected Pods while the wait is still
running, or by setting `rollback_on_failure: false` on the add-on for that
update and deleting the Pods afterwards. In the second case the step still
reports a rollout timeout, but the new revision stays applied.
</Warning>
...
The native engine reconciles each add-on on its own schedule. The platform's
`reconcile_addons` maintenance loop dispatches a reconcile to the agent whenever
an add-on is due. An add-on that just changed or failed is checked again within
a minute or two; one that keeps coming back unchanged backs off in steps (5, 10,
20 minutes) to a check every 30 minutes, and a repeatedly failing one backs off
to once an hour. Any change you make to the add-on makes it due immediately.

The schedule lives in the platform, not in the agent. While a cluster cannot
reach Ankra its workloads keep running untouched, but nothing is reconciled,
deployed or self-healed until the connection returns; the agent reconnects on
its own and the next due reconcile picks up where things stand. If the same
drift reappears on three consecutive reconciles, self-heal for that add-on is
paused for six hours rather than fighting whatever keeps changing the resource.
